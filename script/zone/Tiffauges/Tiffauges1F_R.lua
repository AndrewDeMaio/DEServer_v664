...
dofile("../script/zone/Tiffauges/Tiffauges1F_R_MonsterInfo.lua")
...
function OnInitializeVariables(ZoneCreateInfo)
	...
	ZoneCreateInfo.Name = "Tiffauges1F_R"
	ZoneCreateInfo.TemplateZoneID = 1801
	...
	ZoneCreateInfo.EmptyTimeOut = 30 -- 15 sec
...
function OnInitialize()
	...
	-- 이계 던전 생성
	DEScriptZone:CreateInstanceDoungeon("Tiffauges/Tiffauges1F_O")
	
	-- 현세 던전 생성 설정
	if GetRace() == "Slayer" then
		DEScript:WriteIntegerOnWhiteBoard("Tiffauges.1F.EnterSlayer", 1)
	end
	if GetRace() == "Vampire" then
		DEScript:WriteIntegerOnWhiteBoard("Tiffauges.1F.EnterVampire", 1)
	end
	if GetRace() == "Ousters" then
		DEScript:WriteIntegerOnWhiteBoard("Tiffauges.1F.EnterOusters", 1)
	end
	...
	DEScriptZone:SetTimer(1, 1000 * 5, 1)
...
function OnTimer(TimerID)
	if TimerID == 1 then
		if DEScript:ReadIntegerOnWhiteBoard("Tiffauges.1F.Open", 0) == 1 then
	...
	if TimerID == 2 then
		...
		-- 이계 이동 설정
		if GetRace() == "Slayer" then
			DEScript:WriteIntegerOnWhiteBoard("Tiffauges.1F.EnterSlayer", 2)
		end
		if GetRace() == "Vampire" then
			DEScript:WriteIntegerOnWhiteBoard("Tiffauges.1F.EnterVampire", 2)
		end
		if GetRace() == "Ousters" then
			DEScript:WriteIntegerOnWhiteBoard("Tiffauges.1F.EnterOusters", 2)
		end
...
function OnWarpZone(Creature)
	--print("티포주 1F 현세->이계 이동중~~~~~")

	-- 현세 -> 이계 이동시킴
    WarpZoneID = DEScriptZone:CreateInstanceDoungeon("Tiffauges/Tiffauges1F_O")
