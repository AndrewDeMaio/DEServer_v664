...
function SendRemiainTime(Creature, remainTime)
	DEScriptZone:SendSystemMessage(Creature, 7, "Gentis Dungeon["..remainTime.."]~~~~~~")
end

function StartToIncreaseTrapStage(Interval)
	--DEScriptZone:SetTimer((100 + CurrentMissionRoom * 10), 1000 * 10, 1)	-- 飘乏 窜拌 ?�啊
	DEScriptZone:SetTimer(100, 1000 * Interval, 1)	-- 飘乏 窜拌 ?�啊
end
...
function WriteLog(logFileName, Content)
	DEScript:WriteFileLog(logFileName, Content)
end