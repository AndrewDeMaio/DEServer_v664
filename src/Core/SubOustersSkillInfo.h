...
public :
	...
	// get size of object
	// write() emits m_SkillType, m_ExpLevel, m_Interval and m_CastingTime, so
	// szExpLevel has to be counted here too. Without it every Ousters skill
	// under-declared by 2 bytes, which made GCSkillInfo announce a body far
	// shorter than it wrote. The Slayer and Vampire variants count correctly;
	// only this one was wrong, which is why it only ever hit Ousters.
