...
//#pragma warning (push, 1)
	// Core/Assert.h defines a one-argument Assert(expr) macro, which collides
	// with the two-argument XSerializeEngine::Assert member in Xerces 3.x.
	// Nothing below this point uses our Assert(), so just drop the macro.
	#undef Assert
...
// the DWORD overload lived here; DWORD is now `unsigned int`, so it is the same
...
// the DWORD overload lived here; see the note on AddAttribute above.
