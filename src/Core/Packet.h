...
// Packet Header Size
//----------------------------------------------------------------------
const uint szPacketHeader = sizeof(sPacketHeader);
...
public :
	...
	enum 
	{
		...
		PACKET_CU_REQUEST_LOGIN_MODE,				// 440  // login mode 알아내기
		PACKET_UC_REQUEST_LOGIN_MODE,				// 441	// login mode 알려주기
		...
		PACKET_GC_GUILD_RESPONSE,					// 445	// 결과메세지 보내주는 패킷
		PACKET_CG_REQUEST_GUILD_LIST,				// 446	// 길드 리스트 request (아무나) (O)
		PACKET_CG_APPOINT_SUBMASTER,				// 447	// 서브마스터 임명(길드 마스터)
		PACKET_CG_REQUEST_UNION,					// 448	// 연합가입신청(길드 마스터)
		PACKET_CG_ACCEPT_UNION,						// 449	// 연합가입수락(연합길드 마스터)
		PACKET_CG_DENY_UNION,						// 450	//  ..     거절 ..
		PACKET_CG_REQUEST_UNION_INFO,				// 451	// 연합정보요청(아무나 요청가능)
		PACKET_CG_EXPEL_GUILD,						// 452	// 연합추방 (연합길드 마스터)
		PACKET_CG_QUIT_UNION,						// 453	// 연합탈퇴 (길드 마스터) - 일방/신청 
		PACKET_CG_QUIT_UNION_ACCEPT,				// 454	// 탈퇴수락 (연합길드 마스터) 
		PACKET_CG_QUIT_UNION_DENY,					// 455	// ..  거절  .. 
		PACKET_GC_SHOW_UNION_INFO,					// 456	// 연합정보결과 
		PACKET_GC_UNION_OFFER_LIST,					// 457	// 연합 가입/탈퇴/도망신청 목록
		
		PACKET_GS_GUILD_ACTION,						// 458	// ign
		PACKET_SG_GUILD_RESPONSE,					// 459	// ign
		...
		PACKET_LC_SHOW_WEB_PAGE,					// 484	웹 브라우저를 띄운다.

		PACKET_CG_REQUEST_WEB_MARKET,				// 485  게임내 마켓 결재 페이지 요청
		PACKET_GC_SHOW_WEB_MARKET,					// 486  게임내 마켓 결재 페이지 보여주기

		PACKET_CG_CHANGE_GEARSLOT,					// 487  장착창 변경 요청
		PACKET_GC_CHANGE_GEARSLOT,					// 488  장착창 변경 결과 보여주기

		PACKET_GC_ADD_SYLPH,						// 489  아우, 고속이동 실프 변경 알림
		
		PACKET_CG_INSTANCEDOUNGEON_ENTER,			// 490
		PACKET_GC_INSTANCEDOUNGEON_LOBBY,			// 491
		PACKET_GC_INSTANCEDOUNGEON_RESULT,			// 492
		
		PACKET_GC_ADD_EFFECT_TO_SCREEN,				// 493
...
public :
	...
	inline void readTimeStamp (SocketInputStream & iStream) throw (ProtocolException, Error)
	{
		iStream.read(m_dwTimeStamp);
	}
