	 ...
	 throw (ProtocolException , Error)
{
	...
	string strPCName = pPacket->getPCName();
	
	// SQL Injection 방어
	if ( CheckSQLInjectionString(strPCName) )
	{
	    filelog("Attack_SQLInjection.log", "%s CLSelectPC : [PlayerID=%s PCName=%s]",
	   		connectIP.c_str(),
			pPlayer->getID().c_str(), 
			strPCName.c_str()
		);
...
*/
	Statement* pStmt_Pay   = NULL;
	Result*    pResult_Pay = NULL;
	bool 	   IsPayPlayer = false;
	
	BEGIN_DB
    {
		pStmt_Pay = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
        
		pResult_Pay = pStmt_Pay->executeQuery(
        ...
        SAFE_DELETE(pStmt_Pay);
    }
    END_DB(pStmt_Pay)
	
	
	// Test Server로 설정되어 있다면 유료사용자만 들어올 수 있다.
	if ( g_pConfig->hasKey("IsTestServer") )
	{
		bool bIsNewPC = false;
		
		Statement* pStmt_New   = NULL;
		Result*    pResult_New = NULL;
		
		BEGIN_DB
		{
			pStmt_New = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
			pResult_New = pStmt_New->executeQuery( "SELECT unix_timestamp(EndDate) - unix_timestamp(now()) from MakePCLog WHERE PlayerID= '%s' AND EndDate > now() ", pLoginPlayer->getID().c_str() );
			
			if ( pResult_New->getRowCount() > 0 )
			{
				bIsNewPC = true;	
			}
			else
			...
			SAFE_DELETE(pStmt_New);
		}
		END_DB(pStmt_New)
		
		if(g_pConfig->getPropertyInt("IsTestServer") == 1)
		{		
			if( IsPayPlayer = false && bIsNewPC == false)
			{				
				LCSelectPCError lcSelectPCError;
				lcSelectPCError.setCode( SELECT_PC_NOT_BILLING_CHECK );
				pLoginPlayer->sendPacket( &lcSelectPCError );
...
/*#ifdef __METRO_SERVER__
		// 2007 04 23 15세 미만이라면 피케이 서버에 들어갈 수 없다...
		if( pLoginPlayer->isUnderFifthteen() && !bNonPKServer ) 
		{
			LCSelectPCError lcSelectPCError;
			lcSelectPCError.setCode( SELECT_PC_CANNOT_PLAY_BY_AGE );
			pLoginPlayer->sendPacket( &lcSelectPCError );
			SAFE_DELETE(pStmt);
			return;
		}
		...
		{
			ZoneInfo* pZoneInfo = g_pZoneInfoManager->getZoneInfo(zoneID, pPacket->getPCType());
...
//		g_pGameServerManager->sendPacket(pGameServerInfo->getIP() , 6997 , &lgIncomingConnection);
