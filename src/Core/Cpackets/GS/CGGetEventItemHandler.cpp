	 ...
	 throw (ProtocolException , Error)
{
	...
	else if ( pPacket->getEventType() == EVENT_TYPE_200507_COMEBACK_ITEM )
	{
		executeComebackItem200507( pPacket, pPlayer );
	}
	else if ( pPacket->getEventType() == EVENT_TYPE_200507_COMEBACK_PREMIUM_ITEM )
	{
		executeComebackPremiumItem200507( pPacket, pPlayer );
	}
	else if ( pPacket->getEventType() == EVENT_TYPE_200507_COMEBACK_RECOMMEND_ITEM )
	{
		executeComebackRecommendItem200507( pPacket, pPlayer );
	}
...
void CGGetEventItemHandler::executeComebackItem200507( CGGetEventItem* pPacket, Player* pPlayer )
	throw ( ProtocolException, Error )
{
	__BEGIN_TRY
	...
	{
		pStmt = g_pDatabaseManager->getDistConnection( "PLAYER_DB" )->createStatement();
		Result* pResult = pStmt->executeQuery( "SELECT RecvItemDate FROM Event200507Main WHERE PlayerID = '%s'",
													pGamePlayer->getID().c_str() );
		...
		if ( pResult->next() )
		{
			...
			if ( RecvItemDate == "0000-00-00" )
			{
				...
				// 가져갔다고 DB 에 기록한다.
				pStmt->executeQuery( "UPDATE Event200507Main SET RecvItemDate = now() WHERE PlayerID = '%s'", pGamePlayer->getID().c_str() );
				...
				pNicknameBook->addNewNickname( "Summer Come Back!" );
				Packet* pNicknamePacket = pNicknameBook->getNicknameBookListPacket();
				pGamePlayer->sendPacket( pNicknamePacket );
				SAFE_DELETE( pNicknamePacket );
			}
...
void CGGetEventItemHandler::executeComebackPremiumItem200507( CGGetEventItem* pPacket, Player* pPlayer )
	throw ( ProtocolException, Error )
{
	__BEGIN_TRY
	...
	{
		pStmt = g_pDatabaseManager->getDistConnection( "PLAYER_DB" )->createStatement();
		Result* pResult = pStmt->executeQuery( "SELECT PayPremiumDate, RecvPremiumItemDate FROM Event200507Main WHERE PlayerID = '%s'",
													pGamePlayer->getID().c_str() );
		...
		if ( pResult->next() )
		{
			...
			if ( RecvItemDate == "0000-00-00" )
			{
				...
				// 가져갔다고 DB 에 기록한다.
				pStmt->executeQuery( "UPDATE Event200507Main SET RecvPremiumItemDate = now() WHERE PlayerID = '%s'", pGamePlayer->getID().c_str() );
...
void CGGetEventItemHandler::executeComebackRecommendItem200507( CGGetEventItem* pPacket, Player* pPlayer )
	throw ( ProtocolException, Error )
{
	__BEGIN_TRY
	...
	{
		pStmt = g_pDatabaseManager->getDistConnection( "PLAYER_DB" )->createStatement();
		Result* pResult = pStmt->executeQuery( "SELECT UniqueID, RecvItemDate FROM Event200507Recommend WHERE PlayerID = '%s'",
													pGamePlayer->getID().c_str() );
		...
		if ( pResult->next() )
		{
			...
			if ( RecvItemDate == "0000-00-00" )
			{
				...
				// 가져갔다고 DB 에 기록한다.
				pStmt->executeQuery( "UPDATE Event200507Recommend SET RecvItemDate = now() WHERE UniqueID = '%d'", UniqueID );
