	...
	throw (ProtocolException, Error)
{
	...
	if ( !pZone->NotifyUseItemInGear(pCreature, pItem->getItemClass(), pItem->getItemType()))
	{
		GCCannotUse _GCCannotUse;
		_GCCannotUse.setObjectID(pPacket->getObjectID());
		pGamePlayer->sendPacket(&_GCCannotUse);
...
//
//////////////////////////////////////////////////////////////////////////////
bool CGUseItemFromGearHandler::isValidTargetZoneByCoupleRing(Zone *pTargetZone)
{
	bool bValidTargetZone = false;
	...
	bValidTargetZone = 
		pTargetZone->getZoneID() != 2101 && pTargetZone->getZoneID() != 2102 &&
		 ...
		 pTargetZone->getZoneID() != 1130 && pTargetZone->getZoneID() != 1125 && pTargetZone->getZoneID() != 1121 && pTargetZone->getZoneID() != 1128 && pTargetZone->getZoneID() != 1129 &&
		 !pTargetZone->isDynamicZone() && !pTargetZone->isHellGardenTower() && pTargetZone->getZoneID() != 5000 && pTargetZone->getZoneID() != 5010 && pTargetZone->getZoneID() != 5020 && pTargetZone->getZoneID() != 5030 &&
	...
	throw (ProtocolException, Error)
{
	...
	{
		Ousters* pOusters = dynamic_cast<Ousters*>(pPC);
		pCoupleRing = dynamic_cast<CoupleRingBase*>(pOusters->getWearItem( (Ousters::WearPart)(pPacket->getPart()) ));
	}
	else
	{
		Assert(false);
	...
	if ( pTargetZone != NULL )
	{
		bValidZone = isValidTargetZoneByCoupleRing(pTargetZone);

	}
	filelog("LoveChain.txt", "Use Owner Name : %s, Target Name : %s, ZoneID:%d, TargetZoneID:%d", pPC->getName().c_str(), targetName.c_str(),
			pZone->getZoneID(), pTargetZone->getZoneID());
