	...
	throw (ProtocolException, Error)
{
	...
	else
	{
		...
		// 2006 10 09 ContractOfBlood
		if( checkContractOfBlood( pNewItem ) )
		{
			pPC->initAllStatAndSend();
		}
		...
		{
			pPC->addTimeLimitItem( pNewItem, pPC->getLimitTime(pItem) );     // 20080913   
		...
		makeGCCreateItem(&gcCreateItem, pNewItem, InvenX, InvenY);
		
		// 2007 03 29 Add
		gcCreateItem.setInventoryItemObjectID(pPacket->getInventoryItemObjectID());
