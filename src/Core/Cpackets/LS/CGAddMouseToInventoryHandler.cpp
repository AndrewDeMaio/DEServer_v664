//////////////////////////////////////////////////////////////////////////////
// Filename    : CGAddMouseToInventoryHandler.cpp
...
//				cout << "근데 서브 인벤토리가 없다." <<endl;
		...
		// checkContractOfBlood(pPC, pItem, InvenX, InvenY, pInventory->getContractOfBlood() );
		...
		if (pPrevItem != NULL) 
		{
			//wlzzi 081027 - 영구/시간제 아이템에 대한 검사를 여기서 할 경우 문제가 발생한다.
			//				- 기본적으로 영구/시간제 아이템은 .. 위치 교환이 가능해야 하기 때문.
			//				- 이 검사는 .. 동일한 아이템을 합칠 때 해야 한다.
			//				- 수정방안 : 영구/시간제 아이템일 경우, Stackable검사를 하지 않도록 처리... 서로 다른 아이템을  간주되도록!!
			// 둘다 같은 영구/시간제 아이템이 아니면 실패
			bool bisCheckStackable = true;
		
  			if ( pItem->isTimeLimitItem() ||  pPrevItem->isTimeLimitItem()
			)
			{
				bisCheckStackable = false;
			}
...
//			if (canStack(pItem, pPrevItem))
			if (bisCheckStackable && canStack(pItem, pPrevItem))
			{
		...
		if (Success) 
		{
			...
			if ( pItem != NULL && pItem->getItemClass() == Item::ITEM_CLASS_EVENT_TREE)
			{
				...
				else if ( itemtype > 41 && itemtype <= 53 )
				{
					// 크리스마스 트리 조각이 맞춰지는지 본다.
					TPOINT pt = checkEventPuzzle( pPC, InvenX, InvenY, 42 );
					if ( pt.x != -1 && pt.y != -1 )
					{
						// 맞춰진 트리 조각을 지운다.
						deleteInventoryItem( pInventory, pt.x, pt.y, pt.x + 2, pt.y + 3 );
						...
						// 트리를 생성한다.
						list<OptionType_t> optionType;
						Item* pTreeItem = g_pItemFactoryManager->createItem( Item::ITEM_CLASS_EVENT_TREE, 54, optionType );
						pTreeItem->setQuestItem();
						pZone->getObjectRegistry().registerObject( pTreeItem );
						pInventory->addItem( pt.x, pt.y, pTreeItem );
						pTreeItem->create( pPC->getName(), STORAGE_INVENTORY, invenID, pt.x, pt.y );
				...
				else if ( itemtype > 54 && itemtype <= 66 ) // 20080829 블리츠 문양 맞추기
				{
					// 블리츠 문양 조각이 맞춰지는지 본다.
					TPOINT pt = checkEventPuzzle( pPC, InvenX, InvenY, 55 );
					if ( pt.x != -1 && pt.y != -1 )
					{
						// 맞춰진 블리츠 문양조각을 지운다.
						deleteInventoryItem( pInventory, pt.x, pt.y, pt.x + 2, pt.y + 3 );
						...
						// 블리츠 문양을 생성한다.
						list<OptionType_t> optionType;
						Item* pTreeItem = g_pItemFactoryManager->createItem( Item::ITEM_CLASS_EVENT_TREE, 67, optionType );
						pTreeItem->setQuestItem();
						pZone->getObjectRegistry().registerObject( pTreeItem );
						pInventory->addItem( pt.x, pt.y, pTreeItem );
						pTreeItem->create( pPC->getName(), STORAGE_INVENTORY, invenID, pt.x, pt.y );

						// 클라이언트에 블리츠 문양이 만들어졌다는 걸 알린다.
						GCCreateItem gcCreateItem;
						makeGCCreateItem( &gcCreateItem, pTreeItem, pt.x, pt.y );
						pGamePlayer->sendPacket(&gcCreateItem);
					}
				...
				//080925 smallheart 가을 낙엽이벤트 조각 맞추기.
				else if ( itemtype > 67 && itemtype <= 79 ) // 20080829 블리츠 문양 맞추기
				{
					// 낙엽 문양 조각이 맞춰지는지 본다.
					TPOINT pt = checkEventPuzzle( pPC, InvenX, InvenY, 68 );
					if ( pt.x != -1 && pt.y != -1 )
					{
						// 맞춰진 낙엽 문양조각을 지운다.
						deleteInventoryItem( pInventory, pt.x, pt.y, pt.x + 2, pt.y + 3 );
						...
						// 낙엽 문양을 생성한다.
						list<OptionType_t> optionType;
						Item* pTreeItem = g_pItemFactoryManager->createItem( Item::ITEM_CLASS_EVENT_TREE, 80, optionType );
						pTreeItem->setQuestItem();
						pZone->getObjectRegistry().registerObject( pTreeItem );
						pInventory->addItem( pt.x, pt.y, pTreeItem );
						pTreeItem->create( pPC->getName(), STORAGE_INVENTORY, invenID, pt.x, pt.y );

						// 클라이언트에 낙엽 문양이 만들어졌다는 걸 알린다.
						GCCreateItem gcCreateItem;
						makeGCCreateItem( &gcCreateItem, pTreeItem, pt.x, pt.y );
						pGamePlayer->sendPacket(&gcCreateItem);
					}
				}
				//1219 wlzzi - 2008 12월 이벤트 - 양말문양 조각맞추기
				else if ( itemtype > 80 && itemtype < 90)
				{
					// 양말 문양 조각이 맞춰지는지 본다.
					int width = 3;
					int height = 3;
					TPOINT pt = checkEventPuzzle( pPC, InvenX, InvenY, 81, width, height );
					if ( pt.x != -1 && pt.y != -1 )
					{
						// 맞춰진 낙엽 문양조각을 지운다.
						deleteInventoryItem( pInventory, pt.x, pt.y, pt.x + (width-1), pt.y + (height-1) );

						// 낙엽 문양을 생성한다.
						list<OptionType_t> optionType;
						Item* pSocksItem = g_pItemFactoryManager->createItem( Item::ITEM_CLASS_EVENT_GIFT_BOX, 39, optionType );
					//	pSocksItem->setQuestItem();
						pZone->getObjectRegistry().registerObject( pSocksItem );
						pInventory->addItem( pt.x, pt.y, pSocksItem );
						pSocksItem->create( pPC->getName(), STORAGE_INVENTORY, invenID, pt.x, pt.y );

						// 클라이언트에 낙엽 문양이 만들어졌다는 걸 알린다.
						GCCreateItem gcCreateItem;
						makeGCCreateItem( &gcCreateItem, pSocksItem, pt.x, pt.y );
						pGamePlayer->sendPacket(&gcCreateItem);
					}
				}
				//
				else if ( itemtype > 89 && itemtype < 99)
				{
					// 루돌프 문양 조각이 맞춰지는지 본다.
					int width = 3;
					int height = 3;
					TPOINT pt = checkEventPuzzle( pPC, InvenX, InvenY, 90, width, height );
					if ( pt.x != -1 && pt.y != -1 )
					{
						// 맞춰진 루돌프 조각을 지운다
						deleteInventoryItem( pInventory, pt.x, pt.y, pt.x + (width-1), pt.y + (height-1) );
						...
						// 루돌프 문양을 생성한다.
						list<OptionType_t> optionType;
						Item* pSocksItem = g_pItemFactoryManager->createItem( Item::ITEM_CLASS_EVENT_TREE, 99, optionType );
					//	pSocksItem->setQuestItem();
						pZone->getObjectRegistry().registerObject( pSocksItem );
						pInventory->addItem( pt.x, pt.y, pSocksItem );
						pSocksItem->create( pPC->getName(), STORAGE_INVENTORY, invenID, pt.x, pt.y );

						// 클라이언트에 루돌프 문양이 만들어졌다는 걸 알린다.
						GCCreateItem gcCreateItem;
						makeGCCreateItem( &gcCreateItem, pSocksItem, pt.x, pt.y );
						pGamePlayer->sendPacket(&gcCreateItem);
					}
