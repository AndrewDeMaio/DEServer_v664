	 ...
	 throw (Error)
{
	...
	{
		executeSkillFailException(pSlayer, SKILL_THROW_BOMB);
	...
	try
	{
		...
		SlayerSkillSlot *pSkillSlot = pSlayer->hasSkill(SKILL_THROW_BOMB);
...
//			increaseAlignment(pSlayer, SKILL_DOMAIN_GUN, _GCThrowBombOK1);
	...
	{
		executeSkillFailNormal(pSlayer, SKILL_THROW_BOMB, NULL);
