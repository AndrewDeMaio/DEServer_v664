	 ...
	 throw (ProtocolException , Error)
{
	...
	BYTE DonationType = pPacket->getDonationType();

	if ( DonationType == DONATION_TYPE_200501_PERSONAL || DonationType == DONATION_TYPE_200501_GUILD )
	{
		// 기부 횟수
	...
	// 결혼 축의금 이벤트
	else if ( DonationType == DONATION_TYPE_200505_WEDDING )
	{
		// 기부 이벤트가 활성화된 상태인지 확인한다.
		if ( g_pVariableManager->getVariable( DONATION_EVENT_200505 ) != 1 )
		{
			return;
		}
		...
		bool bAleadyDonated = false;
		{
			...
			{
				pStmt = g_pDatabaseManager->getDistConnection( "PLAYER_DB" )->createStatement();
				pResult = pStmt->executeQuery( "SELECT Name FROM DonationWedding200505 WHERE Name = '%s' AND WorldID = %d", pCreature->getName().c_str(), affectWorldID );
		...
		if ( bAleadyDonated )
		{
			gcNPCResponse.setCode( NPC_RESPONSE_SHOW_ALEADY_DONATED_DIALOG );
			pPlayer->sendPacket( &gcNPCResponse );
			return;
		}
		...
		// DB 에 기록을 남긴다.
		{
			...
			{
				pStmt = g_pDatabaseManager->getDistConnection( "PLAYER_DB" )->createStatement();
				pStmt->executeQuery( "INSERT INTO DonationWedding200505 VALUES ( '%s', '%s', %d, %u, now() )",
											pGamePlayer->getID().c_str(),
											pCreature->getName().c_str(),
											affectWorldID,
											pPacket->getGold() );
		...
		// 결과를 알린다.
		gcNPCResponse.setCode( NPC_RESPONSE_SHOW_DONATION_WEDDING_COMPLETE_DIALOG );
		pPlayer->sendPacket( &gcNPCResponse );
	}
