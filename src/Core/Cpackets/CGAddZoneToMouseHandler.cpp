	...
	throw (ProtocolException, Error)
{
	...
	try 
	{
        ...
        // 시간제 아이템의 경우 인벤토리에 공간이 없다면 주울수 없다.
        if ( pItem->getItemClass() == Item::ITEM_CLASS_EVENT_ITEM )
        {
            ItemType_t itemType = pItem->getItemType();
            if ( itemType == 30 )
            {
                _TPOINT tp;
                if (!pPC->getInventory()->getEmptySlot(pItem, tp))
                    goto ERROR;
            }
        }
...
//			pZone->broadcastPacket(ZoneX, ZoneY, &_GCDeleteObject, pPC);
			...
			// 2007 06 22 헬가든 키를 주울 경우 시간제가 붙는다
			if ( pItem->getItemClass() == Item::ITEM_CLASS_QUEST_ITEM && pItem->getItemType() >= 11 && pItem->getItemType() <= 15 )
			{
				unsigned long timeLimit = 60 * 60;
