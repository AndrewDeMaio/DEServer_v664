	 ...
	 throw (ProtocolException , Error)
{
	...
	try 
	{
		...
		if ( pGamePlayer->isPenaltyFlag(PENALTY_TYPE_MUTE) || pCreature->isFlag(Effect::EFFECT_CLASS_MUTE ) ) return;
		...
		else 
		{
			...
			try 
			{
    	        BEGIN_DB
        	    {
					...
					pResult = pStmt->executeQuery("SELECT PlayerID, Slot-'0' FROM Slayer WHERE Name='%s' LIMIT 2",pPacket->getName().c_str());
	
					if ( pResult->getRowCount() == 1 && pResult->next() )		
					{
						PlayerID				= pResult->getString( 1 );
						BYTE slot               = pResult->getInt( 2 );
						
						SAFE_DELETE(pStmt);

						// 찾은 PlayerID로 Player테이블에서 ServerGroupID와 LogOn정보를 찾는다.
						pStmt	= g_pDatabaseManager->getDistConnection("USERINFO")->createStatement();
						pResult	= pStmt->executeQuery("SELECT CurrentWorldID, CurrentServerGroupID, LogOn, LastSlot FROM Player WHERE PlayerID='%s'",PlayerID.c_str());
					
						// Player 정보를 찾았다.
						if ( pResult->next() )
						{
							WorldID_t CurrentWorldID = pResult->getInt( 1 );
							CurrentServerGroupID	 = pResult->getInt( 2 );
							LogOn					 = pResult->getString( 3 );
							BYTE lastSlot            = pResult->getInt( 4 );

							// 게임중인 상태일때 bServerFind에 true flag
							if ( g_pConfig->getPropertyInt( "WorldID" ) == CurrentWorldID && LogOn == "GAME" && lastSlot == slot )
							{
								bServerFind = true;
								SAFE_DELETE(pStmt);
							}
