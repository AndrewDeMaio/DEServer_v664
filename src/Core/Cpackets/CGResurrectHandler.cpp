	...
	if (pCreature->isFlag(Effect::EFFECT_CLASS_COMA))
	{
		...
		if (pEffectComa->canResurrect())
		{
			...
			*/
			...
			// 2007.02.08 부활 가능 이펙트를 삭제 하여준다.
			if( pCreature->isFlag( Effect::EFFECT_CLASS_REQUEST_RESURRECT ) )
			{
				EffectRequestResurrect* pEffectResurrect = (EffectRequestResurrect*)(pEffectManager->findEffect(Effect::EFFECT_CLASS_REQUEST_RESURRECT));
				Assert( pEffectResurrect != NULL );
