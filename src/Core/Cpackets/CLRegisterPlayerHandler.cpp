...
//		throw InvalidProtocolException("must be guest user");
	...
	try {
		...
		pResult = pStmt->executeQuery(
			// MySQL 8 ���� PASSWORD() �� ���ŵǾ���. CLLoginHandler �� ���ϰ� ��������
			// �ϹǷ� ���� ���� ���� ���� ǥ���� ����.
			"INSERT INTO Player (PlayerID , Password , Name , Sex , SSN , Telephone , Cellular , Zipcode , Address , Nation , Email , Homepage , Profile , Pub) VALUES ('%s' , CONCAT('*', UPPER(SHA1(UNHEX(SHA1('%s'))))) , '%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s' , %d , '%s' , '%s' , '%s' , '%s')" ,
			pPacket->getID().c_str() , 
