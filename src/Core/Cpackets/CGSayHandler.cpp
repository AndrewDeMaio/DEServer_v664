	...
	throw(ProtocolException, Error)
{
	...
	else if(set_type == "evnet_activate")
	{
		if(g_pVariableManager->getEventActivate() == 1)
			message << "이벤트 진행중" ; // 20070727 endl 뺐다.
		else
			message << "이벤트 정지중" ;
		gcSystemMessage.setMessage(message.toString());
	}
	...
	{
		message << "미클리즈 최대 인원 " << (g_pVariableManager->getMikllizzMaxPlayer()) << "명";
		gcSystemMessage.setMessage(message.toString());
	}
	/*
	else if (set_type == "NewAccountEvent")
	{
		message << "새로운 계정 아이템 지급 이벤트 : " << (g_pVariableManager->getNewAccountFirstGoal80Event()? "ON":"OFF");
		gcSystemMessage.setMessage(message.toString());
	}
	*/
	else if (set_type == "NewbieEvent")
	{
		message << "새로운 케릭터 아이템 지급 이벤트 : " << (g_pVariableManager->getNewbieItemEvent()? "ON":"OFF");
		gcSystemMessage.setMessage(message.toString());
	}
	// 20080425 세잎 클로버 이벤트
...
//		cout << guildID << endl;
	...
	else if( command == "startPVPWar")
	{
		if( !g_PVPSystemManager.hasWar() )
		{
			...
			gcSystemMessage.setMessage( "PVP 전쟁이 시작되었습니다." );
		}
		else
			gcSystemMessage.setMessage( "이미 PVP 전쟁이 진행 중입니다." );
	}
	else if( command == "removePVPWar")
	{
		if( g_PVPSystemManager.hasWar() )
		{
			...
			gcSystemMessage.setMessage( "PVP 전쟁이 끝났습니다." );
		}
		else
...
/*	else if ( command == "ReloadAuthTable" )
	...
	else if( command == "startEdgearch" )
	{
		...
		if( pZone->isSurvivalZone() && pZone->getZoneID() == 8001 && addTime != 0 && pZone->getEventSurvivalZoneManager()->getCurrentState() == 2 )
		{
			...
			char msg[256];
			sprintf(msg, "에지아크가 %d분후에 발동됩니다.", addTime);
			gcSystemMessage.setMessage( msg );
		}
		else if( pZone->isSurvivalZone() && pZone->getZoneID() == 8001 && pZone->getEventSurvivalZoneManager()->getCurrentState() == 2 )
		{
			...
			gcSystemMessage.setMessage( "에지아크 발동" );
		}
		else
		{
			gcSystemMessage.setMessage( "이벤트 지역이 아니거나 전투가 시작되지 않았습니다.");
		}
	}
	else if( command == "eraseEdgearch" )
	{
		...
		if( pZone->isSurvivalZone() && pZone->getZoneID() == 8001 )
		{
			...
			gcSystemMessage.setMessage( "에지아크 해제" );
		}
	}
	else if( command == "clearEffect" )
	{
		...
		gcSystemMessage.setMessage( "캐릭터 이펙트 타임아웃 시키기" );
	}
