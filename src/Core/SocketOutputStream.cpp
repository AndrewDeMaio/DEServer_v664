...
//#include "PacketLog.h" // 로그를 남기기 위한 헤더?
...
SocketOutputStream::SocketOutputStream ( Socket * sock , uint BufferLen ) 
	throw ( Error )
: m_Socket(sock), m_Buffer(NULL), m_BufferLen(BufferLen), m_Head(0), m_Tail(0), m_WriteCounter(0)
{
	__BEGIN_TRY
	 ...
	 throw ( ProtocolException , Error )
{
	...
	// 이제 패킷바디를 출력버퍼로 쓴다.
	// A packet must write exactly as many bytes as getPacketSize() promised.
	// When it doesn't, the client reads a bogus PacketID out of the middle of
	// this packet, throws out of processCommand(), and UpdateSocketInput()
	// drops it to MODE_MAINMENU -- a silent "force DC" with no crash and no
	// client-side error. That was the ModifyInfo `ulong` bug: 8 bytes here on
	// LP64 against szuint in getPacketSize(), 4 on the 32-bit client.
	//
	// This only checks THIS side's self-consistency. The other half of the
	// problem -- where the server is self-consistent but the client reads a
	// different number of bytes, because ContentsFilter compiles a field out
	// there (the GCCannotAdd bug) -- cannot be seen from here. The client
	// checks that itself in ClientPlayer::processCommand() and reports it
	// back as "*bug_report PacketSize ID)<id> <read>/<declared>".
	uint writtenBefore = m_WriteCounter;
	pPacket->write( *this );
	uint bodyBytes = m_WriteCounter - writtenBefore;

	if ( bodyBytes != (uint)packetSize )
	{
		filelog("PacketSizeMismatch.log", "ID %d declared %u but wrote %u : %s",
			(int)packetID, (uint)packetSize, bodyBytes, pPacket->toString().c_str());
		cout << "[PacketSizeMismatch] ID " << packetID << " declared " << packetSize
		     << " but wrote " << bodyBytes << endl;
	}

	//add by kim 패킷로그.
	cout<< "[SocketOuput]Send Packet:" << pPacket->toString() <<", PacketID:"<<packetID<< ",size:" << packetSize << endl;
	
//#ifdef __PACKET_LOG__
    filelog("Packet.log", "Send Packet ID : [%d]", packetID);
     ...
     throw (IOException, ProtocolException, InvalidProtocolException, Error)
{
	...
	try {
		...
		if ( m_Head < m_Tail ) {
			...
			while ( nLeft > 0 ) {
				nSent = 0;	// m_Socket->send() 에서 exception 발생시 nSent 값은 신뢰할수 없음 
	 ...
	 throw ( IOException , Error )
{
	...
	if ( m_Socket == NULL )
	{
		...
		filelog( "packetresizeerror.txt", "PacketID = %u", *(PacketID_t*)m_Buffer );
	}
	else
	{
