...
public:
	...
	uint getSize () const throw ()
	{
		return szObjectID
			...
			+ szAlignment
			+ szAttr* 3* 3
			+ szAttr* 3 // 20070907 슬레이어 망각의 수정땜시 추가
			+ szRank + szRankExp
			+ szExp* 3
			+ szHP* 2
			+ szMP* 2
...
public :
	...
	// Advanced Attributes 20070907 망각의 수정땜시 추가
	Attr_t getAdvancedSTR() const { return m_AdvancedSTR; }
	Attr_t getAdvancedDEX() const { return m_AdvancedDEX; }
	Attr_t getAdvancedINT() const { return m_AdvancedINT; }
	void setAdvancedSTR(Attr_t str)  { m_AdvancedSTR = str; }
	void setAdvancedDEX(Attr_t dex)  { m_AdvancedDEX = dex; }
	void setAdvancedINT(Attr_t inte)  { m_AdvancedINT = inte; }
	//20090601 ksym555
