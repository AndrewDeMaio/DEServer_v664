...
//	addFactory(new CLQueryPlayerIDFactory());	// 2008.10.23 사용하지 않는 패킷이므로 등록하지 않음
	addFactory(new CLQueryCharacterNameFactory());
//	addFactory(new CLRegisterPlayerFactory);	// 2008.10.23 사용하지 않는 패킷이므로 등록하지 않음
...
//	addFactory( new GCModifyMoneyFactory() );
	...
	addFactory(new CGRequestRepairInfoFactory());
	addFactory(new GCRepairInfoFactory());
	
	addFactory(new GSLoginFactory());
	addFactory(new GSLogoutFactory());
	...
	addFactory(new SGFriendListFactory());
	addFactory(new GSFriendReqToAddFactory());
	
	addFactory(new SGFriendReqToAddFactory());
	addFactory(new SGFriendSuggestToAddFactory());
	
	addFactory(new GSFriendResToAddFactory());
	addFactory(new SGFriendAddFactory());
	
	addFactory(new GSFriendReqToDelFactory());
	addFactory(new SGFriendDelFactory());
	
	addFactory(new GSFriendViewInfoFactory());
	addFactory(new SGFriendViewInfoFactory());
	
	addFactory(new CGFriendUpdateGlobalOptionFactory());
	addFactory(new CGFriendUpdateIndividualOptionFactory());
	
	addFactory(new SGFriendUpdateConnectFactory());
	
	addFactory(new GSFriendUpdateZoneFactory());
	addFactory(new SGFriendUpdateZoneFactory());
	addFactory(new SGFriendWaitListFactory());
	
	addFactory(new GSFriendUpdateGlobalOptionFactory());
	addFactory(new GSFriendUpdateIndividualOptionFactory());
	
	addFactory(new SGFriendResToAddFactory());
	 ...
	 throw (Error)
{
	...
	if("CG" == pFactory->getPacketName().substr(0,2))
	{
		    m_CgPacket[pFactory->getPacketID()] = pFactory->getPacketName();
	}
...
bool PacketFactoryManager::isCGPacket(PacketID_t packetID)
{ 
	if(m_CgPacket.find(packetID) != m_CgPacket.end()) 
	{
		return true; 
	}
