...
//		resource.getVersion() , 
	...
	// sendfile() needs a raw descriptor; filebuf::fd() was an old libstdc++
	// extension and no longer exists.
	...
	FileSize_t filesize = ( fstat( fd , &filestat ) == 0 ? filestat.st_size : 0 );
	...
	// sendfile() reads from *offset, so there is nothing to rewind.
	off_t offset = 0;
	...
	// 파일 내용을 전송한다.
	DWORD nSent = sendfile( pSocket->getSOCKET() , fd , &offset, filesize );
