	...
	throw ()
{
	...
	// write() puts a one-byte type map ('S'/'V'/'O'/'0') for EVERY slot ahead
	// of the PCInfo bodies, so those SLOT_MAX bytes belong in the declared
	// size too. Leaving them out made this packet announce 3 bytes fewer than
	// it actually wrote (e.g. 120 vs 123), desyncing anything that trusts the
	// header. getPacketMaxSize() always counted them; only this was missing.
	packetSize += SLOT_MAX;
