	...
	throw ( Error )
{
	...
	if ( m_GameServerGroupInfos == NULL )
	{
		return;
	}
	...
	throw ( Error )
{
	...
	try {
		...
		while ( pResult->next() ) {
			...
			pGameServerGroupInfo->setNonPKServer( false );
			addGameServerGroupInfo( pGameServerGroupInfo, WorldID );
		}

		pResult = pStmt->executeQuery( "SELECT WorldID, ServerGroupID FROM NonPKServerList" );
		
		while ( pResult->next() )
		{
			WorldID_t worldID = pResult->getInt(1);
			ServerGroupID_t serveGroupID = pResult->getInt(2);

			getGameServerGroupInfo( serveGroupID, worldID )->setNonPKServer( true );
		}
