...
//	static Timeval currentTime;
	...
	try 
	{
		...
		while ( true ) 
		{
			...
			if (!m_pInputStream->peek( (char *)&header , sizeof(sPacketHeader) ))
			{
				// �Է��� �ƹ� �͵� �����ٸ�, �Է����� �ð��� �ʰ��ߴ��� üũ�Ѵ�.
				Timeval currentTime;
				getCurrentTime(currentTime);
			...
			// ��Ʈ�� ���߽� ��ġ�� ã�� ���� ���(wire) ���.
			{
				StringStream wireMsg;
				wireMsg << "WIRE : packetID " << (int)packetID
					<< " packetSize " << (int)packetSize
					<< " buffered " << (int)m_pInputStream->length() << eos;

				uint wireLen = m_pInputStream->length();
				if ( wireLen > 48 ) wireLen = 48;
				char wireBuf[48];
				if ( wireLen > 0 && m_pInputStream->peek( wireBuf , wireLen ) )
				{
					static const char wireHex[] = "0123456789ABCDEF";
					string wireLine;
					string wireAsc;
					for ( uint i = 0 ; i < wireLen ; i ++ )
					{
						uchar b = (uchar)wireBuf[i];
						wireLine += wireHex[ b >> 4 ];
						wireLine += wireHex[ b & 0x0F ];
						wireLine += ' ';
						wireAsc += ( b >= 32 && b < 127 ) ? (char)b : '.';
					}
					wireMsg << "  hex : " << wireLine << eos;
					wireMsg << "  asc : " << wireAsc << eos;
				}
				cout << wireMsg.toString() << flush;
			}
			...
			*/
			...
			try 
			{
				...
				if ( ! g_pPacketValidator->isValidPacketID( getPlayerStatus() , packetID ) )
				{
					...
					orderMsg << "invalid packet order : packetID " << (int)packetID
						<< " size " << (int)packetSize
						<< " playerStatus " << (int)getPlayerStatus()
						<< " buffered " << (int)m_pInputStream->length() << eos;

					// ����� peek �� �����̹Ƿ� ���۴� �����ϴ�.
					// ��� 6����Ʈ���� �״�� ���ϸ� ��Ʈ���� ��� �������
					// �˾ƺ� �� �ִ�.
					uint dumpLen = m_pInputStream->length();
					if ( dumpLen > 64 ) dumpLen = 64;
					char dumpBuf[64];
					if ( dumpLen > 0 && m_pInputStream->peek( dumpBuf , dumpLen ) )
					{
						static const char hexDigit[] = "0123456789ABCDEF";
						string hexLine;
						string ascLine;
						for ( uint i = 0 ; i < dumpLen ; i ++ )
						{
							uchar b = (uchar)dumpBuf[i];
							hexLine += hexDigit[ b >> 4 ];
							hexLine += hexDigit[ b & 0x0F ];
							hexLine += ' ';
							ascLine += ( b >= 32 && b < 127 ) ? (char)b : '.';
						}
						orderMsg << "  hex : " << hexLine << eos;
						orderMsg << "  asc : " << ascLine << eos;
					}
					throw InvalidProtocolException( orderMsg.toString() );
				}
				...
				// readTimeStamp() ���� -- Ŭ���̾�Ʈ�� Ÿ�ӽ������� ���� ����,
				// �� �ڵ� ��𿡵� writeTimeStamp() �� ����. 4����Ʈ�� �� ��� ��
				// ��Ʈ���� �о ��Ŷ ��� ��ġ�� �ױ� ���Ѵ�.
			...
			catch ( IgnorePacketException& ) 
			{
                ...
                m_pInputStream->skip( sizeof(sPacketHeader) + packetSize );
	...
	throw ()
{
	...
	lcLoginOK.setUnderFifthteen( isUnderFifthteen() );
		...
		{	
			...
			else if ( race == "VAMPIRE" ) 
			{
				...
				pResult2 = pStmt2->executeQuery(
					"SELECT Name, Slot, Sex, BatColor, AdvBatColor, SkinColor, AdvancementClass, STR, DEX, INTE, HP, CurrentHP, `Rank`, GoalExp, Level, Bonus, Fame, Alignment, Shape, CoatColor FROM Vampire WHERE PlayerID = '%s' AND Active = 'ACTIVE' AND Name='%s'",
				...
				pPCVampireInfo->setAdvanceBatColor(pResult2->getInt(++i)); // 2007.07.24 ���� ���� ����
				...
				pPCVampireInfo->setArmType( VAMPIRE_ARM_NONE );
				pPCVampireInfo->setCoatType( getVampireCoatType(shape) );
