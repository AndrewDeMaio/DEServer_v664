...
const int DELAY_PAY_TIME_UPDATE = 600;	// 10분마다 - 테스트용
...
//	m_PayIPType = PAY_IP_TYPE_ALL;        		// 게임방인 경우의 IP type
	...
	m_bPCRoomPlay = false;
	m_bPayZoneTicket = false;
	m_PayPlayRoomType = PAY_PLAY_ROOM_TYPE_HOME;
...
{
//	if (m_PayType!=PAY_TYPE_FREE && m_PayType!=PAY_TYPE_POST && m_PayType!=PAY_TYPE_HANARO_POST)
	if (m_PayType!=PAY_TYPE_FREE && m_PayType!=PAY_TYPE_ZONE_TICKET && m_PayType!=PAY_TYPE_HANARO_POST) // 20080227 후불제가 유료존 티켓으로 바뀌었다.
	{
	...
	else if (m_PayType == PAY_TYPE_ZONE_TICKET)
	{
		VSDateTime currentDateTime(VSDate::currentDate(), VSTime::currentTime());

		if (currentDateTime <= m_PayPlayAvailableDateTime)
		{
			m_bPayZoneTicket = true;
			return true;
		}
		else
			return false;
	...
	throw (ProtocolException, Error)
{
	...
	switch (m_PayType)
	{
		...
		//-----------------------------------------
		// 20080227 유료존 티켓 사용자인 경우
		//-----------------------------------------
		case PAY_TYPE_ZONE_TICKET :
		{
			// 한 시간 정도에 한 번씩 체크한다.
			// 기간제 사용자인 경우는 1시간 정도 서비스되어도 문제 없다고 본다
			if (currentTime.tv_sec >= m_PayPlayStartTime.tv_sec + DELAY_PAY_TIME_UPDATE)
		...
		case PAY_TYPE_TIME :
		{
			...
			( m_PayPlayStartTime.tv_sec != 0 &&
				(
					( currentTime.tv_sec >= m_PayPlayStartTime.tv_sec + DELAY_PAY_TIME_UPDATE )	// 1시간이 지났을때
					||
					( ( m_PayPlayAvailableHours < MINUTE_PAY_TIME_DECREASE ) && ( elapsedSec > remainSec ) ) // 남은 사용시간이 1시간미만이고 사용시간이 남은 시간보다 클때
					||
					( ( m_PayPlayType==PAY_PLAY_TYPE_PCROOM ) && ( m_PayPlayAvailableHours < MINUTE_PAY_TIME_DECREASE*UserMax ) &&	// 피시방이고 남은 사용시간이 유져수*1시간 보다 작고
							( ( elapsedSec * ( min ( 12, max ( 1, ( int ) m_UserMax ) ) ) > remainSec ) || ( elapsedSec>=60 && m_PayPlayAvailableHours <= UserMax )	)
					)
				)
			)
			{
				...
				if ( ( m_PayPlayAvailableHours <= 0 ) && ( getPayPlayType() != PAY_PLAY_TYPE_PCROOM ) )
				{
					//throw ProtocolException("사용시간이 다됐다.");
					m_PayPlayStartTime.tv_sec = 0;
					m_bPremiumPlay = false;
...
//			}
			...
			if (pResult->next())
			{
				...
				setPayPlayType( PAY_PLAY_TYPE_PCROOM );
				bool bAvailable = checkPCRoomPayPlayAvailable();//false;//checkPayPlayAvailable();
				...
				if (bAvailable)
				{
					...
					if (pResult->next())
					{
							...
							|| m_PayType==PAY_TYPE_TIME && users >= m_UserMax)
						{
							...
							//cout << "loginPlayInPayPCRoom() - Free PCRoom" << endl;
							m_PayPlayRoomType = PAY_PLAY_ROOM_TYPE_FREE_PCROOM;
						...
						else
						{
							...
							//cout << "loginPlayInPayPCRoom() - Charge PCRoom" << endl;
							m_PayPlayRoomType = PAY_PLAY_ROOM_TYPE_CHARGE_PCROOM;
			...
			{
				//cout << "loginPlayInPayPCRoom() - Non-PCRoom" << endl;
				m_PayPlayRoomType = PAY_PLAY_ROOM_TYPE_HOME;
	...
	throw (Error)
{
	...
	if (m_PayPlayType==PAY_PLAY_TYPE_PCROOM)
	{
		BEGIN_DB
		{
			...
			//cout << "logoutPlayInPayPCRoom()" << endl;
...
//	}
	...
	// 개인 사용자 요금체계에 대한 체크
	//--------------------------------------------------
	if (payType!=PAY_TYPE_FREE && payType!=PAY_TYPE_HANARO_POST)
	{
		...
		if ( !checkNewPayPlayAvailable() || m_PayType == PAY_TYPE_TIME )
		{
			// 게임방 먼저 체크
		...
		{
//20090209 wlzzi - PC방 혜택 적용 오류수정
	// loginPayPlayPCRoom 함수는 무료PC방일 때 FALSE를 유료PC방일 때 TRUE를 return하기 때문에,
	//	유료사용자가 무료PC방에 접속했을 때 문제가 발생했었음 (09.01 초)
	//		> loginPayPlayPCRoom 함수 내에서~ PayPlayAvailableDate 값이 PC방의 것으로 대입된 이후, return 값에 의해 원상복구 되지 못함.
	//			- 이후, 1시간 주기의 updatePayPlayTime 함수내에서 무료상태로  변경되는 문제가 발생했었음.
	//		> 그에 대한 수정으로, loginPayPlayPCRoom의 return 값이 FALSE일 때, 즉 무료PC방일 때 Player의 Pay관련 값을 원상복구하도록 수정함
	//	위 문제수정 결과, 유료사용자가 유료PC방에 접속했을 때 문제가 발생함 (09.01 말)
	//		> loginPayPlayPCRoom 함수 내에서~ PayType과 PayPlayType이 PC방(유료)기준으로 변경된 것이, 원상복구 되지 않아 발생
	//			- 유료사용자는 유료PC방의 종량제 시간을 사용하지 말아야 하는데... 사용되는 문제...
	//	위 두 가지 문제를 모두 해결할 수 있는 수정방안이 필요함.
	//		-- 현 시점에서의 주요 이슈는... 유료사용자의 PC방 혜택 적용을 위한 .. 정보설정 -- PayPlayRoomType
	//			> loginPayPlayPCRoom 함수의 결과가 T이거나 F이거나 접속한 유저개인의 정보로 재 설정해 준다. ^^
	//		  ! 2009년 2월 현재, 개인 종량제는 판매되지 않으며 PC방은 모두 종량제다. 만일, 개인 종량제가 부활되고 정액제 PC방이 적용될 경우
	//			- 관련하여, 전반적인 구조개선이 필요하다 판단된다. 그래서.. 이번엔 구조개선 없이.. 기존구조를 유지한 채로~ 버그만 잡는다.
			
			bool bChargePCRoom = loginPayPlayPCRoom (ip, playerID); //loginPayPlayPCRoom함수 에서 PayPlayRoomType을 설정하므로, 그것으로 유료PC방인지를 확인할 수도 있다.
			
			if (bChargePCRoom)	//유료PC방이라면, loginPayPlayPCRoom 함수에서 해당PC방의 접속User로 등록된 DB정보를 제거...-_-;; 
	...
	// PayType을 설정한다.
	checkNewPayPlayAvailable();

	if(m_PayType != PAY_TYPE_ZONE_TICKET) // 20080227 유료존 입장권일 경우 프리미엄 서비스는 적용안된다.  
		m_bPremiumPlay = true;

	cout << "PlayerID = " << playerID.c_str() << ", "
		<< "PremiumPlay : " << isPremiumPlay() << " " 
		<< "PCRoomPayType : " << getPayPlayRoomType() << endl;
...
//	}
	...
	// 개인 사용자 요금체계에 대한 체크
	//--------------------------------------------------
	if (m_PayType!=PAY_TYPE_FREE && m_PayType!=PAY_TYPE_HANARO_POST)
	{
		// 개인 정액제 사용자인지 먼저 체크한다.
		setPayPlayType( PAY_PLAY_TYPE_PERSON );
		if (!checkPersonPayPlayAvailable() || m_PayType != PAY_TYPE_PERIOD)
		{
			// loginPayPlayPCRoom에서 값이 바뀌므로.. -_-;
		...
		// 개인 정액 사용자도 PC 방 보너스를 적용시켜주기 위해서
		else if ( m_PayType == PAY_TYPE_PERIOD || m_PayType == PAY_TYPE_ZONE_TICKET)
		{
...
//		else if (bDecreaseTime && m_PayType==PAY_TYPE_POST) 20080227 후불제 관련 코드 삭제
//		{
//			int usedMin = (currentTime.tv_sec-m_PayPlayStartTime.tv_sec)/60;
//
//			if (usedMin > 0)
	...
	throw (Error)
{
	...
	BEGIN_DB
	{
		...
		pStmt->executeQuery( 
				"UPDATE Player SET PayPlayHours=IF(PayPlayHours-%d>0,PayPlayHours-%d,0) WHERE PlayerID='%s'",
							mm, mm, playerID.c_str());

		// AffectedRowCount == 0 의 경우는
		// PayPlayHours == 0 으로 강제로 바꾼 경우. ex) 하나로 종량제 취소/결제실패
		if ( mm != 0 && pStmt->getAffectedRowCount() == 0 )
		{
			m_PayPlayAvailableHours = 0;
		}
...
//			}
			...
			if ( bAvailable )
			{
				cout << "isPlayInPayPCRoom() - Charge PCRoom" << endl;
				m_PayPlayRoomType = PAY_PLAY_ROOM_TYPE_CHARGE_PCROOM;
			}
			else
			{
				cout << "isPlayInPayPCRoom() - Free PCRoom" << endl;
				m_PayPlayRoomType = PAY_PLAY_ROOM_TYPE_FREE_PCROOM;
			}
		...
		{
			cout << "isPlayInPayPCRoom() - Non-PCRoom" << endl;
			m_PayPlayRoomType = PAY_PLAY_ROOM_TYPE_HOME;
...
bool PaySystem::isPCRoomPlay() const
{
	//return m_bPCRoomPlay; 

	// 기존 코드와의 호환성을 위해 남겨 놓는다. 유료 PC방 유무를 돌려줌
	if ( m_PayPlayRoomType == PAY_PLAY_ROOM_TYPE_CHARGE_PCROOM )
	{
		return true;
	}
	else
...
PayPlayRoomType PaySystem::getPayPlayRoomType()
{
	return m_PayPlayRoomType;
}

void PaySystem::setPayPlayRoomType(PayPlayRoomType type)
{
	m_PayPlayRoomType = type;
}

bool PaySystem::CheckPayFunction(const string& ip, const string& playerID)
throw (Error)
{
	VSDateTime currentDateTime( VSDate::currentDate(), VSTime::currentTime() );
	Timeval currentTime;
	...
	BEGIN_DB
	{
		...
		{
			cout << "loginPlayInPayPCRoom() - Non-PCRoom" << endl;
			setPayPlayRoomType(PAY_PLAY_ROOM_TYPE_HOME);
			m_bPCRoomPlay = false;
		}
	}
	catch(SQLQueryException& sqe)
	...
	// 피시방 종류 체크
	int checkopt = checkPCRoomPayPlayAvailable();
	// 피시방에서 접속 여부와  현재 PC방의 동시 접속자 수 최대치를 체크 하여 피시방 여부 체크
	// 이부분 관련 해서 동시 접속자 초과시에 어떻게 될지 확인 필요.
	bool bPayPCRoom = ( ( (checkopt == 1) || (checkopt == 2) ) && CheckPCRoomUserCount(ip, playerID));
	...
	// 위에서 PayType 정보가 바뀌었으므로 피시방인 경우 다시 셋팅
	if( bPayPCRoom )
	{
		setPayType(PAY_TYPE_TIME);
		setPayPlayType(PAY_PLAY_TYPE_PCROOM);
		getCurrentTime(m_PayPlayStartTime);
	}
	else
	{
		setPayType(PAY_TYPE_PERIOD);
		setPayPlayType(PAY_PLAY_TYPE_PERSON);
	}

	// 개인 정액 체크
	bool bPayPerson = checkPersonPayPlayAvailable();

	if( bPayPCRoom && !bPayPerson )
	{
		if ( checkopt == 1 )
		{
			cout <<"===== only 유료 피시방  ====="<<endl;
		}
		else if( checkopt == 2 )
		{
			cout <<"===== only 무료 피시방  ====="<<endl;
		}

		return true;
	}
	else if( bPayPCRoom && bPayPerson )
	{
		if ( checkopt == 1 )
		{
			cout <<"===== 유료 피시방 유료 사용자 ====="<<endl;
		}
		else if( checkopt == 2 )
		{
			cout <<"===== 무료 피시방 유료 사용자 ====="<<endl;
		}
		return true;
	}
	else if ( !bPayPCRoom && bPayPerson )
	...
	{
		cout<< "피시방 아니고 개인 사용자도 아니다" <<endl;
	}
	cout <<"CheckPayFunction false"<<endl;
	return false;
}

int PaySystem::checkPCRoomPayPlayAvailable()
{
	// 여기서의 m_PayType 은  PCRoomInfo 의 PayType 이다 . 그래서  거의 1이다.
	if (m_PayType!=PAY_TYPE_FREE && m_PayType!=PAY_TYPE_ZONE_TICKET && m_PayType!=PAY_TYPE_HANARO_POST) // 20080227 후불제가 유료존 티켓으로 바뀌었다.
	{
		VSDateTime currentDateTime(VSDate::currentDate(), VSTime::currentTime());
		...
		if( getPCRoomID() > 0 )
		{
			...
			{
				//cout<<"피시방 요금제"<<endl;
				setPayPlayRoomType(PAY_PLAY_ROOM_TYPE_CHARGE_PCROOM);
				m_bPCRoomPlay = true;	// 피시방 경험치 증가와 기타등등을 위해
				m_bPremiumPlay = true;	// 피시방일 경우, 유무료에 상관 없이 프리미엄 혜택이 적용된다.
				return 1;
			}
			else
			{
				//cout<<"피시방 시간이 종료"<<endl;
				setPayPlayRoomType(PAY_PLAY_ROOM_TYPE_FREE_PCROOM);
				m_bPCRoomPlay = false;	// 피시방 경험치 증가와 기타등등을 위해
				m_bPremiumPlay = true;	// 피시방일 경우, 유무료에 상관 없이 프리미엄 혜택이 적용된다.
				return 2;
			}
		}
...
int PaySystem::checkPersonPayPlayAvailable()
{
	// m_PayType은 Player 테이블에 PayType 이다. 0은 내부 테스트 계정이나 특수한 때 이고, 1은 일반 유져 계정일때다.
	if (m_PayType!=PAY_TYPE_FREE && m_PayType!=PAY_TYPE_ZONE_TICKET && m_PayType!=PAY_TYPE_HANARO_POST) // 20080227 후불제가 유료존 티켓으로 바뀌었다.
	{
		VSDateTime currentDateTime(VSDate::currentDate(), VSTime::currentTime());
		...
		if( currentDateTime <= m_PayPlayAvailableDateTime )	//남은 사용 일이 있는지
		{
			//cout<<"개인 유료"<<endl;
			m_bPremiumPlay = true;
			return 1;
		...
		{
			//cout<<"개인 사용 종료"<<endl;
			if( getPayPlayType() != PAY_PLAY_TYPE_PCROOM )
			{
				m_bPremiumPlay = false;	// 피시방일때는 무조건 프리미엄 서비스를 줘야한다.
			}
			return 0;
		}
	}
...
bool PaySystem::CheckPCRoomUserCount(const string& ip, const string& playerID)
{
	// 한게임방에 접속 가능한 최대치 체크
	__BEGIN_TRY
	Statement* pStmt = NULL;
	...
	BEGIN_DB
	{
		...
		{
			pResult = pStmt->executeQuery(
						"SELECT count(*) from PCRoomUserInfo WHERE ID=%d", getPCRoomID() );
		}
		catch (Throwable& t)
		{
			filelog("paySystem.txt", "%s", t.toString().c_str());
		...
		if (pResult->next())
		{
			...
			if ( ( m_PayType==PAY_TYPE_PERIOD && users >= m_UserLimit )
				|| ( m_PayType==PAY_TYPE_TIME && users >= m_UserMax ) )
			{
				//cout << "[PayPCRoom] User Limit Exceed! PayType = "<< m_PayType<<": users = "<< users << ": UserMax = "<< m_UserMax << " : m_UserLimit = " << m_UserLimit << endl;
				return false;
			}
			else
			{
				...
				{
					users = pResult->getInt(1);
					// 추가 하는 사이 다른 사람이 들어와서 접속 최대치 오버했는지
					if (m_PayType==PAY_TYPE_PERIOD && users >= m_UserLimit
						|| m_PayType==PAY_TYPE_TIME && users >= m_UserMax)
					{
						// 접속 최대치를 오버 했으면 지운다.
						pStmt->executeQuery(
							"DELETE FROM PCRoomUserInfo WHERE PlayerID='%s'",
								playerID.c_str());
						//cout << "[PayPCRoom] User Limit Exceed and Delete" << endl;
					...
					else
					{
						// 정상적인 경우
						SAFE_DELETE(pStmt);
						//cout << "최대 접속자 수는 통과" << endl;
						return true;
					}
				}
				else
				{
					//cout << "PCRoomUserInfo 에 접속자 정보가 하나도 없다" << endl;
				}
			}
			SAFE_DELETE(pStmt);
			return false;
