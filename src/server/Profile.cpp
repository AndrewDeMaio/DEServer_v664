...
void ProfileSampleSet::beginProfile(const string& name)
{
	...
	else
	{
		...
		// 그러므로 에러다.
		// Out of profile sample slots. Diagnostic only -- do not abort.
	}
}
...
void ProfileSampleSet::endProfile(const string& name)
{
	...
	if (itr == m_NameMap.end())
	{
		// No matching beginProfile -- happens when a profiled scope is left via an
		// exception, or when beginProfile hit the MAX_PROFILE_SAMPLES ceiling and
		// never registered the name. The original code asserted AND then fell
		// through to dereference this end() iterator. Profiling is a diagnostic;
...
{
	return;	//by kim 서버팅김현상 수정. 리턴값을 넣음으로서 사용안함. by kim 2021-08-13
