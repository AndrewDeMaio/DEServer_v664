	...
	throw()
{
	...
	BEGIN_DB
	{
		...
		{
			Guild::setMaxGuildID( g_pConfig->getPropertyInt("Dimension") * 30000 + g_pConfig->getPropertyInt("WorldID") * 10000 + 3100 );
		}
		else
		{
			// get & set MaxGuildID
...
GuildID_t GuildManager::getAvailableGuildID() const
	throw()
{
	__BEGIN_TRY

	// GuildID 허용 범위
	static GuildID_t maxGuildID = g_pConfig->getPropertyInt("Dimension") * 30000 + g_pConfig->getPropertyInt("WorldID") * 10000 + 3100;
	static GuildID_t minGuildID = g_pConfig->getPropertyInt("Dimension") * 30000 + ( g_pConfig->getPropertyInt("WorldID") - 1 ) * 10000 + 3100;

	hash_map<GuildID_t, Guild*>::const_iterator endItr = m_Guilds.end();

	// 순찾적으로 비어있는 GuildID 를 찾는다.
	for ( GuildID_t guildID = minGuildID; guildID < maxGuildID; ++guildID )
	{
		if ( m_Guilds.find( guildID ) == endItr )
		{
			return guildID;
		}
	}
