...
#ifdef __GAME_SERVER__
int Guild::getActiveMemberCountFromDB() const
	throw()
{
	__BEGIN_TRY
	...
	{
		pStmt	= g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
		pResult	= pStmt->executeQuery( "SELECT COUNT(*) FROM GuildMember WHERE GuildID = %d AND `Rank` IN (0,1,2)", m_ID );

		if ( pResult->next() )
		{
			num = pResult->getInt(1);

			cout << "GuildID : " << (int)m_ID << " Num: " << num << endl;
		}
...
Level_t Guild::getAverageLevelOfMember() const
	throw()
{
	__BEGIN_TRY
	...
	char sql[256];
	if ( m_Race == GUILD_RACE_SLAYER )
	{
		sprintf( sql, "SELECT GREATEST(S.BladeLevel, S.SwordLevel, S.GunLevel, S.EnchantLevel, S.HealLevel) FROM Slayer S, GuildMember GM WHERE GM.GuildID = %d AND GM.`Rank` in (0,1,2) AND GM.Name = S.Name", m_ID );
	}
	else if ( m_Race == GUILD_RACE_VAMPIRE )
	{
		sprintf( sql, "SELECT V.Level FROM Vampire V, GuildMember GM WHERE GM.GuildID = %d AND GM.`Rank` in (0,1,2) AND GM.Name = V.Name", m_ID );
	}
	else if ( m_Race == GUILD_RACE_OUSTERS )
	{
		sprintf( sql, "SELECT O.Level FROM Ousters O, GuildMember GM WHERE GM.GuildID = %d AND GM.`Rank` in (0,1,2) AND GM.Name = O.Name", m_ID );
	}
	else
	{
		return (Level_t)aveLevel;
	}
