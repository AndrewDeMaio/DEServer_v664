...
enum PayPlayRoomType
{
	PAY_PLAY_ROOM_TYPE_HOME,			// 등록되지 않은 PC방(또는 집)
	PAY_PLAY_ROOM_TYPE_FREE_PCROOM,		// 무료 가맹 PC방
	PAY_PLAY_ROOM_TYPE_CHARGE_PCROOM,	// 유료 가맹 PC방

	PAY_PLAY_ROOM_TYPE_MAX
};

enum PayType
{
	PAY_TYPE_FREE,      // 테스트용 유료 사용자 !!일반사용자는 이 타입이 나올 수 없어야 한다.
	PAY_TYPE_PERIOD,    // 월 사용자
	PAY_TYPE_TIME,      // 시간제 사용자
//	PAY_TYPE_POST,		// 후불 시간제 사용자
	PAY_TYPE_ZONE_TICKET,		// 20080227 후불제에서 유료존 입장권으로 바꾼다.
	PAY_TYPE_HANARO_POST,	// 하나로 후불제 사용자
    PAY_TYPE_MAX
};
...
public:
	...
	bool			isPayZoneTicket() const { return m_bPayZoneTicket; } // 20080227 유료존 입장권 사용자인지
...
public :
	...
	int				checkPCRoomPayPlayAvailable();
	int				checkPersonPayPlayAvailable();
	bool			CheckPCRoomUserCount(const string& ip, const string& playerID);
	// pay시작
		bool 			CheckPayFunction(const string& ip, const string& playerID)
												throw (Error);
...
//	void			setPCRoomPlay(bool bPCRoom = true) 		{ m_bPCRoomPlay = bPCRoom; }
	...
	PayPlayRoomType getPayPlayRoomType();
	void setPayPlayRoomType(PayPlayRoomType type);
...
//	PayIPType   	m_PayIPType;        		// 게임방인 경우의 IP type
	...
	bool			m_bPayZoneTicket;			// 20080227 유료존티켓
	bool			m_bPremiumPlay;
	bool			m_bPCRoomPlay;				// PC 방 사용자인가?
												// PC 방 유저 제한에 상관없이 유료 적용 중인 PC 방에서 플레이하고 있는가?
	PayPlayRoomType m_PayPlayRoomType;			// 어떤 공간에서 게임을 하는가 ?(집/무료PC방/유료PC방등)
