...
bool OptionInfo::isUpgradeThirdSucceed() const
{
	return (rand()%100 < (int)m_UpgradeThirdRatio);
}
	...
	throw()
{
	...
	msg << "OptionInfo ("
		...
		<< "UpgradeThirdRatio:"   << (int)m_UpgradeThirdRatio
	...
	throw ()
{
	...
	BEGIN_DB
	{
		...
		sql << "SELECT OptionType, Name, HName, Nickname, "
			...
			<< "PreviousOptionType, UpgradeOptionType, UpgradeRatio, UpgradeSecondRatio, UpgradeThirdRatio, "
		...
		while (pResult->next())
		{
			...
			pInfo->setUpgradeThirdRatio(pResult->getInt(++i));
		...
		pResult = pStmt->executeQuery("SELECT OptionClassType, Name, HName, Level, TotalGrade, OptionGroup, IndexName FROM OptionClassInfo");
		...
		while ( pResult->next() )
		{
			...
			pInfo->setIndexName( pResult->getString(7) );
		...
		// 20070726
		pResult = pStmt->executeQuery("SELECT OptionType, Ratio FROM PetEnchantOptionRatioInfo2");
		...
		while ( pResult->next() )
		{
			...
			ratio = ratio + getTotalPetEnchantOption2();
			setTotalPetEnchantOption2( ratio );

			PetEnchantOption* pPetEnchantOption = new PetEnchantOption();
			pPetEnchantOption->setOptionType( optionType );
			pPetEnchantOption->setRatio( ratio );

			addPetEnchantOption2( pPetEnchantOption );
		}

		// 20080218 by Kim Sung Yong
		pResult = pStmt->executeQuery("SELECT OptionType, Ratio FROM CrystalEnchantInfo");
		...
		while ( pResult->next() )
		{
			...
			ratio = ratio + getTotalCrystalEnchantOption();
			setTotalCrystalEnchantOption( ratio );

			CrystalEnchantOption* pCrystalEnchantOption = new CrystalEnchantOption();
			pCrystalEnchantOption->setOptionType( optionType );
			pCrystalEnchantOption->setRatio( ratio );

			addCrystalEncantOption( pCrystalEnchantOption );
		}
...
// 20070726
void OptionInfoManager::addPetEnchantOption2(PetEnchantOption* pPetEnchantOption)
	throw()
{
	__BEGIN_TRY

	Assert( pPetEnchantOption != NULL);
	m_PetEnchantOptionList2.push_back(pPetEnchantOption);

	__END_CATCH
}

// 20080218 by Kim Sung Yong
void OptionInfoManager::addCrystalEncantOption(CrystalEnchantOption* pClystalEnchantOption)
	throw()
{
	__BEGIN_TRY

	Assert( pClystalEnchantOption != NULL);
	m_CrystalEnchantList.push_back(pClystalEnchantOption);
