...
GQuestStatus::~GQuestStatus()
{
	...
	{
		MissionInfo *pMissionInfo = (*itr);

		//SAFE_DELETE((*itr));
		SAFE_DELETE(pMissionInfo);
	}
			...
			case GQuestElement::OK:
				{
					...
					// 2007 03 09 BAT Color Init 이거 또 없애 달라네... 2007 04 20
/*					else if( m_QuestID == 2012 && m_pOwner->isVampire() )
					{
						cout << "BatColor 초기화" << endl;

						Vampire* pVampire = dynamic_cast<Vampire*>(m_pOwner);
					...
					{
						EffectQuestSuccess *pEffect = new EffectQuestSuccess(m_pOwner);
						pEffect->setDeadline(30);	// 3 sec

						m_pOwner->addEffect( pEffect );

						GCAddEffect gcAddEffect;
						gcAddEffect.setObjectID( m_pOwner->getObjectID() );
						gcAddEffect.setEffectID( pEffect->getSendEffectClass() );
						gcAddEffect.setDuration( 30 ); // 3 sec

						m_pOwner->getPlayer()->sendPacket( &gcAddEffect );
					}
