	...
	throw (Error)
{
	...
	if (m_NextHeartbeat < currentTime) 
	{
		...
		for (int i = 1; i < GroupCount; i++) 
		{
			...
			m_numPC += pZoneGroup->getZonePlayerManager()->size();
		}
		...
		static int portNum2 = g_pConfig->getPropertyInt("LoginServerUDPPortNum2");
		static const string& loginServerIP2 = g_pConfig->getProperty("LoginServerIP2");
		static int loginServerUDPPort2 = g_pConfig->getPropertyInt("LoginServerUDPPort2");
		static int loginServerBaseUDPPort2 = g_pConfig->getPropertyInt("LoginServerBaseUDPPort2");

		g_pLoginServerManager->sendPacket( loginServerIP2, loginServerUDPPort2, &gmServerInfo);

		if (portNum2 > 1)
		{
			for ( int j = 0 ; j < portNum2 ; j++ )
			{
				g_pLoginServerManager->sendPacket(loginServerIP2, loginServerBaseUDPPort2+j, &gmServerInfo);
			}
		}
