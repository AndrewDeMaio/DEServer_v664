...
//					pResZoneInfo = g_pZoneInfoManager->getZoneInfo( pPC->getResurrectZoneID() );
					pResZoneInfo = g_pZoneInfoManager->getZoneInfo( zoneCoord.id );
				} catch (Throwable& t) {
...

		{
			...
			// 게임이 가능한 시점에서만 Dynamic존에 대한 부활 처리를 하도록 한다.
			if ( pGamePlayer != NULL && pGamePlayer->getPlayerStatus() == GPS_NORMAL )
			{
				Zone *pZone = pPC->getZone();
				
				if ( pZone != NULL && pZone->NotifyResurrectCreature(pPC, zoneCoord ) )
				{
					bFindPosition = true;
				}
			}
		...
		{
			// 메네그로스 던전 안에서 죽었을때 부활 위치 설정(부활 위치 설정시)
			Zone *pZone = pPC->getZone();
			ZoneInfo* pZoneInfo = g_pZoneInfoManager->getZoneInfo(pZone->getZoneID());
			if ( pZoneInfo != NULL && pZoneInfo->isMenegrothDoungeonZone() )
			{
				zoneCoord.id = pZone->getZoneID();

				// 해당층의 안전지대로 이동한다.
				if ( g_MenegrothDoungeonManager.getDoungeonSafePos(zoneCoord.id, zoneCoord.x, zoneCoord.y ) )
				{
					g_MenegrothDoungeonManager.sendRemainMenegrothTime(pPC);

					bFindPosition = true;
				}
				// 각 종족의 기본 부활위치로 보낸다.
				else if ( getRaceDefaultPosition( pPC->getRace(), zoneCoord ) )
				{
					bFindPosition = true;
				}
				else
