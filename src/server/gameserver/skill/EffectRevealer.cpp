	...
	throw(Error)
{
	...
	pCreature->getViewAbility()->addCanSeeEffect( Effect::EFFECT_CLASS_HIDE );
...
//	pZone->updateHiddenScan(pCreature);
	...
	ViewAbility prevView = *(pCreature->getViewAbility());
	pCreature->getViewAbility()->removeCanSeeEffect( Effect::EFFECT_CLASS_HIDE );
	pCreature->getZone()->updateViewAbility( pCreature, prevView );
	...
	throw( Error )
{
	...
	SlayerSkillSlot* pSkillSlot = pSlayer->getSkill( SKILL_SNIPING );
