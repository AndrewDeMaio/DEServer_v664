...
// 뱀파이어 인벤토리 핸들러
//////////////////////////////////////////////////////////////////////////////
bool BloodyTunnel::isValidTargetZone(Zone *pTargetZone)
{
	bool bInvalidTargetZone;
	...
	throw(Error)
{
	...
	try 
	{
		...
		if ( price == 0 )
		{
			price = (Price_t)(pow((double)level, 1.3) * 100 );
		...
		bool bHitRoll       = true;
		bool bEnoughMoney	= pVampire->getGold() >= price;

		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bEnoughMoney)
		{
			decreaseMana(pVampire, RequiredMP, _GCSkillToInventoryOK1);

			// 소지금을 줄인다.
			pVampire->decreaseGoldEx( price );
			_GCSkillToInventoryOK1.addLongData( MODIFY_GOLD, pVampire->getGold() );
			...
			// 차지 수를 줄인다.
			// 2005.06.02 더 이상 충전 방식이 아니다. by bezz
			/*
