	...
	throw(Error)
{
	...
	else if (pCreature->isOusters())
	{
		...
		_GCModifyInformation.addShortData(MODIFY_VISION, NewSight);
		pPlayer->sendPacket(&_GCModifyInformation);
		pZone->updateScan(pOusters, FLARE_SIGHT, NewSight );

		GCChangeDarkLight gcChangeDarkLight;
		gcChangeDarkLight.setDarkLevel(13);
		gcChangeDarkLight.setLightLevel(6);
		pPlayer->sendPacket(&gcChangeDarkLight);
	...
	throw(Error)
{
	...
	os << "DELETE FROM EffectFlare WHERE OwnerID = '" << ownerID << "'";
