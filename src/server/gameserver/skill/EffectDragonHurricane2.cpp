//////////////////////////////////////////////////////////////////////////////
// Filename    : EffectDragonHurricane2.cpp
// Written by  :
// Description : DragonHurricane2 tornado. Same as EffectDragonTornado.
//////////////////////////////////////////////////////////////////////////////

#include "DB.h"
#include "EffectDragonHurricane2.h"
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
EffectDragonHurricane2::EffectDragonHurricane2(Zone* pZone, ZoneCoord_t zoneX, ZoneCoord_t zoneY) 
	throw(Error)
{
	__BEGIN_TRY
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void EffectDragonHurricane2::affect()
	throw(Error)
{
	__BEGIN_TRY
	...
	else if ( action <= 8 )
	{
		...
		if ( m_bCanSplit )
		{
			Creature* pCastCreature = m_pZone->getCreature( m_UserOID );
			Creature* pTarget = m_pZone->getCreature( m_TargetOID );
			if ( pTarget == NULL || pTarget->isDead() )
			{
				Range_t dist = 255;
				pTarget = NULL;
				for ( ZoneCoord_t x = m_X-3; x <= m_X+3; ++x )
				for ( ZoneCoord_t y = m_Y-3; y <= m_Y+3; ++y )
				{
					if ( !isValidZoneCoord( m_pZone, x, y ) ) continue;
					Tile& rTile = m_pZone->getTile(x, y);
					slist<Object*>& oList = rTile.getObjectList();

					for ( slist<Object*>::iterator itr = oList.begin() ; itr != oList.end() ; ++itr )
					{
						Creature* pCreature = dynamic_cast<Creature*>(*itr);
						if ( pCreature == NULL || !canAttack(pCastCreature, pCreature) || pCreature->isSlayer() ) continue;

						if ( pTarget == NULL )
						{
							pTarget = pCreature;
							dist = getDistance( m_X, m_Y, pTarget->getX(), pTarget->getY() );
						}
						else if ( pTarget->isMonster() && pCreature->isPC() )
						{
							pTarget = pCreature;
							dist = getDistance( m_X, m_Y, pTarget->getX(), pTarget->getY() );
						}
						else if ( pTarget->isPC() && pCreature->isMonster() )
						{
							continue;
						}
						else
						{
							if ( dist > getDistance( m_X, m_Y, pTarget->getX(), pTarget->getY() ) )
							{
								pTarget = pCreature;
								dist = getDistance( m_X, m_Y, pTarget->getX(), pTarget->getY() );
							}
			...
			if ( pTarget != NULL )
				dir = getDirectionToPosition( m_X, m_Y, pTarget->getX(), pTarget->getY() );
		}

		ZoneCoord_t newX = m_X + dirMoveMask[dir].x;
		ZoneCoord_t newY = m_Y + dirMoveMask[dir].y;
		ZoneLevel_t zoneLevel = m_pZone->getZoneLevel(newX, newY);

		if ( isValidZoneCoord( m_pZone, newX, newY ) && m_pZone->getTile(newX, newY).canAddEffect()  && zoneLevel != COMPLETE_SAFE_ZONE )
		{
	...
	else
	{
		...
		{
			EffectDragonHurricane2* pEffect = new EffectDragonHurricane2( m_pZone, newX, newY );
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void EffectDragonHurricane2::affect(Creature* pCreature)
	throw(Error)
{
	__BEGIN_TRY
	...
	if (pCreature->isSlayer()) 
	{
		...
		::setDamage( pSlayer, m_Damage, pCastCreature, SKILL_DRAGON_HURRICANE_2, &gcSkillToObjectOK6, &gcAttackerMI);
...
/*						Player* pPlayer = pSlayer->getPlayer();
	...
	else if (pCreature->isVampire())
	{
		...
		::setDamage( pVampire, m_Damage, pCastCreature, SKILL_DRAGON_HURRICANE_2, &gcSkillToObjectOK6, &gcAttackerMI );
...
/*						Player* pPlayer = pVampire->getPlayer();
	...
	else if (pCreature->isOusters())
	{
		...
		::setDamage( pOusters, m_Damage, pCastCreature, SKILL_DRAGON_HURRICANE_2, &gcSkillToObjectOK6, &gcAttackerMI );
...
/*						Player* pPlayer = pOusters->getPlayer();
	...
	else if (pCreature->isMonster())
	{
		...
		::setDamage( pMonster, m_Damage, pCastCreature, SKILL_DRAGON_HURRICANE_2, NULL, &gcAttackerMI );
	...
	SkillType_t skillType = (isStun)?SKILL_DRAGON_HURRICANE_2:SKILL_ATTACK_MELEE;
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void EffectDragonHurricane2::unaffect(Creature* pCreature)
	throw(Error)
{
	__BEGIN_TRY
	__END_CATCH
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void EffectDragonHurricane2::unaffect()
	throw(Error)
{
	__BEGIN_TRY
...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
string EffectDragonHurricane2::toString()
	const throw()
{
	__BEGIN_TRY
