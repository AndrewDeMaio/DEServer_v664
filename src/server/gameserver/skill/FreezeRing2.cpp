...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType FreezeRing2::execute(Slayer* pSlayer, SlayerSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
...
//		bool bHitRoll    = HitRoll::isSuccessMagic(pSlayer, pSkillInfo, pSkillSlot);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && !bEffected && !bEffected2 && !bBlockZone)
		{
			...
			Duration_t duration = min(70, (pSlayer->getINT() / pSlayer->getLevel() ) * 35);

			// 이팩트 클래스를 만들어 붙인다.
			EffectFreezeRing2* pEffect = new EffectFreezeRing2(pSlayer, pSlayer->getX(), pSlayer->getY());
			pEffect->setUserObjectID( pSlayer->getObjectID() );
			pEffect->setDeadline(999999);
			pEffect->setDamage(output.Damage);
			pEffect->setDuration(duration);
			pSlayer->addEffect(pEffect);
			pSlayer->setFlag(Effect::EFFECT_CLASS_FREEZE_RING2);
			pEffect->affect();
