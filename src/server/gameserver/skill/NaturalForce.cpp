...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType NaturalForce::execute(Ousters* pOusters, OustersSkillSlot* pOustersSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		bool bEffected   = pOusters->isFlag(Effect::EFFECT_CLASS_NATURAL_FORCE);
		...
		if ( bManaCheck && !bEffected )
		{
			...
			pEffect->setEffectClass(Effect::EFFECT_CLASS_NATURAL_FORCE);
			pEffect->setDeadline(300);
			pOusters->setFlag(Effect::EFFECT_CLASS_NATURAL_FORCE);
			pOusters->addEffect(pEffect);
			
			// 패킷을 날린다.
			_GCSkillToSelfOK1.setSkillType(SkillType);
			_GCSkillToSelfOK1.setCEffectID(Effect::EFFECT_CLASS_NATURAL_FORCE);
			...
			gcAddEffect.setEffectID( Effect::EFFECT_CLASS_NATURAL_FORCE );
