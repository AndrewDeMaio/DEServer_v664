...
void EffectDestructionSpear::affect(Creature* pCreature) throw(Error)
{
	...
	&& !(pZone->getZoneLevel() & COMPLETE_SAFE_ZONE) )
	{
		if ( pCreature->isPC() )
		{
			...
			::setDamage( pPC, getDamage(), NULL, SKILL_DESTRUCTION_SPEAR, &gcMI, &gcAttackerMI, false, m_CanSteal );
			pPC->getPlayer()->sendPacket( &gcMI );
		}
		else if ( pCreature->isMonster() )
		{
			// 공격자가 없다. 샤프실드 적용을 안받고 스틸도 적용 안 받게 하기 위해서다.
			::setDamage( pCreature, getDamage(), NULL, SKILL_DESTRUCTION_SPEAR, NULL, &gcAttackerMI, false, m_CanSteal );
		}
