...
// 아우스터즈 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType MagnumSpear::execute(Ousters* pOusters, ObjectID_t TargetObjectID, OustersSkillSlot* pOustersSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		*/
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bCanHit && bPK && !bEffect)
		{
			...
			if ( pMastery != NULL )
			{
				Damage += ( pMastery->getExpLevel() * 5 / 3 ) + 15;     // 수정전공식
			}
			else
			{
				bool dummy;
			...
			// 한번만 Damage 적용, Client의 연출은 그대로 하기 위해 여기서 spearNum 고정
			spearNum = 1;
