...
SkillResultType SummonWaterElemental::execute(Ousters* pOusters, OustersSkillSlot* pOustersSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && bSatisfyRequire)
		{
			...
			//cout << pOusters->getName() << "이 SummonWaterElemental를 썼고 데미지는 " << output.Damage << "입니다." << endl;
