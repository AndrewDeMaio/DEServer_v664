...
SkillResultType SimpleTileMissileSkill::execute(Slayer* pSlayer, int X, int Y, SlayerSkillSlot* pSkillSlot,
				...
				if (rect.ptInRect(tileX, tileY))
				{
					...
					for(; itr != targetList.end(); itr++)
					{
						...
						if (pTargetCreature != pSlayer)
						{
							...
							{
								if( param.SkillType == SKILL_HEAVENLY_SPARK ) // 20080429 원본 스킬레벨로 Tohit Bonus 설정
								{
									SlayerSkillSlot* pThunderSkillSlot = pSlayer->hasSkill(SKILL_INFINITY_THUNDERBOLT);
									if ( pThunderSkillSlot != NULL )
									{
										SkillLevel = pThunderSkillSlot->getExpLevel();
									}
									else
									{
										SkillLevel = 0;
								...
								if( param.SkillType == SKILL_GENESIS ) // 20080429 원본 스킬레벨로 Tohit Bonus 설정
								{
									SlayerSkillSlot* pBombingSkillSlot = pSlayer->hasSkill(SKILL_BOMBING_STAR);
									if ( pBombingSkillSlot != NULL )
									{
										SkillLevel = pBombingSkillSlot->getExpLevel();
									}
									else
									{
										SkillLevel = 0;
							...
							if ( ( param.SkillType == SKILL_INFINITY_THUNDERBOLT || param.SkillType == SKILL_ROTTEN_APPLE  || param.SkillType == SKILL_HEAVENLY_SPARK )
								&& mask.x == 0 && mask.y == 0 )
							{
								bRaceCheck = !pTargetCreature->isNPC();
//								bRaceCheck = !pTargetCreature->isSlayer();
							}
...
//							bool bHitRoll        = HitRoll::isSuccess( pSlayer, pTargetCreature, SkillLevel );
							...
							if (bPK && bRaceCheck && bZoneLevelCheck && bHitRoll )
							{
								...
								if ( bCriticalHit && pSlayer->isFlag( Effect::EFFECT_CLASS_BURST_GUN ) )
								{
								...
								{
									bHit = true;
									if ( maxEnemyLevel < pTargetCreature->getLevel() ) // 20080502 도메인 경험치 추가 
									{
										maxEnemyLevel = pTargetCreature->getLevel();
										pSlayer->setEnemyTarget(pTargetCreature->getObjectID());
									}
									EnemyNum++;
								}
							}
...
//					_GCSkillToTileOK2.setSkillType(param.SkillType);
	...
	pSlayer->setEnemyTarget( 0 ); // 20080502 도메인 경험치 보너스 추가
...
SkillResultType SimpleTileMissileSkill::execute(Vampire* pVampire, int X, int Y, VampireSkillSlot* pVampireSkillSlot, 
	const SIMPLE_SKILL_INPUT& param,  SIMPLE_SKILL_OUTPUT& result,
	CEffectID_t CEffectID, bool bForceKnockback, int HitBonus) throw (Error)
{
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck )
		{
...
#ifdef VERSION_SKILL_1
								bHitRoll &= HitRoll::isSuccessMagic(pVampire, pTargetCreature);
#endif
							}
							else
							{
			...
			// 스킬 후 처리
			postSkillSuccessProcess( param.SkillType, pVampire, &_GCSkillToTileOK1 );
...
SkillResultType SimpleTileMissileSkill::execute(Ousters* pOusters, int X, int Y, OustersSkillSlot* pOustersSkillSlot, 
	const SIMPLE_SKILL_INPUT& param,  SIMPLE_SKILL_OUTPUT& result,
	CEffectID_t CEffectID, bool bForceKnockback, int HitBonus) throw (Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck )
		{
			...
			for (; itr != param.MaskList.end(); itr++)
			{
				...
				if ( 
					( param.SkillType == SKILL_DESTRUCTION_SPEAR || param.SkillType == SKILL_ICE_LANCE || param.SkillType == SKILL_BRIONAC ) // 20080416 SKILL_BRIONAC
				&&  (mask.x != 0 || mask.y != 0)
				)
				{
					canSteal = false;
				...
				if (rect.ptInRect(tileX, tileY))
				{
					...
					for(; itr != targetList.end(); itr++)
					{
						...
						if (pTargetCreature != pOusters)
						{
							...
							if ( (param.SkillType == SKILL_DESTRUCTION_SPEAR || param.SkillType == SKILL_BRIONAC) && mask.x == 0 && mask.y == 0 )
							{
								bRaceCheck = pTargetCreature->isNPC();
							}
...
#ifdef VERSION_SKILL_1
								bHitRoll &= HitRoll::isSuccessMagic(pOusters, pTargetCreature);
#endif
							}
							else
							{
							...
							if (bPK && !bRaceCheck && bZoneLevelCheck && bHitRoll)
							{
								...
								if (param.bMagicDamage)
								{
									// 아우도 마법저항 적용되도록 바꾼다. 
#ifndef VERSION_SKILL_1
									Damage += computeMagicDamage(pTargetCreature, computeOustersMagicDamage( pOusters, pTargetCreature, param.SkillDamage, param.SkillType ), param.SkillType);
#else
									Damage += param.SkillDamage;
#endif
...
//									int exp = computeCreatureExp(pTargetCreature, 100, pOusters);
			...
			// 스킬 후 처리
			postSkillSuccessProcess( param.SkillType, pOusters, &_GCSkillToTileOK1 );
...
//							static bool bNonPK = g_pGameServerInfoManager->getGameServerInfo( 1, g_pConfig->getPropertyInt( "ServerID" ), g_pConfig->getPropertyInt( "WorldID" ) )->isNonPKServer();
							...
							if (bPK && bRaceCheck && bZoneLevelCheck)
							{
								...
								else if( param.SkillType == SKILL_ICE_LANCE )
								{
									Damage = 125;
									setDamage(pEnemy, Damage, pMonster, param.SkillType, NULL, NULL, false);
								}
