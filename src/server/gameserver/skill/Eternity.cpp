...
// 슬레이어 셀프
//////////////////////////////////////////////////////////////////////////////
SkillResultType Eternity::execute(Slayer* pSlayer, SlayerSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected)
		{
			...
			// 2007.02.08 부활 이펙트가 붙어 있으면 없애 준다.
			if( pSlayer->isFlag( Effect::EFFECT_CLASS_REQUEST_RESURRECT ) )
			{
				Effect* pEffectResurrect = pSlayer->findEffect( Effect::EFFECT_CLASS_REQUEST_RESURRECT );
				if( pEffectResurrect != NULL )
				{
					pEffectResurrect->setDeadline(0);
				}
			}
