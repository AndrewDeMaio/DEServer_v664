...
//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
void CreateHolyPotion::execute(Slayer * pSlayer , ObjectID_t InvenObjectID, CoordInven_t X, CoordInven_t Y, CoordInven_t TargetX, CoordInven_t TargetY, SlayerSkillSlot * pSkillSlot)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bCanMake)
		{
			...
			if ( pPrevHolyPotion != NULL
				&& pPrevHolyPotion->getNum() + pHolyPotion->getNum() > ItemMaxStack[pHolyPotion->getItemClass()] )
			{
				//cout << (int)pPrevHolyPotion->getNum() << " + " << (int)pHolyPotion->getNum() << " > "
				//	<< (int)ItemMaxStack[pHolyPotion->getItemClass()] << endl;
			...
			// pPotion의 개수가 1인 경우 위의 decreaseItemNum 에서 삭제되게된다.
			// 이때 pPrevHolyPotion == pPotion 이었으므로
			// pPrevHolyPotion 은 더 이상 유효한 포인터가 아니므로 다시 받아온다.
			// 급하게 수정하는 관계로 인벤토리에서 포인터를 다시 세팅하도록 한다.
			pPrevHolyPotion = pInventory->getItem(TargetX, TargetY);
...
//				cout << (int)pPrevHolyPotion->getNum() << " + " << (int)pHolyPotion->getNum() << endl;
...
//				cout << (int)pPrevHolyPotion->getNum() << endl;
