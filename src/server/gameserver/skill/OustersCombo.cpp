...
// 뱀파이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType OustersCombo::execute(Ousters* pOusters, ObjectID_t TargetObjectID, OustersSkillSlot* pSkillSlot, CEffectID_t CEffectID)
    throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		bool bCanHit     = canHit(pOusters, pTargetCreature, pSkillSlot->getSkillType());
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bCanHit && bPK)
		{
			...
			CheckCrossCounter(pOusters, pTargetCreature, output.Damage, pSkillInfo->getRange());
...
//			pEffect->setMaxStep( pSkillSlot->getCombo() );
			...
			// 스킬 후 처리
			postSkillSuccessProcess( pSkillSlot->getSkillType(), pOusters, &_GCSkillToObjectOK1 );
		...
		{
			executeSkillFailNormal(pOusters, pSkillSlot->getSkillType(), pTargetCreature );
		}
	} 
	catch (Throwable & t) 
	{
