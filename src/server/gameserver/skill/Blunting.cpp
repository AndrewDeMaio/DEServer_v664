...
SkillResultType Blunting::execute(Ousters* pOusters, ObjectID_t TargetObjectID, OustersSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
...
//		// 2005.11.16 bHitRoll은 Magic HitRoll 공식에서 Success HitRoll 공식으로 대체 되었습니다. by Taehyoun
		bool bHitRoll	 = false;
		...
		{
			successRatio = min( 50, (input.STR / 5) * (1 + input.SkillLevel * 2) );
		}
		else if ( input.SkillLevel > 15 && input.SkillLevel < 30 )
		{
			successRatio = min( 70, (input.STR / 5) * (1 + input.SkillLevel * 2) );
		}
		else
		{
			successRatio = min( 80, (input.STR / 5) * (1 + input.SkillLevel *2) );
		}

		bHitRoll = (rand()%100) < successRatio;

//		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bHitRoll2 && bCanHit && !bEffected && bPK && bSatisfyRequire)
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bCanHit && !bEffected && bPK && bSatisfyRequire)
		{
