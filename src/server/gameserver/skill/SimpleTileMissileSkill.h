...
public:
	...
	SkillResultType execute(Vampire* pVampire, int X, int Y, VampireSkillSlot* pVampireSkillSlot, 
		const SIMPLE_SKILL_INPUT& param,  SIMPLE_SKILL_OUTPUT& result,
		CEffectID_t CEffectID=0, bool bForceKnockback=false, int HitBonus=0) throw (Error);
	SkillResultType execute(Ousters* pOusters, int X, int Y, OustersSkillSlot* pOustersSkillSlot, 
		const SIMPLE_SKILL_INPUT& param,  SIMPLE_SKILL_OUTPUT& result,
		CEffectID_t CEffectID=0, bool bForceKnockback=false, int HitBonus=0) throw (Error);
