...
SkillResultType FuryOfGnome::execute(Ousters* pOusters, ObjectID_t TargetObjectID, OustersSkillSlot* pOustersSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
...
SkillResultType FuryOfGnome::execute(Ousters* pOusters, ZoneCoord_t X, ZoneCoord_t Y, OustersSkillSlot* pOustersSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
...
/*	if ( pTargetCreature->isSlayer() )
	...
	if(result.bSuccess)
	{
		...
		if( markTile.getEffect(Effect::EFFECT_CLASS_SUMMON_CLAY) != NULL ) bTileCheck=false;

		if ( markTile.canAddEffect() && bTileCheck )
		{
