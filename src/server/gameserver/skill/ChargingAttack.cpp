...
// 아우스터즈 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType ChargingAttack::execute(Ousters * pOusters, ObjectID_t TargetObjectID, OustersSkillSlot * pOustersSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bCanHit && bPK && !bEffected )
		{
			...
			if (pZone->moveFastPC(pOusters, pOusters->getX(), pOusters->getY(), pTargetCreature->getX(), pTargetCreature->getY(), getSkillType())) 
			{
				...
				else if( !pTargetCreature->isFlag(Effect::EFFECT_CLASS_STUN ) )
				{
					EffectStun* pEffectStun = new EffectStun(pTargetCreature);
					pEffectStun->setDeadline(output.Duration);
					pTargetCreature->addEffect(pEffectStun);
					pTargetCreature->setFlag(Effect::EFFECT_CLASS_STUN);

					GCAddEffect gcAddEffect;
					gcAddEffect.setObjectID(pTargetCreature->getObjectID());
					gcAddEffect.setEffectID(Effect::EFFECT_CLASS_STUN);
