...
// 슬레이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType IntimateGrail::execute(Slayer* pSlayer, ObjectID_t TargetObjectID, SlayerSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected)
		{
			...
			// hpbonus, defensebonus 는 슬레이어만 적용. InitAllStat 에서 따로 계산
			int hpbonus = min( 225, (int)( ( 50 + ( input.INTE / 3 ) + ( input.SkillLevel / 4 ) * ( pSlayer->getLevel() / 150.0 ) ) ) );
			int defensebonus = min( 80, 10 + ( input.INTE / 10 ) + (int)( ( input.SkillLevel / 10 ) * ( pSlayer->getLevel() / 150.0 ) ) );
...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType IntimateGrail::execute(Slayer* pSlayer, SlayerSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
