//////////////////////////////////////////////////////////////////////////////
// Filename    : ShadyDouble.cpp
// Written by  : excel96
// Description : Shady Double (skill 554). Single-target vampire magic: three hits, one as each of the
//               client's ghosts (launched 0.3 s apart) reaches the target.
//               Same flow as Acid Eruption, which the v9 gameserver (ShadyDouple) was cloned from.
//////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
// When the client's first ghost reaches the target (0.1 s): it leaves on cast
// frame 5 and flies 20 pixels a frame (tiles are 48x24 pixels, 16 frames a
// second); the other two ghosts follow 0.3 s apart.
//////////////////////////////////////////////////////////////////////////////
static Turn_t firstGhostArrival(ZoneCoord_t fromX, ZoneCoord_t fromY, ZoneCoord_t toX, ZoneCoord_t toY)
{
	double dx = ((int)fromX - (int)toX) * 48.0;
	double dy = ((int)fromY - (int)toY) * 24.0;
	int frames = 5 + (int)ceil(sqrt(dx * dx + dy * dy) / 20.0);

	return (Turn_t)max(1, (frames * 10 + 8) / 16);
}

//////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////
SkillResultType ShadyDouble::execute(Vampire* pVampire, ObjectID_t TargetObjectID, VampireSkillSlot* pVampireSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		bool bEffected   = pTargetCreature->isFlag(Effect::EFFECT_CLASS_SHADY_DOUPLE);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bCanHit && bPK && !bEffected)
		{
			...
			EffectShadyDouble* pEffect = new EffectShadyDouble( pTargetCreature );
#ifndef VERSION_SKILL_1
			pEffect->setDamage( computeMagicDamage( pTargetCreature, output.Damage, SKILL_SHADY_DOUPLE, true, pVampire ) );
#else
			pEffect->setDamage( output.Damage );
#endif
			pEffect->setTimes(3);	// one hit per ghost, as each one reaches the target
			pEffect->setTick(3);
			pEffect->setNextTime(firstGhostArrival(vampX, vampY, targetX, targetY));
