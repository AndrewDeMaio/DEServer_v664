...
SkillResultType SimpleLinearMeleeSkill::execute(Slayer* pSlayer, int X, int Y, SlayerSkillSlot* pSkillSlot,
			...
			for (; ptitr != ptList.end(); ptitr++)
			{
				...
				if (rect.ptInRect(tileX, tileY))
				{
					...
					for(; itr != targetList.end(); itr++)
					{
						...
						if ( (param.SkillType == SKILL_SPIT_STREAM || param.SkillType == SKILL_BLADE_STORM) && tileX == X && tileY == Y )
						{
						...
						else
						{
							...
							if( param.SkillType == SKILL_BLADE_STORM ) // 20080429 원본 스킬레벨로 Tohit Bonus 설정
							{
								SlayerSkillSlot* pSpitSkillSlot = pSlayer->hasSkill(SKILL_SPIT_STREAM);
								if ( pSpitSkillSlot != NULL )
								{
									SkillLevel = pSpitSkillSlot->getExpLevel();
								}
								else
								{
									SkillLevel = 0;
