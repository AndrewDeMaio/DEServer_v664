...
// 슬레이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType CureAll::execute(Slayer* pSlayer, ObjectID_t TargetObjectID, SlayerSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (pTargetCreature->isEffect(Effect::EFFECT_CLASS_POISON)) 
		{
			...
			bGreenPoison = HitRoll::isSuccessNewCurePoison( pSlayer->getINT(), SkillLevel, pEffectPoison->getPoisonLevel() );
			bEffected = true;
			bHPCheck = true;
		}
		...
		if (pTargetCreature->isEffect(Effect::EFFECT_CLASS_YELLOW_POISON_TO_CREATURE)) 
		{
			...
			bYellowPoison = HitRoll::isSuccessNewCurePoison( pSlayer->getINT(), SkillLevel, pEffectYellowPoisonToCreature->getPoisonLevel() );
			bEffected = true;
			bHPCheck = true;
		}
		...
		if (pTargetCreature->isEffect(Effect::EFFECT_CLASS_DARKBLUE_POISON)) 
		{
			...
			bDarkBluePoison = HitRoll::isSuccessNewCurePoison( pSlayer->getINT(), SkillLevel, pEffectDBP->getPoisonLevel() );
			bEffected = true;
			bHPCheck = true;
		}
		...
		if (pTargetCreature->isEffect(Effect::EFFECT_CLASS_GREEN_STALKER)) 
		{
			...
			bGreenStalker = HitRoll::isSuccessNewCurePoison( pSlayer->getINT(), SkillLevel, pEffectGreenStalker->getPoisonLevel() );
			bEffected = true;
			bHPCheck = true;
		}
		...
		if (bHPCheck && bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && pTargetCreature->isAlive())
		{
			...
			// 스킬 후 처리
			postSkillSuccessProcess( getSkillType(), pSlayer, &_GCSkillToObjectOK1 );
...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType CureAll::execute(Slayer* pSlayer, SlayerSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bHPCheck && pSlayer->isAlive())
		{
			...
			// 스킬 후 처리
			postSkillSuccessProcess( getSkillType(), pSlayer, &_GCSkillToSelfOK1 );
