...
SkillResultType SimpleTileMeleeSkill::execute(Slayer* pSlayer, int X, int Y, SlayerSkillSlot* pSkillSlot,
...
SkillResultType SimpleTileMeleeSkill::execute(Vampire* pVampire, int X, int Y, VampireSkillSlot* pVampireSkillSlot, 
	const SIMPLE_SKILL_INPUT& param,  SIMPLE_SKILL_OUTPUT& result,
	CEffectID_t CEffectID, bool bForceKnockback) throw (Error)
{
	__BEGIN_TRY
				...
				if (rect.ptInRect(tileX, tileY))
				{
					...
					for(; itr != targetList.end(); itr++)
					{
						...
						if (pTargetCreature->getObjectID() != pVampire->getObjectID())
						{
							...
							if ( ((param.SkillType == SKILL_VIOLENT_PHANTOM) || (param.SkillType == SKILL_LETHAL_CLAW)) && mask.x == 0 && mask.y == 0 )
							{
...
SkillResultType SimpleTileMeleeSkill::execute(Ousters* pOusters, int X, int Y, OustersSkillSlot* pOustersSkillSlot,
	const SIMPLE_SKILL_INPUT& param, SIMPLE_SKILL_OUTPUT& result,
	CEffectID_t CEffectID, bool bForceKnockback) throw (Error)
{
	__BEGIN_TRY
