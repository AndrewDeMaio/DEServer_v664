...
// 아우스터즈 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType HellFire::execute(Ousters* pOusters, ObjectID_t TargetObjectID, OustersSkillSlot* pOustersSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
...
// 아우스터즈 타일 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType HellFire::execute(Ousters* pOusters, ZoneCoord_t X, ZoneCoord_t Y, OustersSkillSlot* pOustersSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		bool bEffected   = pOusters->isFlag(Effect::EFFECT_CLASS_FIERCE_FLAME); // 20080131 피어스프레임이랑 같이 못쓴다.

		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bTileCheck && bSatisfyRequire && !bEffected)
		{
			...
			// 이펙트 클래스를 생성한다.
			//20071227 - 헬파이어 수정
			//		EffectHellFire 에 설정되는 값들이 변경된다.
			//			Damage에 output.Damage 가 적용된다. 이 값은 EffectHellFire의 Damage에는 0.4배로, EffectHellFireToEnemy의 Damage에는 1/7배로 적용될 것이다.
			//		!<참고> setNextTime (0) 은 저기 아래에 pEffect->affect 를 호출하기 때문에 의미가 없다. 왜? affect 함수에서 setNextTime (m_Tick)이 호출되기 때문.
			//		!<참고> output.Duration값은 95으로 고정값이다. 즉, 무조건 10초동안 EffectHellFire가 유지된다는 의미.
			//		!<참고> output.Tick값은 20으로 고정값이다. 즉, 2초 주기로 EffectHellFire::affect가 호출될 것이다.
			...
			//			pEffect->setDamage(output.ToHit);
						...
						pEffect->setNextTime(output.Tick);	//아래에서 Damage를 한 번 준다. 따라서... Tick 이후에 affect가 호출되도록 한다.
			// -
...
//								&& ( (pTargetCreature = dynamic_cast<Creature*>(pTarget))->isSlayer() || pTargetCreature->isVampire() )
								&& ( (pTargetCreature = dynamic_cast<Creature*>(pTarget))->isSlayer() || pTargetCreature->isVampire() || pTargetCreature->isMonster())
								&& checkZoneLevelToHitTarget( pTargetCreature ) && canAttack( pOusters, pTargetCreature )
#ifdef VERSION_SKILL_1
								&& HitRoll::isSuccessMagic( pOusters, pTargetCreature )
#endif
							) 
							{
								...
								pEffect->affect ();
					/*
								//20071227 - 수정	-- EffectHellFire의 Damage는 0.4배.
								//								setDamage( pTargetCreature, output.Damage, pOusters, pOustersSkillSlot->getSkillType(), &gcMI, NULL, true, false );
								setDamage( pTargetCreature, output.Damage * 0.4, pOusters, pOustersSkillSlot->getSkillType(), &gcMI, NULL, true, false );
								// -
