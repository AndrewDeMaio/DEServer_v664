	...
	throw(Error)
{
	...
	try 
	{
		...
		bool bEffected   = pTargetCreature->isFlag(Effect::EFFECT_CLASS_BLOOD_DRAIN);
		...
		if (bHitRoll && bCanHit && bTimeCheck && bRangeCheck && bPK)
		{
			...
			if ( pTargetCreature->isSlayer() && !bEffected )
			{
...
//			}
			...
			// 스킬 후 처리
			postSkillSuccessProcess( getSkillType(), pVampire, &_GCBloodDrainOK1 );
	...
	throw (Error)
{
	...
	bool bEffectBloodDrain = pEnemy->isFlag(Effect::EFFECT_CLASS_BLOOD_DRAIN);
