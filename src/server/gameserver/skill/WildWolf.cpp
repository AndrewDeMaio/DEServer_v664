...
// 뱀파이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType WildWolf::execute(Vampire* pVampire, ObjectID_t TargetObjectID, VampireSkillSlot* pVampireSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try
	{
		...
		if ( pTargetCreature==NULL )
		{
			...
			return eatCorpse( pVampire, pTargetItem, pVampireSkillSlot );
		}
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bCanHit && bPK)
		{
			...
			// 스킬 후 처리
			postSkillSuccessProcess( getSkillType(), pVampire, &_GCSkillToObjectOK1 );
...
SkillResultType WildWolf::eatCorpse(Vampire* pVampire, Item* pCorpse, VampireSkillSlot* pVampireSkillSlot)
{
	__BEGIN_TRY
		...
		&& pCorpse->getItemType() == MONSTER_CORPSE )
	{
		...
		if ( pMonsterCorpse->isFlag( Effect::EFFECT_CLASS_SLAYER_RELIC_TABLE )
			...
			|| pMonsterCorpse->getMonsterType() == 808
			|| pMonsterCorpse->getMonsterType() == 809
	...
	if ( bManaCheck && bTimeCheck && bRangeCheck )
	{
		...
		_GCSkillToObjectOK1.setDuration(output.Delay);
