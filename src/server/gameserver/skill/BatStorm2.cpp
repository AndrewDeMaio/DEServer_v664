...
// 뱀파이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType BatStorm2::execute(Vampire* pVampire, ObjectID_t TargetObjectID, VampireSkillSlot* pVampireSkillSlot, CEffectID_t CEffectID)
    throw(Error)
{
    __BEGIN_TRY
...
// 뱀파이어 타일 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType BatStorm2::execute(Vampire* pVampire, ZoneCoord_t X, ZoneCoord_t Y, VampireSkillSlot* pVampireSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
...
//		bool bHitRoll    = HitRoll::isSuccessMagic(pVampire, pSkillInfo, pVampireSkillSlot, HitBonus);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bTileCheck)
		{
			...
			Effect* pOldEffect = tile.getEffect(Effect::EFFECT_CLASS_BAT_STORM2);
			if (pOldEffect != NULL)
			{
...
//			checkMine( pZone, X, Y );
			...
			// 이펙트 오브젝트를 생성한다.
			EffectBatStorm2* pEffect = new EffectBatStorm2(pZone, X, Y, true);
	...
	throw(Error)
{
	...
	try 
	{
		...
		SkillType_t SkillType  = SKILL_BAT_STORM2;
		SkillInfo*  pSkillInfo = g_pSkillInfoManager->getSkillInfo(SkillType);
...
//		bool bHitRoll    = HitRoll::isSuccessMagic(pMonster, pSkillInfo);
		...
		if (bRangeCheck && bTileCheck)
		{
			...
			// 이펙트 오브젝트를 생성한다.
			EffectBatStorm2* pEffect = new EffectBatStorm2(pZone, X, Y);
