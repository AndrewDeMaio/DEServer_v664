...
// 뱀파이어 타일 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType EatCorpse::execute(Vampire* pVampire, ZoneCoord_t X, ZoneCoord_t Y, VampireSkillSlot* pVampireSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		  ...
		  && pItem->getItemType() == MONSTER_CORPSE )
		{
			...
			if ( !pCorpse->isFlag(Effect::EFFECT_CLASS_SLAYER_RELIC_TABLE)
			  ...
			  && pCorpse->getMonsterType() != 808
			  && pCorpse->getMonsterType() != 809
			  && pCorpse->getMonsterType() != 819) || (pVampire->isGOD() || pVampire->isDM())
			  && !pCorpse->isShrine() )
			{
				bEatableCorpse = true;
			}
...
//			int RemainMP = max((int)pVampire->getHP(ATTR_MAX), (int)pVampire->getHP(ATTR_CURRENT) + 10);
			// 2006.09.19 수정
			int RemainMP = min(60, 10+(pCorpse->getLevel()/3));
			if( !pVampire->isFlag(Effect::EFFECT_CLASS_EAT_CORPSE) )
			{
				EffectEatCorpse* pEffect  = new EffectEatCorpse((Creature*)pVampire);
				pEffect->setRemainHP((HP_t)RemainMP);
				pEffect->setDelay(20);
				pEffect->setNextTime(0);
				pEffect->setDeadline(300);
				pVampire->setFlag(Effect::EFFECT_CLASS_EAT_CORPSE);
