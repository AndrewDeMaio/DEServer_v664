...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType Thorns::execute(Slayer* pSlayer, SlayerSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected)
		{
			...
			// 이팩트 클래스를 만들어 붙인다.
			EffectThorns* pEffect = new EffectThorns(pSlayer);
			...
			pSlayer->setFlag(Effect::EFFECT_CLASS_THORNS);
...
// 슬레이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
SkillResultType Thorns::execute(Slayer* pSlayer, ObjectID_t ObjectID, SlayerSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && bSameParty)
		{
			...
			// 이팩트 클래스를 만들어 붙인다.
			EffectThorns* pEffect = new EffectThorns(pTargetCreature);
			pEffect->setCasterID( pSlayer->getObjectID() );
			pEffect->setDeadline(output.Duration);
			pEffect->setDamage( output.Damage );
			pEffect->setNextTime( 0 );			// 일단 단번에 발동한다. by Sequoia
			...
			pTargetCreature->setFlag(Effect::EFFECT_CLASS_THORNS);
