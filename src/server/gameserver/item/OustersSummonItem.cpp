	...
	throw(Error)
{
	...
	os << "INSERT INTO OustersSummonItemObject "
	...
	throw(Error)
{
	...
	os << "UPDATE OustersSummonItemObject SET "
		<< "ObjectID=" << m_ObjectID
		<< ", ItemType=" << m_ItemType
	...
	BEGIN_DB 
	{
		...
		*/
		...
		if (migCharID == 0)
		{
			...
			filelog("CharIDMigration.log", "OustersSummonItemObject: CharID unresolved for [%s], using name",
		...
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, Charge FROM OustersSummonItemObject WHERE OwnerCharID = %u AND Storage IN(0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
