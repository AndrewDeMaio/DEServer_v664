	...
	throw(Error)
{
	...
	os << "INSERT INTO CarryingReceiverObject "
	...
	throw(Error)
{
	...
	os << "UPDATE CarryingReceiverObject SET "
		<< "ObjectID=" << m_ObjectID
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		if (migCharID == 0)
		{
			...
			filelog("CharIDMigration.log", "CarryingReceiverObject: CharID unresolved for [%s], using name",
				pCreature->getName().c_str());

			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, ThirdOptionType, Grade, EnchantLevel, ItemFlag FROM CarryingReceiverObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 9)",
				pCreature->getName().c_str() );
		}
		else
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, ThirdOptionType, Grade, EnchantLevel, ItemFlag FROM CarryingReceiverObject WHERE OwnerCharID = %u AND Storage IN(0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
#else
				// CharID migration: load by the surrogate key rather than the name.
		...
		if (migCharID == 0)
		{
			...
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Grade, EnchantLevel, ItemFlag FROM CarryingReceiverObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 9)",
				pCreature->getName().c_str() );
		}
		else
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Grade, EnchantLevel, ItemFlag FROM CarryingReceiverObject WHERE OwnerCharID = %u AND Storage IN(0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
#endif
		...
		while (pResult->next())
		{
			try {
				...
				pCarryingReceiver->setThirdOptionType(thirdoptionTypes);
#endif
				...
				switch(storage)
				{
					...
					case STORAGE_GEAR:
						if (pCreature->isSlayer())
						{
							...
							{
								pSlayer->putItemToGear(x, y, pCarryingReceiver);
							}
						}
						else if (pCreature->isVampire())
						{
