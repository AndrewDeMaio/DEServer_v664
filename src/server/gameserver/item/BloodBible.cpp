	...
	throw(Error)
{
	...
	os << "INSERT INTO BloodBibleObject "
	...
	throw(Error)
{
	...
	filelog( "WarLog.txt", "Update BloodBibleObject : %s", field );
}
	...
	throw(Error)
{
	...
	os << "UPDATE BloodBibleObject SET "
		<< "ObjectID=" << m_ObjectID
		<< ", ItemType=" << m_ItemType
	...
	BEGIN_DB 
	{
		...
		*/
		...
		if (migCharID == 0)
		{
			...
			filelog("CharIDMigration.log", "BloodBibleObject: CharID unresolved for [%s], using name",
				pCreature->getName().c_str());

			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Durability, EnchantLevel FROM BloodBibleObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 9)",
				pCreature->getName().c_str() );
		}
		else
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Durability, EnchantLevel FROM BloodBibleObject WHERE OwnerCharID = %u AND Storage IN(0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
