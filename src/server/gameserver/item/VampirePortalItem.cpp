	...
	throw(Error)
{
	...
	os << "INSERT INTO VampirePortalItemObject "
	...
	throw(Error)
{
	...
	os << "UPDATE VampirePortalItemObject SET "
		<< "ObjectID=" << m_ObjectID
		<< ", ItemType=" << m_ItemType
	...
	BEGIN_DB 
	{
		...
		*/
		...
		if (migCharID == 0)
		{
			...
			filelog("CharIDMigration.log", "VampirePortalItemObject: CharID unresolved for [%s], using name",
		...
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, Charge, TargetZID, TargetX, TargetY FROM VampirePortalItemObject WHERE OwnerCharID = %u AND Storage IN(0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
