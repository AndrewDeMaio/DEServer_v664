	...
	throw(Error)
{
	...
	os << "INSERT INTO VampireCoupleRingObject "
	...
	throw(Error)
{
	...
	os << "UPDATE VampireCoupleRingObject SET "
		<< "ObjectID=" << m_ObjectID
		<< ", ItemType=" << m_ItemType
	...
	BEGIN_DB 
	{
		...
		*/
		...
		if (migCharID == 0)
		{
			...
			filelog("CharIDMigration.log", "VampireCoupleRingObject: CharID unresolved for [%s], using name",
				pCreature->getName().c_str());

			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, ThirdOptionType, Name, PartnerItemID FROM VampireCoupleRingObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 9)",
				pCreature->getName().c_str() );
		}
		else
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, ThirdOptionType, Name, PartnerItemID FROM VampireCoupleRingObject WHERE OwnerCharID = %u AND Storage IN(0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
#else
				// CharID migration: load by the surrogate key rather than the name.
		...
		if (migCharID == 0)
		{
			...
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Name, PartnerItemID FROM VampireCoupleRingObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 9)",
				pCreature->getName().c_str() );
		}
		else
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Name, PartnerItemID FROM VampireCoupleRingObject WHERE OwnerCharID = %u AND Storage IN(0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
#endif
		...
		while (pResult->next())
		{
			try {
				...
				pVampireCoupleRing->setThirdOptionType(thirdoptionTypes);
#endif
				...
				switch(storage)
				{
					...
					case STORAGE_GEAR:
						if (pCreature->isSlayer())
						{
							...
							{
								pSlayer->putItemToGear(x, y, pVampireCoupleRing);
							}
						}
						else if ( pCreature->isVampire() )
						{
							...
							{
								pVampire->putItemToGear(x, y, pVampireCoupleRing);
							}
						}
						break;
