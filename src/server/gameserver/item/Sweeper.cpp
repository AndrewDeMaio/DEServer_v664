	...
	throw(Error)
{
	...
	os << "INSERT INTO SweeperObject "
		<< "(ItemID,  ObjectID, ItemType, OwnerID, Storage, StorageID ,"
		<< " X, Y, Durability) VALUES(" 
		<< m_ItemID << ", "
		<< m_ObjectID << ", " 
		<< m_ItemType << ", '"
		<< ownerID << "', " 
	...
	filelog( "WarLog.txt", "%s", ownerID.c_str(), os.str().c_str() );
...
// save item
//--------------------------------------------------------------------------------
void Sweeper::tinysave(const string & field) const
	throw(Error)
{
	__BEGIN_TRY
	
	Item::tinysave(field);
	filelog( "WarLog.txt", "Update SweeperObject : %s", field.c_str() );
	...
	BEGIN_DB 
	{
		...
		*/
		...
		if (migCharID == 0)
		{
			...
			filelog("CharIDMigration.log", "SweeperObject: CharID unresolved for [%s], using name",
				pCreature->getName().c_str());

			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Durability, EnchantLevel FROM SweeperObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 9)",
				pCreature->getName().c_str() );
		}
		else
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Durability, EnchantLevel FROM SweeperObject WHERE OwnerCharID = %u AND Storage IN(0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
