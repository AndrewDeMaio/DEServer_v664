...
ITEMIDREGISTRY(CommonQuestItem,"CommonQuestItemObject");
ITEMIDREGISTRY(EtherealChain,"EtherealChainObject");
ITEMIDREGISTRY(OustersHarmonicPendent,"OustersHarmonicPendentObject");
ITEMIDREGISTRY(CheckMoney,"CheckMoneyObject");
ITEMIDREGISTRY(CueOfAdam,"CueOfAdamObject");
ITEMIDREGISTRY(ContractOfBlood,"ContractOfBloodObject");

ITEMIDREGISTRY(VampireWingItem,"VampireWingItemObject");
ITEMIDREGISTRY(OustersWingItem,"OustersWingItemObject");
ITEMIDREGISTRY(SlayerTunningItem,"SlayerTunningItemObject");
ITEMIDREGISTRY(VampireTunningItem,"VampireTunningItemObject");
ITEMIDREGISTRY(OustersTunningItem,"OustersTunningItemObject");

ITEMIDREGISTRY(CallNPCCard,"CallNPCCardObject");
...
void SkillBook::initItemIDRegistry(void)
	throw()
{
	__BEGIN_TRY
	...
	{
		pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
		Result* pCountResult = pStmt->executeQuery("SELECT COUNT(*) from SkillBookObject");
		pCountResult->next();
		int count = pCountResult->getDWORD(1);
		SAFE_DELETE(pStmt);
		...
		{
			pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
			Result* pResult = pStmt->executeQuery("SELECT MAX(ItemID) FROM SkillBookObject");
			pResult->next();
			m_ItemIDRegistry = pResult->getDWORD(1);
			SAFE_DELETE(pStmt);
		}
	...
	cout << "SkillBookItem...ItemIDRegistry:" << m_ItemIDRegistry << endl;
