	...
	throw(Error)
{
	...
	os << "INSERT INTO BeltObject ("
		<< "ItemID,  ObjectID, ItemType, OwnerID, Storage, StorageID, X, Y, "
		<< "OptionType, ThirdOptionType, Durability, Grade, ItemFlag) VALUES(" 
		<< m_ItemID << ", " 
		<< m_ObjectID << ", " 
		<< getItemType()<< ", '" 
		<< ownerID.c_str() << "', " 
	...
	BEGIN_DB
	{
		...
		*/
		...
		if (migCharID == 0)
		{
			...
			filelog("CharIDMigration.log", "BeltObject: CharID unresolved for [%s], using name",
				pCreature->getName().c_str());

			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, ThirdOptionType, Durability, Grade, EnchantLevel, ItemFlag FROM BeltObject WHERE OwnerID = '%s' AND Storage IN (0, 1, 2, 3, 4, 9)",
				pCreature->getName().c_str() );
		}
		else
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, ThirdOptionType, Durability, Grade, EnchantLevel, ItemFlag FROM BeltObject WHERE OwnerCharID = %u AND Storage IN (0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
#else
				// CharID migration: load by the surrogate key rather than the name.
		...
		if (migCharID == 0)
		{
			...
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Durability, Grade, EnchantLevel, ItemFlag FROM BeltObject WHERE OwnerID = '%s' AND Storage IN (0, 1, 2, 3, 4, 9)",
				pCreature->getName().c_str() );
		}
		else
		{
			pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, Durability, Grade, EnchantLevel, ItemFlag FROM BeltObject WHERE OwnerCharID = %u AND Storage IN (0, 1, 2, 3, 4, 9)",
				(uint)migCharID );
		}
#endif
		...
		while (pResult->next())
		{
			try {
				...
				pBelt->setThirdOptionType(thirdoptionTypes);
#endif
				...
				switch(storage)
				{
					...
					case STORAGE_GEAR:
						if (pCreature->isSlayer())
						{
							...
							{
								pSlayer->putItemToGear(x, y, pBelt);
							}
						}
						else if (pCreature->isVampire())
						{
	...
	throw(Error)
{
	...
	BEGIN_DB
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,"
			<< " OptionType, ThirdOptionType, Durability, EnchantLevel, ItemFlag FROM BeltObject"
