...
void PCFinder::addNPC(NPC *pNPC) throw(DuplicatedException, Error)
{
    ...
    m_NPCs[ Name ] = pNPC;
    m_NPCsByNPCID[pNPC->getNPCID()] = pNPC;
    ...
    throw ()
{
    ...
    hash_map< string , NPC* >::iterator itr = m_NPCs.find(name);
    hash_map< NPCID_t, NPC* >::iterator itrByNPCID;
    ...
    if ( pNPC != NULL )
    {
	    itrByNPCID = m_NPCsByNPCID.find(pNPC->getNPCID());    
	    
	    if ( itrByNPCID == m_NPCsByNPCID.end() )
	    {
	    	m_Mutex.unlock();
    ...
    m_NPCsByNPCID.erase(itrByNPCID);
...
NPC* PCFinder::getNPC(const NPCID_t NPCID) const throw()
{
    __BEGIN_TRY
...
NPC* PCFinder::getNPC_LOCKED(const NPCID_t NPCID) const throw()
{
    __BEGIN_TRY

    hash_map< NPCID_t , NPC* >::const_iterator itr;

    itr = m_NPCsByNPCID.find(NPCID);

    if (itr == m_NPCsByNPCID.end())
    {
        return NULL;
    }
