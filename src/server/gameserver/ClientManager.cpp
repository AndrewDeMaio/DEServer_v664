...
static class TimePeriodBroadcaster : public TimeChecker::TimePeriodCallback
{
public:
	void periodStart( int type, int len )
	{
		//static const char* messages[] =
		//{
		//	"",
		//	"지금부터 %d분간 경험치가 2배로 적용됩니다.",
		//	"지금부터 %d분간 특정 몬스터가 2배 많이 출현합니다.",
		//	"지금부터 %d분간 계급 경험치가 2배로 적용됩니다.",
		//	"지금부터 %d분간 행운이 2배로 적용됩니다.",
		//	"지금부터 %d분간 몬스터를 사냥하면 특정 확률로 특수한 아이템이 드랍됩니다.",
		//	""
		//};

		// 20070814 StringPool에서 얻어온다.
		static const int messageid[] =
		{
			0,
			STRID_EXP_2X_START,
			STRID_MONSTER_2X_START,
			STRID_RANK_EXP_2X_START,
			STRID_LUCK_2X_START,
			STRID_SPECIAL_ITEM_START,
			0
		};

		if ( type >= TIME_PERIOD_MAX || type < 0 ) return;

		//const char* message = messages[type];
		//if ( message == "" ) return;
		const char* message = g_pStringPool->c_str(messageid[type]);
		if ( message == "" ) return;

		static char buffer[255];
//		sprintf(buffer, message, len/60);
		sprintf(buffer, g_pStringPool->c_str(messageid[type]), len/60);		
		cout << buffer << endl;
	...
	void periodEnd( int type )
	{
		//static const string messages[] =
		//{
		//	"",
		//	"경험치가 2배로 적용되는 시간이 종료되었습니다.",
		//	"몬스터가 2배로 출현하는 시간이 종료되었습니다.",
		//	"계급 경험치가 2배로 적용되는 시간이 종료되었습니다.",
		//	"행운이 2배로 적용되는 시간이 종료되었습니다.",
		//	"특수한 아이템이 드랍되는 시간이 종료되었습니다.",
		//	""
		//};
		...
		static const int messageid[] =
		{
			0,
			STRID_EXP_2X_END,
			STRID_MONSTER_2X_END,
			STRID_RANK_EXP_2X_END,
			STRID_LUCK_2X_END,
			STRID_SPECIAL_ITEM_END,
			0
		};
		...
		cout << g_pStringPool->c_str(messageid[type]) << endl;

		GCSystemMessage gcSM;
		gcSM.setMessage(g_pStringPool->c_str(messageid[type]));
		g_pZoneGroupManager->pushBroadcastPacket(&gcSM);
	}
...
/*			if (UserGateway::getInstance()->getTotalInfo() > 0)
		...
		// 돌지 않으므로 여기에 추가
		g_pTimeChecker->heartbeat(theBroadcaster);
	} //while
