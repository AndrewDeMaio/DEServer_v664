...
//				pZoneInfo->setPremiumZone(pResult->getInt(++i)!=0);
				pZoneInfo->setPremiumZoneType(pResult->getInt(++i));		//1223 wlzzi - 프리미엄 존 혜
				...
				if ( pZoneInfo->getZoneID() == 1130 )
				{
					// 미클리즈 레어 이다. 셋팅
					pZoneInfo->setMikllizzLair(true);
				}
				if ( pZoneInfo->getZoneID() == 8001 )
				{
					// 서바이벌 존 활성화
					pZoneInfo->setSurvivalZone(true);
				}

				// 2007 06 23 
				if ( pZoneInfo->getZoneID() >= 5001 && pZoneInfo->getZoneID() <= 5004 )
				{
					pZoneInfo->setHellGardenTower(true);
					// 헬가든 1층 셋팅
				}
				else if ( pZoneInfo->getZoneID() >= 5011 && pZoneInfo->getZoneID() <= 5014 )
				{
					pZoneInfo->setHellGardenTower(true);
					// 헬가든 2층 셋팅
				}
				else if( pZoneInfo->getZoneID() >= 5021 && pZoneInfo->getZoneID() <= 5024 )
				{
					pZoneInfo->setHellGardenTower(true);
					// 헬가든 3층 셋팅
				}
				else if( pZoneInfo->getZoneID() >= 5031 && pZoneInfo->getZoneID() <= 5034 )
				{
					pZoneInfo->setHellGardenTower(true);
					// 헬가든 4층 셋팅
				}
				else if( pZoneInfo->getZoneID() >= 5041 && pZoneInfo->getZoneID() <= 5044 )
				{
					pZoneInfo->setHellGardenTower(true);
					// 헬가든 5층 셋팅
				}
	...
	throw (NoSuchElementException)
{
	...
	if (itr != m_ZoneInfos.end()) 
	{
		...
		if (fitr != m_FullNameMap.end())
		{
			m_FullNameMap.erase(fitr);
		}

		hash_map<string, ZoneInfo*>::iterator sitr = m_ShortNameMap.find(pZoneInfo->getShortName());
		if (sitr != m_ShortNameMap.end())
		{
			m_ShortNameMap.erase(sitr);
		}
