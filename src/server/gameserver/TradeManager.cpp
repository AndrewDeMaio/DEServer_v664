...
// 20080926 멀티팩체크땜시 추가
int TradeInfo::getItemClassNum(Item* pItem)
	throw ()
{
	__BEGIN_TRY
	...
	for (; itr != m_ItemList.end(); itr++)
	{
		if ( (*itr)->getItemClass() == pItem->getItemClass() )
			ItemNum += (*itr)->getNum();
	}
	return ItemNum;
	...
	throw ()
{
	...
	for (ItemMap::iterator itr = itemMap1.begin(); itr != itemMap1.end(); itr++)
	{
		...
		{
			pPlayerCreature2->initAllStatAndSend();
			pPlayerCreature1->initAllStatAndSend();
		}
...
//			pFlagSet->turnOn(FLAGSET_RECEIVE_GREEN_GIFT_BOX);
			pFlagSet->turnOn(FLAGSET_RECEIVE_GREEN_GIFT_BOX_2006_12);
		...
		if ( pItem->getItemType() == Item::ITEM_CLASS_CHECK_MONEY )
		{
			remainMoneyTraceLog( pCreature1->getName(), pCreature2->getName(), ITEM_LOG_TRADE, DETAIL_TRADE, 1000000000 );
		}
...
//			pFlagSet->turnOn(FLAGSET_RECEIVE_GREEN_GIFT_BOX);
	...
	os << "INSERT INTO TradeLog (Timeline, Name1, IP1, Name2, IP2, Content) VALUES ("
	...
	g_GameDBExecuteManager.RequestExecQuery(pCreature1->getObjectID(), os.str());	
