     ...
     throw ()
{
	...
	{
		filelog("EventManagerBug.log", "EventManager::addEvent() : Duplicated event class\n PrevEvent=%s AddingEvent=%s",
			pPrevEvent->toString().c_str(),
     ...
     throw (Error)
{
	...
	while (current != m_Events.end()) 
	{
		...
		if ( pEvent == NULL )
		{
			filelog("EventManagerBug.log", "EventManager::heartbeat() pEvent == NULL");
			
			// 이벤트를 삭제한다.
			m_Events.erase(current);

			if (before == m_Events.end()) 
			{ 
				// first effect
