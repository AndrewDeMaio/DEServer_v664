...
public:
	...
	// 2007 01 30 스텍이 가능한 아이템의 위치를 찾아준다.
	bool findAddStackItem(Item* pItem, TPOINT& pt) throw();
...
public:
	...
	int getItemClassTypeNum( Item::ItemClass itemClass, ItemType_t itemType, bool bSubinventory = false ) throw();
	// 20070910 아이템 클래스의 숫자를 얻어오는 함수를 만들자. 멀티팩 땜시 
	int getItemClassNum( Item::ItemClass itemClass ) throw();

	int decreaseItemClassTypeNum( Item::ItemClass itemClass, ItemType_t itemType, int num, Player* pPlayer, bool bSubInventory=false ) throw();
