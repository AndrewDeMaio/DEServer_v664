...
// renamed from DEScript: the class DEScript in DEScript.h collides with it
namespace DEScriptError
{
	class ScriptException : public Throwable {
	public :
		ScriptException () throw () : Throwable() {}
		ScriptException (const string& msg) throw () : Throwable(msg) {}
		string getName () const throw () { return "ScriptException"; }
	};
	
	class CallException : public ScriptException {
	public :
		CallException () throw () : ScriptException() {}
		CallException (const string& msg) throw () : ScriptException(msg) {}
		string getName () const throw () { return "CallException"; }
	};
};

namespace lua_tinker
{
	// forward declaration: the templates below call push_meta long before its
	// definition further down, which two-phase lookup no longer tolerates.
	...
	RVal call(lua_State* L, const char* name)
	{
		...
		else
		{
			...
			message << "lua_tinker::call() attempt to call global `" << name << "' (not a function)";
