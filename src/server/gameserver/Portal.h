...
class ScriptPortal : public Portal
{

public: // constructor & destructor
	ScriptPortal(ObjectID_t id=0) throw();
	~ScriptPortal() throw() {}

public: // methods from base class
	PortalClass getPortalClass() const throw() { return PORTAL_CLASS_SCRIPT; }
	bool activate(Creature* pCreature) throw(Error);
	virtual string toString() const throw();
	
	void setDungeonID(string& strDungeonID) { m_strDungeonID = strDungeonID; }
	void setDungeonIndex(int iIndex) { m_iDoungeonIndex = iIndex; } 
	
protected :
	string m_strDungeonID;
	int m_iDoungeonIndex;
