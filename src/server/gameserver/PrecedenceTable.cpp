...
double PrecedenceTable::getDamagePercent(const string& Name, int PartyID) const
{
	...
	if ( PartyID != 0 )
	{
		...
		if ( itr != m_PartyMap.end() )
		{
			PrecedenceElement *pPrecedenceElement = itr->second;
						
			if ( pPrecedenceElement != NULL )
			{
				partyDamage = (double)(itr->second->getDamage());
			}
			else
			{
				filelog("ErrorPrecedenceTable.log", "PrecedenceTable::getDamagePercent() pPrecedenceElement == NULL Name=%s, PartyID=%d", 
					Name.c_str(), PartyID
				);
			}
		}
