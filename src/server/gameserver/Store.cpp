...
int Store::getItemNum(Item* pItem) const
{
	vector<StoreItem>::const_iterator itr = m_StoreItems.begin();
	...
	for ( ; itr != m_StoreItems.end(); ++itr )
	{
		if ( itr->isExists() )
		{
			if ( itr->getItem()->getItemType() == pItem->getItemType() && itr->getItem()->getItemClass() == pItem->getItemClass() )
			{
				ItemNum += itr->getItem()->getNum();
			}
		}
	}
...
void Store::setPaintColor( WORD paintColor )
{
	    m_StoreInfo.setPaintColor( paintColor );
}
