	...
	throw()
{
	...
	for(list<Effect*>::const_iterator itr = m_Effects.begin(); itr != m_Effects.end(); itr++) 
	{
		...
		if ( pEffect == NULL )
		{
			filelog("EffectManagerBug.txt", "EffectManager::getEffectInfo() pEffect == NULL Size=%d", getSize());
     ...
     throw (Error)
{
	...
	while (current != m_Effects.end()) 
	{
		...
		// 이펙트가 expire 되었으면 삭제한다.
		if ( pEffect == NULL || currentTime > pEffect->getDeadline()) 
		{
			if (before == m_Effects.end()) 
			{
				// first effect
		...
		{
			// by sigi. for debugging. 2002.12.23 -> rappi76, 위치 이동
