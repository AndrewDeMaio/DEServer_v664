	...
	throw(Error)
{
	...
	switch( gip.ItemStorage )
	{
		case STORAGE_INVENTORY:
		{
			...
			{
				// SubInventory 이다
				SubInventoryItemPosition* pSIP = new SubInventoryItemPosition();
				pSIP->setOwnerName( gip.OwnerID );
				pSIP->setSubInventoryID( gip.StorageID );
				pSIP->setInventoryX( gip.X );
				pSIP->setInventoryY( gip.Y );
				return (GlobalItemPosition*)pSIP;
			}
		}
		break;
