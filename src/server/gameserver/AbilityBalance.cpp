...
HP_t computeTestModeHP(Creature* pCreature, BASIC_ATTR* pAttr, int enhance)
{
	...
	if ( pCreature->isSlayer() )
	{
		...
		switch ( domain )
		{
			...
			case SKILL_DOMAIN_SWORD:
				{
					maxHP = (int)(CSTR*3.00);
//					maxHP += pSlayer->getSkillDomainLevel( domain );
					maxHP += (pSlayer->getSkillDomainLevel( domain ) + pSlayer->getAdvancementClassLevel()) * 5;
//					2007 02 26 수정
//					maxHP += (pSlayer->getSkillDomainLevel( domain ) + pSlayer->getAdvancementClassLevel()) * 4;
				}
				break;
...
//		2007 02 26 수정
//		maxHP = (int)(CSTR*5.00 + CINT + CDEX + CLEVEL);
		// DB attrinfo 테이블에 VAMPIRE_HP_RATIO 수치 확인 바람
		maxHP = getPercentValue( maxHP, g_pVariableManager->getVariable(VAMPIRE_HP_RATIO) );
		maxHP = min((int)maxHP+10, VAMPIRE_MAX_HP);
//		2007 02 26 수정
//		maxHP = min((int)maxHP, VAMPIRE_MAX_HP);
	}
	else if ( pCreature->isOusters() )
	{
		...
		{
//			maxHP = (int)(CSTR*2.50 + CINT + CDEX + CLEVEL );
			maxHP = (int)(CSTR*3.00 + CINT + CDEX + (CLEVEL + pOusters->getAdvancementClassLevel()) );
//			2007 02 26 수정
//			maxHP = (int)(CSTR*5.00 + CINT + CDEX + (CLEVEL + pOusters->getAdvancementClassLevel()) );
		}
		else
		{
//			maxHP = (int)(CSTR*1.50 + CINT / 2  + CDEX + CLEVEL );
			maxHP = (int)(CSTR*2.40 + CINT + CDEX + (CLEVEL + pOusters->getAdvancementClassLevel()));
		}
		maxHP = getPercentValue( maxHP, g_pVariableManager->getVariable(OUSTERS_HP_RATIO) );
//		maxHP = min((int)maxHP, OUSTERS_MAX_HP);
		maxHP = min((int)maxHP+10, OUSTERS_MAX_HP);
//		2007 02 26 수정
//		maxHP = min((int)maxHP, OUSTERS_MAX_HP);

//		cout << (int)CSTR << "," << (int)(CINT) << "," << (int)CDEX << "," << (int)CLEVEL << "," << g_pVariableManager->getVariable(OUSTERS_HP_RATIO) << "," << (int)maxHP << endl;
	}
...
// 20070918
ToHit_t computeMagicToHit(Creature::CreatureClass CClass, BASIC_ATTR* pAttr, int enhance)
{
	Assert(pAttr != NULL);
...
/*
	MagicDefense = CINTE / 2;
	MagicDefense += getPercentValue(CINTE / 2, enhance);
*/
	MagicToHit = CINTE / 3;
...
Defense_t computeMagicDefense(Creature::CreatureClass CClass, BASIC_ATTR* pAttr, int enhance)
{
	Assert(pAttr != NULL);

	double CLEVEL = pAttr->nLevel;

	ToHit_t MagicDefense = CLEVEL;
	
	return min(100, (int)MagicDefense);
}
