	...
	throw (IOException , Error)
{
	...
	for (int i = m_MinFD ; i <= m_MaxFD ; i ++) 
	{
		...
		{
			if (m_pPlayers[i] != NULL && m_pPlayers[i]->getSocket() != NULL) 
			{
			...
			else 
			{
				...
				try 
				{
					...
					if ( g_pConfig->hasKey("IsTestServer") )
					{
					    ...
					    {
					    	Creature* pCreatureEffect = pTempPlayer->getCreature();
					    	
					    	if ( !pCreatureEffect->isFlag( Effect::EFFECT_CLASS_JOYMAX_PREMIUM_SERVICE ) )
					    	{
...
/*								EventKick* pEventKick = new EventKick(pTempPlayer);
				...
				catch (ProtocolException & pe) 
				{
					...
					catch ( DisconnectException& de )
					{
						...
						{
							// 이미 deletePlayer() 를 호출한거 같음
							// 무시하고 로그를 남긴다.
							filelog( "DIFF_ZG.log", "aleady deleted : %d", pTempPlayer->getSocket()->getSOCKET() );
						}
					}
	...
	throw(Error) 
{
	...
	while(! m_PlayerListQueue.empty())
	{
		...
		if (pGamePlayer == NULL)
		{
			filelog("ZoneBug.txt", "%s : %s", "ZonePlayerManager::heartbeat(1)", "pGamePlayer가 NULL입니다.");
			continue;
		}
	...
	throw(Error) 
{
	...
	while(!m_PlayerOutListQueue.empty()) 
	{
		...
		if (pGamePlayer->getPlayerStatus() == GPS_WAITING_FOR_CG_READY )
		{
			...
			Zone* pNewZone = pCreature->getNewZone();
			Assert(pNewZone != NULL);
			
			pNewZone->IncreaseEntranceCount();
		}
