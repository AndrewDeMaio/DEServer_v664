...
enum {
	...
	SKILL_PROPERTY_TYPE_SINGLE_TARGET = 3,
	SKILL_PROPERTY_TYPE_WIDE_TARGET = 4,
...
public:
	enum {
		SKILL_AVOID_PHYSIC,			// 1
		SKILL_AVOID_MAGIC,			// 2
		SKILL_AVOID_ATTR_MAX		// 3
	};
#endif
...
#ifdef VERSION_SKILL_1
	void setAvoidAttribute(uint AvoidAttribute) { m_AvoidAttribute = AvoidAttribute; }
	uint getAvoidAttribute() { return m_AvoidAttribute; }
	bool isMagicAvoidAttribute() { 
		if( m_AvoidAttribute == 1 )
			return true;
		return false;
	}
	bool isPhysicAvoidAttribute() {
		if( m_AvoidAttribute == 0 )
			return true;
		return false;
	}
#endif
