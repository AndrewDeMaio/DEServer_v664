	...
	throw (Error)
{
	...
	try
	{
		pNewZone = getZoneByZoneID(m_ZoneID);
	}
	catch (NoSuchElementException& e)
	{
		filelog("ZoneBug.txt", "EventTransport::activate : No Such Zone Element" );
		filelog("ZoneBug.txt", e.toString().c_str() );
	...
	if ( pNewZone == NULL || !pNewZone->NotifyBeforeEnterZone(pCreature) )
	{
		// 입장 조건 해당 안됨
		return;
	}
	...
	pOldZone->NotifyBeforeLeaveZone(pCreature);
	...
	try 
	{
		...
		pOldZone->NotifyAfterLeaveZone(pCreature);
