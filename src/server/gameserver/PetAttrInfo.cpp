...
//20090601 ksym555
int PetAttrInfoManager::PetRandomAttr()
{
	int value = rand()%100;
	int iOptionClassNumber = 0;

	cout << "펫 조합기 옵션 클래스 결정을 위한 랜덤 값 : " << value << endl;
	...
	for ( ; itr != endItr ; ++itr )
	{
		...
		if ( pPetAttrInfo->getEnchantRatio() > value )
		{
			cout << "피카츄 너로 결정했다!" << endl;
			
			iOptionClassNumber = (int)pPetAttrInfo->getPetAttr();
			
			return iOptionClassNumber;
		}
