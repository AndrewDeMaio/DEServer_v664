...
bool SweeperSetManager::putSweeper( Item* pSweeper, MonsterCorpse* pSafe )
{
	...
	int lostRace = g_pSweeperBonusManager->getSweeperBonus( pSweeper->getItemType() )->getRace();
#endif

	g_pSweeperBonusManager->getSweeperBonus( pSweeper->getItemType() )->setRace( getSafeIndex( pSafe ) );
#ifdef __CONTRIBUTION_SYSTEM__
	int winRace = getSafeIndex( pSafe );
	calcSweeperWinPoint(lostRace, winRace);
#endif
...
//int SweeperSetManager::getSweeperWinPoint(Race_t race)
//{
//	int nSweeperWinPoint = 0;
//	for(int i = 0; i < 3; i++)
//	{
//		int nType = ((m_Level - 1) * 3) + i;
//		if(nType > 12 || nType < 0)
//			cout << "getSweeperWinPoint() m_Level : " << m_Level << ", ItemType : << nType << endl;
//		else
//		{
//			if(race == g_pSweeperBonusManager->getSweeperBonus( ((m_Level - 1) * 3) + i )->getRace() )
//				nSweeperWinPoint++;
//		}
//	}
//	return nSweeperWinPoint;
//}

void SweeperSetManager::resetSweeperWinPoint()
{
	memset(m_SweeperWinPoint, 0, 3 * sizeof(int));
}

void SweeperSetManager::calcSweeperWinPoint(Race_t lostRace, Race_t winRace)
{
	switch(winRace)
	{
	case RACE_SLAYER:
		{
			...
			{
				m_SweeperWinPoint[RACE_SLAYER]++;
				if(m_SweeperWinPoint[RACE_SLAYER] > 3) // 3점 이상은 안준다.
					m_SweeperWinPoint[RACE_SLAYER] = 3;

				m_SweeperWinPoint[lostRace]--;
				if(m_SweeperWinPoint[lostRace] < -3) // -3점 이상은 안뺀다.
					m_SweeperWinPoint[lostRace] = -3;
			}
		}
		break;
	case RACE_VAMPIRE:
		{
			...
			{
				m_SweeperWinPoint[RACE_VAMPIRE]++;
				if(m_SweeperWinPoint[RACE_VAMPIRE] > 3) // 3점 이상은 안준다.
					m_SweeperWinPoint[RACE_VAMPIRE] = 3;
	...
	case RACE_OUSTERS:
		{
			...
			{
				m_SweeperWinPoint[RACE_OUSTERS]++;
				if(m_SweeperWinPoint[RACE_OUSTERS] > 3) // 3점 이상은 안준다.
					m_SweeperWinPoint[RACE_OUSTERS] = 3;
