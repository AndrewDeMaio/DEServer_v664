...
void SiegeManager::init(ZoneID_t zoneID)
{
	...
	MonsterCorpse* pShrine = new MonsterCorpse( 560, "보관대", 2 );
	Assert( pShrine != NULL );
...
/*	if ( !g_pCastleInfoManager->modifyCastleOwner( castleZoneID, pPC ) )
	...
	char buffer[256];
	sprintf( buffer, g_pStringPool->c_str(STRID_VICTORY_ATTACKER_MOVE_REVIVE_POINT), side ); // 20070814
