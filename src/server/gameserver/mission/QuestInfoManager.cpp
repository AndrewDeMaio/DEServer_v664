...
bool QuestInfoManager::giveReward( QuestID_t qID, PlayerCreature* pPC ) const throw(Error)
{
    ...
    void QuestInfoManager::getEventQuestIDs( int questLevel, PlayerCreature* pPC, OutITR outItr ) const throw(Error)
    {
        __BEGIN_TRY
    ...
    // OutITR은 container<QuestID_t>::output_iterator 타입이어야 한다.
    template <class OutITR>
    void QuestInfoManager::getPossibleQuestIDs( PlayerCreature* pPC, OutITR outItr ) const throw(Error)
    {
        __BEGIN_TRY
    ...
    void QuestInfoManager::getMonsterKillQuests( InITR b, InITR e, OutITR t )
    {
		QuestInfoHashMap::const_iterator endItr = m_QuestInfos.end();
		for ( InITR i = b ; i != e ; ++i )
		{
