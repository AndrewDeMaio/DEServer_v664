...
//		pCreature->removeFlag( Effect::EFFECT_CLASS_CAN_ENTER_GDR_LAIR );
		...
		if (pZone != NULL)
		{
			...
			if ( pLocalPartyManager != NULL )
			{
				pLocalPartyManager->deletePartyMember(m_ID, pCreature);
			}
			else
			{
				filelog("ErrorParty.log", "Party::destroyParty(), PartyID=%d, pLocalPartyManager == NULL", m_ID);
			}
		}
	}
...
void Party::TraversePartyMember(TraversePartyMemberCommand *pTraversePartyMemberCommand)
{
	__ENTER_CRITICAL_SECTION(m_Mutex)
	...
	for (; itr != m_MemberMap.end(); itr++)
	{
		...
		pTraversePartyMemberCommand->Execute(pCreature);
	}
	...
	throw()
{
	...
	for (; mitr != m_MemberMap.end(); mitr++)
	{
			...
			)
		{
			...
			{
////////////////////////////////////////////////////////////////////////////////////////////////
//              승직캐릭터의 승직레벨도 포함시켜서 계산한다.
//
	        	        Vampire* pVampire = dynamic_cast<Vampire*>(pCreature);
        	        	//if(pVampire->isAdvanced())
	                	//    LevelSum += pVampire->getLevel() + pVampire->getAdvancementClassLevel();
		                //else
        		            LevelSum += pVampire->getLevel();
	...
	Assert(pLeader->isVampire());
	writeExpLog(dynamic_cast<Vampire*>(pLeader), "PartyExp", amount);
...
//
	                	...
	                	//    LevelSum += pOusters->getLevel() + pOusters->getAdvancementClassLevel();
	                	//else
	...
	Assert(pLeader->isOusters());
	writeExpLog(dynamic_cast<Ousters*>(pLeader), "PartyExp", amount);
	...
	throw()
{
	...
	if ( nMemberSize != 0 ) amount = (int) computeRankExp( LevelSum2 / nMemberSize, otherLevel );
	...
	throw()
{
	...
	for (; mitr != m_MemberMap.end(); mitr++)
	{
		...
		// 쪽이 좋지 않을까? -- 김성민
/////////////////////////////////////////////////////////////////////////////////////////////////////
//      비승직 캐릭터의 레벨도 포함시켜서 계산한다.
//      비승직 캐릭터의 경험치 계산은 하지 않는다.
//      다음 if문에서 pCreature->isAdvanced() 이것이 없어졌다.
   	     if (pCreature->getDistance(cx, cy) <= 8
        	    && !pCreature->isFlag( Effect::EFFECT_CLASS_TRANSFORM_TO_BAT )
	            && !pCreature->isFlag( Effect::EFFECT_CLASS_CASKET )
        	   )
	        {
        	    MemberList.push_back(pCreature);
//////////////////////////////////////////////////////////////////////////////////////////////////////
//          승직 캐릭터는 승직레벨 포함 비승직은 레벨만
//
	            if(pCreature->isAdvanced())
        	        LevelSum += pCreature->getLevel() + pCreature->getAdvancementClassLevel();
	            else
        	        LevelSum += pCreature->getLevel(); // 20070813 본섭 적용
		}
	}
	...
	for (; itr != MemberList.end(); itr++)
	{
		...
		int myQuota = 0;
		if(LevelSum != 0)
			myQuota = amount * pCreature->getLevel() / LevelSum;
		// 승직만 경험치 올려준다.
		if ( pCreature->isAdvanced() )
	...
	throw (Error)
{
	...
	Assert( pSlayer != NULL );
	SlayerSkillSlot* pSkillSlot = pSlayer->getSkill( SKILL_REVEALER );
	...
	for (; litr != MemberList.end(); litr++)
	{
		...
		if (pCreature != pCaster)
		{
			...
			pEffectRevealer->affect( pSlayer );
	...
	throw (Error)
{
	...
	for (; litr != MemberList.end(); litr++)
	{
		...
		if (pCreature != pCaster)
		{
			...
			pEffectDetectHidden->affect( pSlayer );
	...
	throw (Error)
{
	...
	for (; litr != MemberList.end(); litr++)
	{
		...
		if (pCreature != pCaster)
		{
			...
			pEffectDetectInvisibility->affect( pSlayer );
...
void PartyManager::TraverseParty(TraversePartyCommand *pTraversePartyCommand)
{
	Party *pParty = NULL;

	hash_map<int, Party*>::iterator iter;
	
	iter = m_PartyMap.begin();
	
	for( ; iter != m_PartyMap.end() ; ++iter )
	{
		pParty = iter->second;
		...
		pTraversePartyCommand->Execute(pParty);
	}
}
	...
	throw (NoSuchElementException, DuplicatedException, Error)
{
	...
	// 파티원이 추가되었다 폭주 포인트는 쫑난거야
	// 클라이언트는 파티원 추가 패킷보고 알아서 리셋처리하도록 한다.
	// 존락을 넘나드는 작업이므로 뿌리기엔 위험하다.
	pParty->resetPartyBloodBurstPoint();
...
void GlobalPartyManager::broadcastPartyBloodBurstPoint_NOLOCKED( Party* pParty )
{
	Assert( pParty != NULL );

	GCModifyInformation gcMI;
	gcMI.addShortData( MODIFY_PARTY_BLOOD_BURST_POINT, pParty->getPartyBloodBurstPoint() );

	pParty->broadcastPacket( &gcMI );
}

// !!!!! Caution !!!!!
// 모든 파티원이 한 존에 있다는걸 전재로함
void GlobalPartyManager::increasePartyBloodBurstPoint( int ID, Attr_t point )
{
	__ENTER_CRITICAL_SECTION(m_Mutex)
	...
	pParty->increasePartyBloodBurstPoint( point );

	broadcastPartyBloodBurstPoint_NOLOCKED( pParty );
...
// !!!!! Caution !!!!!
// 모든 파티원이 한 존에 있다는걸 전재로함
void GlobalPartyManager::resetPartyBloodBurstPoint( int ID )
{
	__ENTER_CRITICAL_SECTION(m_Mutex)
	...
	throw()
{
	...
	if (PartyID != 0)
	{
		...
		if (pZone != NULL)
		{
			...
			{
				filelog("ErrorParty.log", "deleteAllPartyInfo(), PartyID=%d, pLocalPartyManager == NULL", PartyID);
			}
		}
