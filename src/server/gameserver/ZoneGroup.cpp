	...
	throw (Error)
{
	...
	vector<ZoneID_t> DeleteZoneList;
	vector<ZoneID_t>::iterator DeleteZoneListIter;
	...
	for (hash_map< ZoneID_t , Zone* >::iterator itr = m_Zones.begin() ; itr != m_Zones.end() ; itr ++) 
	{
		...
		if ( pZone != NULL )
		{
			...
			{
				if ( pZone->GetEntranceCount() <= 0 && pZone->getDynamicZone()->GetDeleteFlag() )
				{
					DeleteZoneList.push_back(pZone->getZoneID());
				}
			}
		}
	}

	DeleteZoneListIter = DeleteZoneList.begin();
	for( ; DeleteZoneListIter != DeleteZoneList.end(); ++DeleteZoneListIter)
	{
		deleteZone(*DeleteZoneListIter);
	}

	DeleteZoneList.clear();
	...
	throw (Error)
{
	...
	for (hash_map< ZoneID_t , Zone* >::iterator itr = m_Zones.begin() ; itr != m_Zones.end() ; itr ++) 
	{
		...
		gmServerInfo.addZoneUserData( pZone->getZoneID(), pZone->getPCCount(), pZone->getPCNumByRace() );
	...
	throw (NoSuchElementException)
{
	...
	if (itr != m_Zones.end()) 
	{
		...
		try 
		{
			if ( pZone != NULL && pZone->getZoneID() == zoneID )
			{
				// 존을 삭제한다.
				SAFE_DELETE(pZone);			
			}
			...
			{
				filelog("ErrorDeleteZone.log", "Different ZoneID. ZoneGroupID=%d, RequestZoneID=%d, DeleteZoneID=%d",
					this->getZoneGroupID(),
					zoneID, 
					pZone->getZoneID()
				);
		...
		{
			filelog("ErrorDeleteZone.log", "Throwable Exception : %s\nDifferent ZoneID. ZoneGroupID=%d, RequestZoneID=%d",
				t.toString().c_str(),
				this->getZoneGroupID(),
				zoneID
			);
...
void ZoneGroup::pushBroadcastPacket( Packet* pPacket, BroadcastFilter* pFilter )
	throw(Error)
{
	__BEGIN_TRY

	hash_map<ZoneID_t, Zone*>::const_iterator itr = m_Zones.begin();
	hash_map<ZoneID_t, Zone*>::const_iterator endItr = m_Zones.end();

	for ( ; itr != endItr; ++itr )
	{
		Zone* pZone = itr->second;

		pZone->pushBroadcastPacket( pPacket, pFilter );
	}
