	...
	throw(Error)
{
	...
	BEGIN_DB
	{
		...
		{
			pResult = pStmt->executeQuery(
					"SELECT count(*) FROM CoupleInfo WHERE ( MalePartnerName='%s' AND FemalePartnerName='%s' ) OR ( MalePartnerName='%s' AND FemalePartnerName='%s')",
						pPC1->getName().c_str(), name2.c_str(), name2.c_str(), pPC1->getName().c_str() );
		}
	...
	throw(Error)
{
	...
	// 2006.05.22
	if ( pPC1->getSex() == pPC2->getSex() && !( pPC1->isOusters() && pPC2->isOusters() ) ) return false;
	...
	BEGIN_DB
	{
		...
		else
		{
			pResult = pStmt->executeQuery(
					"SELECT count(*) FROM CoupleInfo WHERE ( MalePartnerName='%s' AND FemalePartnerName='%s' ) OR ( MalePartnerName='%s' AND FemalePartnerName='%s')",
						pPC1->getName().c_str(), pPC2->getName().c_str(), pPC2->getName().c_str(),  pPC1->getName().c_str() );
		}
	...
	throw(Error)
{
	...
	BEGIN_DB
	{
		...
		{
			pResult = pStmt->executeQuery(
					"SELECT count(*) FROM CoupleInfo WHERE MalePartnerName='%s' OR FemalePartnerName='%s'",
						 pPC->getName().c_str(), pPC->getName().c_str() );
		}
	...
	throw(Error)
{
	...
	BEGIN_DB
	{
		...
		{
			pResult = pStmt->executeQuery(
					"SELECT if(MalePartnerName='%s', FemalePartnerName, MalePartnerName) AS WHICH FROM CoupleInfo WHERE MalePartnerName='%s' or FemalePartnerName='%s'",
						pPC->getName().c_str(), pPC->getName().c_str(), pPC->getName().c_str() ); 
		}
	...
	throw(Error)
{
	...
	// 2006.05.22 아우스터가 아니면 체크
	if( !(pPC1->isOusters() && pPC2->isOusters()) )
	...
	BEGIN_DB
	{
		...
		{
			pStmt->executeQuery(
					"INSERT INTO CoupleInfo (MalePartnerName, FemalePartnerName, Race, CoupleDate ) VALUES ('%s', '%s', %u, now())",
						pPC1->getName().c_str(), pPC2->getName().c_str(), (uint)pPC1->getRace() );
		}
	...
	throw(Error)
{
	...
	BEGIN_DB
	{
		...
		{
			pStmt->executeQuery(
					"DELETE FROM CoupleInfo WHERE (MalePartnerName='%s' AND FemalePartnerName='%s') OR (MalePartnerName='%s' AND FemalePartnerName='%s')",
						pPC1->getName().c_str(), pPC2->getName().c_str(), pPC2->getName().c_str(), pPC1->getName().c_str() );
		}
	...
	throw(Error)
{
	...
	BEGIN_DB
	{
		...
		else 
		{
			pStmt->executeQuery( 
					"DELETE FROM CoupleInfo WHERE (MalePartnerName='%s' AND FemalePartnerName='%s') OR (MalePartnerName='%s' AND FemalePartnerName='%s')",
						pPC1->getName().c_str(), strPC2.c_str(), strPC2.c_str(), pPC1->getName().c_str() );
		}
		SAFE_DELETE(pStmt);
	}
	END_DB(pStmt)
...
void CoupleManager::removeCoupleForce( PlayerCreature* pPC1 ) throw(Error)
{
	...
	BEGIN_DB
	{
		...
		{
			pStmt->executeQuery( "DELETE FROM CoupleInfo WHERE MalePartnerName='%s' OR FemalePartnerName='%s'",
						pPC1->getName().c_str(), pPC1->getName().c_str() );
		}
		SAFE_DELETE(pStmt);
	}
	END_DB(pStmt)
