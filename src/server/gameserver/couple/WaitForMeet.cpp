...
static const char* CoupleRingOptions[3][4] =
...
uint WaitForMeet::canMakeCouple( PlayerCreature* pPC1, PlayerCreature* pPC2 )
{
	...
	if ( pPC1->getSex() == pPC2->getSex() && !(pPC1->isOusters() && pPC2->isOusters()) ) return COUPLE_MESSAGE_SAME_SEX;
	...
	throw(Error)
{
	...
	if ( g_pVariableManager->isRestrictionConnectCouple() ) 
	{
		// 무료 사용자에 대한 커플 연결 제약되어 있음
