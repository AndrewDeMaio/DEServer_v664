	...
	throw(Error)
{
    ...
    if (pTargetCreature!=NULL)
    {
		...
		if ( pWaitingPC->getName().c_str() == pTargetPC->getName().c_str() )
		{
			g_pPCFinder->unlock();
			return NPC_RESPONSE_COUPLE_CANNOT_MEET;
		}
    }
	...
	throw(Error)
{
	...
	// 성별이 같으면 성사될 수 없다.
	if ( pWaitingPC->getSex() == pRequestedPC->getSex() && !(pWaitingPC->isOusters() && pRequestedPC->isOusters() )) return COUPLE_MESSAGE_SAME_SEX;
