...
//		for ( Slayer::WearPart i = Slayer::WEAR_FINGER1 ; i <= Slayer::WEAR_FINGER4 ; ++(int)(i) )
		for ( int i = Slayer::WEAR_FINGER1 ; i <= Slayer::WEAR_FINGER4 ; ++i )
		{
			Item* pRing = pSlayer->getWearItem( (Slayer::WearPart)i );
			if ( pRing != NULL )
			{
				if ( isMatchCoupleRing( pPC, pRing ) )
				{
//					pSlayer->deleteWearItem( i );
					pSlayer->takeOffItem( (Slayer::WearPart)i, false, true );
					pRing->destroy();
					SAFE_DELETE( pRing );
...
//		for ( Vampire::WearPart i = Vampire::WEAR_FINGER1 ; i <= Vampire::WEAR_FINGER4 ; ++(int)i )
		for ( int i = Vampire::WEAR_FINGER1 ; i <= Vampire::WEAR_FINGER4 ; ++i )
		{
			Item* pRing = pVampire->getWearItem( (Vampire::WearPart)i );
			if ( pRing != NULL )
			{
				if ( isMatchCoupleRing( pPC, pRing ) )
				{
//					pVampire->deleteWearItem( i );
					pVampire->takeOffItem((Vampire::WearPart)i, false, true );
					pRing->destroy();
					SAFE_DELETE( pRing );
...
//		for( Ousters::WearPart i = Ousters::WEAR_NECKLACE1 ; i <= Ousters::WEAR_NECKLACE3 ; ++(int)i )
		for( int i = Ousters::WEAR_NECKLACE1 ; i <= Ousters::WEAR_NECKLACE3 ; ++i )
		{
			Item* pHarmonicPendent = pOusters->getWearItem( (Ousters::WearPart)i );
			if( pHarmonicPendent != NULL )
			{
				if( isMatchCoupleRing( pPC, pHarmonicPendent) ) 
				{
					pOusters->takeOffItem( (Ousters::WearPart)i, false, true );
					pHarmonicPendent->destroy();
					SAFE_DELETE( pHarmonicPendent );
