...
MonsterAI::MonsterAI() throw()
{
	init();
}

MonsterAI::MonsterAI(Monster* pMonster, string& strAI, string& strDeadAI) 
	throw()
{
	Assert(pMonster != NULL);
	
	m_pBody         = pMonster;
	m_AIType        = INVALID_DIRECTIVETYPE;
	
	m_pDirectiveSet = new DirectiveSet;
	
	g_pDirectiveSetManager->createDirectiveSet(m_pDirectiveSet, strAI.c_str(), strDeadAI.c_str());
	...
	throw()
{
	...
	if ( m_AIType == INVALID_DIRECTIVETYPE )
	{
		delete m_pDirectiveSet;
	}
...
void MonsterAI::init()
{
	...
	m_pCondChecker[DIRECTIVE_COND_ENEMY_NOT_FEEBLE_VIRUS]		= checkEnemyNotFeebleVirus;
}
...
bool MonsterAI::approachFast(Creature* pEnemy) throw()
{
	Zone *pZone   = m_pBody->getZone();
	ZoneCoord_t enemyX  = pEnemy->getX();
	ZoneCoord_t enemyY  = pEnemy->getY();
		
	if ( !pZone->moveFastMonster( m_pBody, m_pBody->getX(), m_pBody->getY(), enemyX, enemyY, SKILL_RAPID_GLIDING ) )
	{
		return approach(pEnemy);
	}
	...
	throw()
{
	...
	if ( !m_pBody->getZone()->NotifyBeforeUseMonsterSkill(m_pBody, SkillType) )
	{
	...
	switch (SkillType)
	{
		...
		case SKILL_PARASITE:
			...
			if (m_pBody->isMaster()
				...
				|| m_pBody->getMonsterType() == 814
				|| m_pBody->getMonsterType() == 815
				|| m_pBody->getMonsterType() == 816
				|| m_pBody->getMonsterType() == 817
		...
		// 2006.06.26 미클리즈 스킬
		case SKILL_SOUND_WAVE:
		case SKILL_BRANDISH:
		case SKILL_FIRE_WAVE:
		case SKILL_BLOODY_BURST:
		case SKILL_TRACE_CREATURE:
		...
		case SKILL_SUMMON_MONSTERS:
			...
			// 2006.07.20 TargetSummonMonsters Test Code
			// 2006.08.02 실질적으로 적용 
			if ( m_pBody->getMonsterType() == 808 )
			{
				Zone* pZone = m_pBody->getZone();
				if( !pZone->isMikllizzLair() )
					break;
				MikllizzLairManager* pMikllizzLaireManager = pZone->getMikllizzLairManager();
				if( !pMikllizzLaireManager->isSummonTiming() )
				{
					delay.tv_sec = 50;
					delay.tv_usec = 500000;
					break;
				}
				pSkillHandler = g_pSkillHandlerManager->getSkillHandler(SKILL_TARGET_SUMMON_MONSTERS);
				Assert( pSkillHandler != NULL );
				pSkillHandler->execute(m_pBody);
			}
			else	
		...
		// Menegroth Doungeon Monser Skill
		case SKILL_ATTACK_CERBERUS :
		case SKILL_ATTACK_MANTICORET :   
		case SKILL_ATTACK_BOGLETH :     
		case SKILL_ATTACK_BOGLETB :    
		case SKILL_ATTACK_MASSACRE : 
		case SKILL_ATTACK_PLUMPYBOAR :
		case SKILL_ATTACK_CABRACAM :
			pSkillInfo = g_pSkillInfoManager->getSkillInfo(SkillType);
			Assert(pSkillInfo != NULL);
		...
		// 아이센 4F 몬스터 스킬들
		case SKILL_VILLAIN_LADY_SELF_DESTRUCTION :
			pSkillInfo = g_pSkillInfoManager->getSkillInfo(SkillType);
			Assert(pSkillInfo != NULL);
		...
		case SKILL_AGONY_CHILD_DROP_HEAD :
		case SKILL_AGONY_CHILD_SHADOW_FOUR :
		case SKILL_VILLAIN_LADY_GHOST_SHADOW :
		case SKILL_ONI_SPANIEL_THROWING_AXE :
		case SKILL_BLACKASTRAL_MAGIC_CRYSTAL :
	...
	m_pBody->getZone()->NotifyAfterUseMonsterSkill(m_pBody, SkillType);
	...
	for (; itr != directiveList.end(); itr++)
	{
		...
		if (checkDirective(pDirective, pEnemy))
		{
			switch (pDirective->getAction())
			{
				...
				case DIRECTIVE_ACTION_APPROACH_FAST :
					__BEGIN_PROFILE_MONSTER("DIRECTIVE_ACTION_APPROACH_FAST");
					approachFast(pEnemy);
					__END_PROFILE_MONSTER("DIRECTIVE_ACTION_APPROACH_FAST");
					break;
	...
	throw()
{
	...
	|| m_pBody->isFlag( Effect::EFFECT_CLASS_NATURAL_PEACE ) 
	|| m_pBody->isFlag( Effect::EFFECT_CLASS_ABERRATION ) 
	)
	{
...
bool checkEnemyNotFeebleVirus(Monster* pMonster, Creature* pEnemy)
{
	Assert(pMonster != NULL);

	if (pEnemy == NULL) return false;
	if (!pEnemy->isFlag(Effect::EFFECT_CLASS_FEEBLE_VIRUS)) return true;
	return false;
}
