	...
	throw (Error)
{
	...
	if ( count[0] < 5 || count[1] < 5 || count[2] < 5 )
	{
		...
		gcSM.setMessage( g_pStringPool->c_str(STRID_NEED_15_QUEST_ITEM) ); // 20070814
	...
	{
		GCSystemMessage gcSM;
		gcSM.setMessage( g_pStringPool->c_str(STRID_NOT_ENOUGH_INVENTORY_SPACE) ); // 20070814
		pPC->getPlayer()->sendPacket( &gcSM );
		return;
	}
	...
	filelog("GQuestEvent.log", "이벤트 퀘스트 아이템을 줬습니다. : [%s]%u/%s/%u", pPC->getName().c_str(), iType, option.c_str(), pItem->getItemID());
	...
	GCSystemMessage gcSM;
	gcSM.setMessage( g_pStringPool->c_str(STRID_GIVE_REWARD_ITEM) ); // 20070814
	pPC->getPlayer()->sendPacket( &gcSM );
