	...
	throw (Error)
{
	...
	if ( pCreature2->getLevel() < 130 )
	{
		gcNPCResponse.setCode( NPC_RESPONSE_NOT_ENOUGH_LEVEL );
		pPC->getPlayer()->sendPacket( &gcNPCResponse );
		return;
	}

	if ( pGuild->getActiveMemberCountFromDB() < 20 )
	{
		gcNPCResponse.setCode( NPC_RESPONSE_NOT_ENOUGH_GUILD_MEMBER );
		pPC->getPlayer()->sendPacket( &gcNPCResponse );
		return;
	}

	if ( pGuild->getAverageLevelOfMember() < 60 )
	{
		gcNPCResponse.setCode( NPC_RESPONSE_NOT_ENOUGH_GUILD_LEVEL );
		pPC->getPlayer()->sendPacket( &gcNPCResponse );
		return;
	}
