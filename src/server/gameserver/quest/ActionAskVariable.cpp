...
//						pParam->setValue("지하수로");
		...
		else if ( keyword == "RacePetQuestTarget" )
		{
			...
			if ( level < 40 )
			{
				...
				gcsm.setMessage(g_pStringPool->c_str(STRID_LOW_LEVEL_PET_QUEST)); // 20070814
...
//			}
		...
		else if ( keyword == "GambleAccumulation" )
		{
			Race_t CreatureRace = pCreature2->getRace();
			char	accumulation[15];

			if( CreatureRace == RACE_SLAYER )
			{
				Slayer* pSlayer = dynamic_cast<Slayer*>(pCreature2);
				sprintf(accumulation, "%d", pSlayer->getGambleAccumulation() );
			}
			else if( CreatureRace == RACE_VAMPIRE )
			{
				Vampire* pVampire = dynamic_cast<Vampire*>(pCreature2);
				sprintf(accumulation, "%d", pVampire->getGambleAccumulation() );
			}
			else if( CreatureRace == RACE_OUSTERS )
			{
				Ousters* pOusters = dynamic_cast<Ousters*>(pCreature2);
				sprintf(accumulation, "%d", pOusters->getGambleAccumulation() );
			}
			string strAccumulation;
			strAccumulation = accumulation;
			pParam->setValue( strAccumulation );
		}
		else if ( keyword == "GambleTryCount" )
		{
			Race_t CreatureRace = pCreature2->getRace();
			char count[15];
			...
			{
				Slayer* pSlayer = dynamic_cast<Slayer*>(pCreature2);
				sprintf(count, "%d", pSlayer->getGambleTryCount());
			}
			else if( CreatureRace == RACE_VAMPIRE )
			{
				Vampire* pVampire = dynamic_cast<Vampire*>(pCreature2);
				sprintf(count, "%d", pVampire->getGambleTryCount());
			}
			else if( CreatureRace == RACE_OUSTERS )
			{
				Ousters* pOusters = dynamic_cast<Ousters*>(pCreature2);
				sprintf(count, "%d", pOusters->getGambleTryCount());
			}
			string strCount;
			strCount = count;
			pParam->setValue( strCount );
		}
		else if ( keyword == "GambleMoney" )
		{
			Gold_t	defaultMoney = 10000;
			BYTE	count = 0;
			Race_t	CreatureRace = pCreature2->getRace();
			...
			defaultMoney = (Gold_t)(pow(1.5, count) * defaultMoney);
			char price[15];
			sprintf(price, "%d", defaultMoney);
			string strPrice = price;
			pParam->setValue(strPrice);
		}
