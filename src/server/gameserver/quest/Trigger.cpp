	...
	throw (Error)
{
	...
	try
	{
		...
		while (true)
		{
			...
			// read Action-Type
			//
			// Some action types in the trigger data are deliberately not registered
			// in this build (e.g. GiveTestServerReward, which needs the Lua script
			// subsystem v664 does not compile). An unregistered action threw, and
			// Zone::load()'s catch-all turned that into an Assert(false) that killed
			// the server. Skip the unknown action, keep the rest of the trigger.
			ActionType_t actionType;
			try
			{
				actionType = g_pActionFactoryManager->getActionType(ActionTypeString);
			}
			catch ( Throwable & t )
			{
				filelog( "UnknownActionType.log", "skipping unregistered action [%s] : %s",
					ActionTypeString.c_str(), t.toString().c_str() );
				continue;
			}
