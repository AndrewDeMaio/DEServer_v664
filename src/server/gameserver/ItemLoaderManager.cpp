	...
	throw (Error)
{
	...
	m_pItemLoaders[Item::ITEM_CLASS_COMMON_QUEST_ITEM] = g_pCommonQuestItemLoader = new CommonQuestItemLoader();
	m_pItemLoaders[Item::ITEM_CLASS_ETHEREAL_CHAIN] = g_pEtherealChainLoader = new EtherealChainLoader();
	m_pItemLoaders[Item::ITEM_CLASS_OUSTERS_HARMONIC_PENDENT] = g_pOustersHarmonicPendentLoader = new OustersHarmonicPendentLoader();
	m_pItemLoaders[Item::ITEM_CLASS_CHECK_MONEY] = g_pCheckMoneyLoader = new CheckMoneyLoader();
	m_pItemLoaders[Item::ITEM_CLASS_CUE_OF_ADAM] = g_pCueOfAdamLoader = new CueOfAdamLoader();
	m_pItemLoaders[Item::ITEM_CLASS_CONTRACT_OF_BLOOD] = g_pContractOfBloodLoader = new ContractOfBloodLoader();
	//20070821
	m_pItemLoaders[Item::ITEM_CLASS_SKILL_BOOK] = g_pSkillBookLoader= new SkillBookLoader();

	m_pItemLoaders[Item::ITEM_CLASS_VAMPIRE_WING_ITEM] = g_pVampireWingItemLoader = new VampireWingItemLoader();
	m_pItemLoaders[Item::ITEM_CLASS_OUSTERS_WING_ITEM] = g_pOustersWingItemLoader = new OustersWingItemLoader();
	//20080604
	m_pItemLoaders[Item::ITEM_CLASS_SLAYER_TUNNING_ITEM] = g_pSlayerTunningItemLoader = new SlayerTunningItemLoader();
	m_pItemLoaders[Item::ITEM_CLASS_VAMPIRE_TUNNING_ITEM] = g_pVampireTunningItemLoader = new VampireTunningItemLoader();
	m_pItemLoaders[Item::ITEM_CLASS_OUSTERS_TUNNING_ITEM] = g_pOustersTunningItemLoader = new OustersTunningItemLoader();
	
	m_pItemLoaders[Item::ITEM_CLASS_CALLNPC_CARD] = g_pCallNPCCardLoader = new CallNPCCardLoader();
	...
	throw (Error)
{
	...
	g_pCommonQuestItemLoader->load(pSlayer);
	g_pEtherealChainLoader->load(pSlayer);
	g_pCheckMoneyLoader->load(pSlayer);
	g_pCueOfAdamLoader->load(pSlayer);
	g_pContractOfBloodLoader->load(pSlayer);
	//20070821
	g_pSkillBookLoader->load(pSlayer);
	//20080604
	g_pSlayerTunningItemLoader->load(pSlayer);
	
	g_pCallNPCCardLoader->load(pSlayer);
	...
	throw (Error)
{
	...
	g_pCommonQuestItemLoader->load(pVampire);
	g_pEtherealChainLoader->load(pVampire);
	g_pCheckMoneyLoader->load(pVampire);
	g_pCueOfAdamLoader->load(pVampire);
	g_pContractOfBloodLoader->load(pVampire);
	//20070821
	g_pSkillBookLoader->load(pVampire);

	g_pVampireWingItemLoader->load(pVampire);
	//20080604
	g_pVampireTunningItemLoader->load(pVampire);
	
	g_pCallNPCCardLoader->load(pVampire);
	...
	throw (Error)
{
	...
	g_pCommonQuestItemLoader->load(pOusters);
	g_pEtherealChainLoader->load(pOusters);
	g_pOustersHarmonicPendentLoader->load(pOusters);
	g_pCheckMoneyLoader->load(pOusters);
	g_pCueOfAdamLoader->load(pOusters);
	g_pContractOfBloodLoader->load(pOusters);
	//20070821
	g_pSkillBookLoader->load(pOusters);

	g_pOustersWingItemLoader->load(pOusters);
	//20080604
	g_pOustersTunningItemLoader->load(pOusters);

	g_pCallNPCCardLoader->load(pOusters);
	...
	throw (Error)
{
	...
	g_pMotorcycleLoader->load(pZone);
