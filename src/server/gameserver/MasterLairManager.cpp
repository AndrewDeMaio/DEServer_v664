...
{	
	...
	if (pCreature->isFlag( Effect::EFFECT_CLASS_MASTER_LAIR_PASS ))
	{
		if (g_pVariableManager->isRetryMasterLair())
		{
			...
			{
				//cout << "[" << (int)m_pZone->getZoneID() << "] MasterLairManager: " << pCreature->getName().c_str() << " + EffectPass" << endl;
				m_Mutex.unlock();
...
//		case EVENT_WAITING_HIDDEN_READY :
//			processEventWaitingHiddenReady();
//		break;
...
//
////////////////////////////////////////////////////////////////////////////////
DynamicZoneScript *MasterLairManager::CreateHiddenStage()
{
	DynamicZoneScript *pDynamicZoneScript = NULL;
	
	string strScriptName = "";
	int iDoungeonIndex = 0;
					
	switch(m_pZone->getZoneID())
	{
		case 1104 :
			strScriptName = "BathoryHidden";
			iDoungeonIndex = 0;
			break;
		case 1106 :
			strScriptName = "BathoryHidden";
	...
	if ( !strScriptName.empty() )
	{
		pDynamicZoneScript = InstanceDoungeonManager::CreateInstanceDoungeon(strScriptName.c_str(), iDoungeonIndex);
	...
	throw (Error)
{
	...
	if (m_bOpenHiddenStage == false && currentTime >= m_HiddenReadyTime)
	{
		m_bOpenHiddenStage = true;
			...
			
			{
				...
				for (itr=creatures.begin(); itr!=creatures.end(); itr++)
				{
					...
					if (pCreature->isPC())
					{
						...
						bool bLevelCondition;
						bool bItemCondition;
						
						bLevelCondition = pPC->getLevel() > 100;
						bItemCondition = pInventory->hasEnoughNumItem(Item::ITEM_CLASS_COMMON_QUEST_ITEM, 5, 20);	// 손상된 가죽 20개
						...
						if ( bItemCondition )
						{
							switch(m_pZone->getZoneID())
							{
								case 1104 :	// 바토리 레어
								case 1106 :
									bItemCondition = bItemCondition && pInventory->hasEnoughNumItem(Item::ITEM_CLASS_QUEST_ITEM, 1, 1);	// 바토리 펜덴트 1개		
									break;
								case 1114 :	// 테페즈 레어
								case 1115 :
									bItemCondition = bItemCondition && pInventory->hasEnoughNumItem(Item::ITEM_CLASS_QUEST_ITEM, 3, 1);	// 테페즈 펜덴트 1개
									break;
								default :
									bItemCondition = false;
									break;
							}
						}
						
						if ( bLevelCondition && bItemCondition )
						{
							GamePlayer *pGamePlayer = dynamic_cast<GamePlayer *>(pCreature->getPlayer());
							
							GCAddEffectToScreen gcAddEffectToScreen;
										
							gcAddEffectToScreen.setEffectID(1);		// 화면 어둡게 하기
							gcAddEffectToScreen.setDuration(10 * 15);
							
							pGamePlayer->sendPacket(&gcAddEffectToScreen);
							
							if ( pDynamicZoneScript == NULL )
							{
								pDynamicZoneScript = CreateHiddenStage();
							}
							...
							if ( pDynamicZoneScript != NULL )
							{
								...
								pEventTransport->setDeadline(10 * 10);
	...
	throw (Error)
{
	...
	// 히든 스테이지 갈때까지 대기 시간
	getCurrentTime( m_HiddenReadyTime );	
	m_HiddenReadyTime.tv_sec += 20;
	m_bOpenHiddenStage = false;
