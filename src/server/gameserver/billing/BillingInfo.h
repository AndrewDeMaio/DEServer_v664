...
// 20070824 PCROOM 체크용 MSSQL 접속 모듈
#define __NETMARBLE_PCROOM_BILLING__  
#endif
...
struct _AUTH_GAME {
	...
	_AUTH_GAME()
	{
		Packet_Type = 0;
		Result = 0;
		memset(S_KEY, 0, sizeof(char)*32);
		memset(Session, 0, sizeof(char)*32);
		memset(User_CC, 0, sizeof(char)*4);
		memset(User_No, 0, sizeof(char)*4);
		memset(User_ID, 0, sizeof(char)*40);
		memset(User_IP, 0, sizeof(char)*24);
		User_Gender = 0;
		memset(User_Status, 0, sizeof(char)*3);
		memset(User_PayType, 0, sizeof(char)*2);
		User_Age = 0;
		Game_No = 0;
		memset(Bill_PayType, 0, sizeof(char)*2);
		memset(Bill_Method, 0, sizeof(char)*2);
		memset(Expire_Date, 0, sizeof(char)*12);
		Remain_Time = 0;
	}
