...
#ifdef __NETMARBLE_PCROOM_BILLING__
...
void BillingPlayerManager::init() 
	throw(Error)
{
#ifdef __NETMARBLE_PCROOM_BILLING__	

	g_pMSSqlEngine = new MSSqlEngine();

	if(g_pMSSqlEngine->InitDBEngine("PCRoomDB.log") == 0)
	{
		cout << "MSSQL InitDBEngine Failed" << endl;
		throw Error("InitDBEngine Failed");
		return;
	}
		
	if(g_pMSSqlEngine->ConnectDB() == 0)
	{
		cout << "MSSQL ConnectDB Failed" << endl;
		throw Error("ConnectDB Failed");
		return;
	}
#endif
...
#ifdef __NETMARBLE_PCROOM_BILLING__
				g_pMSSqlEngine->QuerySQL("SELECT 1");
#endif
