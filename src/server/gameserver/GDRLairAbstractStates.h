...
public:
	...
	MonsterSummonState( Zone* pZone, DWORD clearState, DWORD failState, DWORD timeoutState, Turn_t timeLimit ) :
		TimerState( timeoutState, timeLimit ), m_pZone( pZone ), m_ClearState( clearState ), m_FailState( failState ), m_TimeLimit( true ), m_bStartSummon(false) 
		{ 
			getCurrentTime(m_previousTime);
		}
	MonsterSummonState( Zone* pZone, DWORD clearState, DWORD failState ) :
		TimerState( clearState, 999999999 ), m_pZone(pZone), m_ClearState( clearState ), m_FailState( failState ), m_TimeLimit(false), m_bStartSummon(false) 
		{
			getCurrentTime(m_previousTime);
		}
...
protected:
	...
	bool IsCurrentSummonMonster(MonsterType_t MType);
};
