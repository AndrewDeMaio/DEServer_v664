...
#include "skill/EffectInvisibility.h"
	...
	throw (Error)
{
	...
	if ( m_OwnerObjectID != 0 )
	{
		...
		if ( pOwner != NULL )
		{
			if ( pOwner->getCreatureClass() == pEnemy->getCreatureClass() )
			{
				// 동일 종족
				return false;
			}
			
			if ( pOwner->getPartyID() != 0 && pOwner->getPartyID() == pEnemy->getPartyID() )
			{
				// 동일 파티
				return false;
			}
			
			if ( pOwner->getClanType() != 0 && 
				pEnemy->getClanType() != 0 && 
				pOwner->getClanType() == pEnemy->getClanType() )
			{
				// 동일 길드
				return false;
			}
			
			if ( !canAttack( pOwner, pEnemy ) )
			{
				return false;
			}
...
Monster::Monster (MonsterType_t monsterType)
	throw () : Creature(0, NULL), m_MonsterType(monsterType)
{
	...
	Init(monsterType, m_pMonsterInfo); 

	__END_CATCH
}
	
Monster::Monster (MonsterType_t monsterType, string& strEnhance)
	throw () : Creature(0, NULL), m_MonsterType(monsterType)
{

	m_MonsterType = monsterType;
	
	MonsterInfo *pMonsterInfo = NULL;
	
	pMonsterInfo = (MonsterInfo *)g_pMonsterInfoManager->getMonsterInfo(monsterType);
	if ( pMonsterInfo != NULL )
	{
		m_pMonsterInfo = pMonsterInfo->Clone();
		m_pMonsterInfo->parseEnhanceAttr(strEnhance.c_str());
	}
...
void Monster::Init(MonsterType_t monsterType, MonsterInfo *pMonsterInfo)
{
	...
	m_Exp           = pMonsterInfo->getExp() + pMonsterInfo->getEnhanceExp();
	m_EnhanceHP		= pMonsterInfo->getEnhanceHP(); // 20080502 도메인 경험치 보너스 추가 땜시 
	m_Moral         = pMonsterInfo->getMoral();
	m_Delay         = pMonsterInfo->getDelay() + pMonsterInfo->getEnhanceDelay();
	m_AttackDelay   = pMonsterInfo->getAttackDelay() + pMonsterInfo->getEnhanceADelay();
	m_Sight         = pMonsterInfo->getSight() + pMonsterInfo->getEnhanceSight();
	...
	if ( pMonsterInfo->getEnhanceSight() < 0 && ( pMonsterInfo->getSight() < (-1) * pMonsterInfo->getEnhanceSight() ) )
	{
		// EnhanceSight 값이 음수이고, 원래 설정보다 큰경우
	...
	m_HP[ATTR_MAX]         = computeTestModeHP(this, &attr, pMonsterInfo->getEnhanceHP());
	...
	m_DecreaseDamagePercent= min( 100 ,(int)pMonsterInfo->getEnhanceDecreaseDamage() );
	m_MonsterNextRatio	   = max( 0   ,(int)pMonsterInfo->getEnhanceNextRatio());
	...
	switch (pMonsterInfo->selectRegenType())
	{
		case REGENTYPE_HIDE :
			...
			getViewable()->addViewEffect( Effect::EFFECT_CLASS_HIDE, 0 );
		break;
		...
		case REGENTYPE_INVISIBLE :
		{
			...
			EffectInvisibility* pEffect = new EffectInvisibility(this);
			pEffect->setDeadline(99999999); // 무한이다.
			pEffect->setLevel( getLevel() );
			pEffect->setBroadcastingEffect(false);
			addEffect(pEffect);

			getViewable()->addViewEffect( Effect::EFFECT_CLASS_INVISIBILITY, getLevel() );
		}
...
//	올저항 관련 셋팅...			
	...
	// 2006.06.15 미클리스 추가 움직이지 않으므로 오레와 같이 고정으로 움직이지 못하게 한다.
...
Packet* Monster::getAddPacket() const
{
	m_pAddPacket->setObjectID(getObjectID());
	m_pAddPacket->setMonsterType(getMonsterType());
	m_pAddPacket->setMonsterName(getName());
	m_pAddPacket->setX(getX());
	m_pAddPacket->setY(getY());
	m_pAddPacket->setDir(getDir());
	m_pAddPacket->setEffectInfo(getEffectInfo());
	m_pAddPacket->setCurrentHP(getHP());
	m_pAddPacket->setMaxHP(getHP(ATTR_MAX)); 
			...
			case ATTACK_PC :
				{
					...
					if (itr == m_Enemies.end()) 
					{
						if ( pCreature->isPC() )
						{
							itr = find_if (m_Enemies.begin() , m_Enemies.end() , IsMonster(m_pZone) );
		
							if (itr == m_Enemies.end())
								m_Enemies.push_back( pCreature->getObjectID() );
							else
								m_Enemies.insert(itr, pCreature->getObjectID());
						}
						else
						{
							m_Enemies.push_back( pCreature->getObjectID() );
...
{
	int total = computeMonsterExp(this);
...
bool Monster::isEnemyToAttack(Creature* pCreature) const
{
	...
	if ( m_OwnerObjectID != 0 )
	{
		...
		if ( pOwner != NULL)
		{
			if ( pOwner->getCreatureClass() == pCreature->getCreatureClass() )
			{
				// 동일 종족
				return false;
			}
			
			if ( pOwner->getPartyID() != 0 && pOwner->getPartyID() == pCreature->getPartyID() )
			{
				// 동일 파티
				return false;
			}
			...
			if ( pOwner->getClanType() != 0 && 
				pCreature->getClanType() != 0 && 
				pOwner->getClanType() == pCreature->getClanType() )
			{
				// 동일 길드
				return false;
			}
			
			if ( !canAttack( pOwner, pCreature ) )
			{
				return false;
			}
