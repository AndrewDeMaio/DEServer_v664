	...
	throw()
{
	...
	try
	{
		...
		if (pInfo == NULL)
		{
			// Unknown option type: its *Info table has no rows (feature with no
			// content configured). Neutralise the entry rather than aborting the
			// whole server start -- ratio 0 means it can never be selected.
			m_Ratio = 0;
		}
		else
		{
	...
	catch (NoSuchElementException & nsee)
	{
		...
		m_Ratio = 0;   // unknown option -> never selectable
	}
	...
	throw()
{
	...
	for (size_t i=0; i<m_TreasureOptionTypes.size(); i++)
	{
	...
	throw()
{
	...
	if (!g_pItemInfoManager->isPossibleItem((Item::ItemClass)itemClass, m_ItemType, optionNULL))
	{
		...
		cerr << msg.toString();
		// The .bin data files reference item types whose *Info table has no rows
		// (a feature with no content configured). Dropping the zone's whole
		// treasure set over one unknown type is worse than ignoring the entry:
		// warn and give it ratio 0. Selection uses a fixed modulus, so a
		// zero-ratio entry can never be chosen.
		m_Ratio = 0;
	}
	...
	throw()
{
	...
	for (size_t i=0; i<m_TreasureItemTypes.size(); i++)
	{
	...
	throw()
{
	...
	for (size_t i=0; i<m_TreasureItemClasses.size(); i++)
	{
	...
	throw()
{
	...
	file.read((char*)&m_PremiumItemRatio, sizeof(int));
	file.read((char*)&m_PremiumOptionRatio, sizeof(int));
	file.read((char*)&ItemClassCount, sizeof(int));

	if ( m_ItemRatio == 999999 )
	{
		// 레어마스터 용
		m_ItemRatio = 999999;
		m_OptionRatio = 999999;
		m_PremiumItemRatio = 999999;
		m_PremiumOptionRatio = 999999;
	}
	else
	{
		m_ItemRatio = 40000;
		m_OptionRatio = 40000;
		m_PremiumItemRatio = 40000;
		m_PremiumOptionRatio = 40000;
	}
...
bool Treasure::getRandomItem(ITEM_TEMPLATE* pTemplate, bool bPremium)
	throw()
{
	__BEGIN_TRY
	...
	// 아이템이 나올 확률
	int ThisItemRatio = ( bPremium ? m_PremiumItemRatio : m_ItemRatio );
	...
	if (ItemRatio < ThisItemRatio * EventMultiplier)
	{
...
bool Treasure::getRandomItem(ITEM_TEMPLATE* pTemplate, int nPercent, bool bPremium)
	throw()
{
	__BEGIN_TRY
	...
	// 아이템 확률에 수정치 대입
	int ModifyItemRatio = getPercentValue( ThisItemRatio, nPercent );
	...
	throw()
{
	...
	msg << "\tTreasure\n\t("
		...
		<< "\n\t\tPremiumItemRatio:" << m_PremiumItemRatio
		<< "\n\t\tPremiumOptionRatio:" << m_PremiumOptionRatio
		<< "\n";
