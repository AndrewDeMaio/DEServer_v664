...
public:
	...
	Monster (MonsterType_t monsterType, string& strEnhance) throw ();
...
public:
	...
	// 2007 06 21 add
	BYTE getDecreaseDamagePercent() const throw() { return m_DecreaseDamagePercent; }
	void setDecreaseDamagePercent(BYTE percent) throw() { m_DecreaseDamagePercent = percent; }
...
public:
	...
	void setChiefMonsterIndex(int index)	{ m_ChiefMonsterIndex = index; }
	int  getChiefMonsterIndex()	const		{ return m_ChiefMonsterIndex; }
	bool isChiefMonster() const				{ return m_ChiefMonsterIndex<0xFFFF; }	// -_-;
	...
	int getMonsterNextRatio() const { return m_MonsterNextRatio; }

protected :
	void Init(MonsterType_t monsterType, MonsterInfo *pMonsterInfo);
...
private:
	...
	Exp_t m_Exp;
	int		m_EnhanceHP; // 20080502 
public:
	// 추가적인 경험치 보너스를 주는 것으로 코딩...
	Exp_t getMonsterAddExp(void) { return m_Exp; }
	int	getEnhanceHP(void) { return m_EnhanceHP; }
private:
...
//#endif
	...
	GCAddMonster* m_pAddPacket;
	
	MonsterInfo *m_pMonsterInfo;
	
	int m_MonsterNextRatio;
};
