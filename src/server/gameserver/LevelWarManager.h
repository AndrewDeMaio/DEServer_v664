...
#ifdef __CONTRIBUTION_SYSTEM__
	void setGiveContributeTime(const VSDateTime &dt) throw(Error);
	int getSweeperWinPoint(Race_t race); /// { return m_SweeperSetManager.getSweeperWinPoint(race); }
	void warpToDefaultPosition(Creature* pCreature) { m_LevelWarItemManager.warpToDefaultPosition(pCreature); }
	int  getLevel() { return m_Level; }
	int expBonusTimeCheck(Race_t race) throw(Error);
#endif
private:
	int		m_Level;
	Zone*	m_pZone;
...
#ifdef __CONTRIBUTION_SYSTEM__
	VSDateTime m_GiveContributePointTime;
	LevelWarItemManager m_LevelWarItemManager;
#endif
};
