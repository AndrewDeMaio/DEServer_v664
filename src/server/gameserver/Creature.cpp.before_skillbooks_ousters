...
: Object(objectID)
{
	...
	m_pViewable = new Viewable;
	m_pViewAbility = new ViewAbility;

	// 기본적으로 모든 Effect가 붙을수 있도록 한다.
	setCanAddAllEffectFlag();
    ...
    throw ()
{
	...
	SAFE_DELETE( m_pViewable );
	SAFE_DELETE( m_pViewAbility );
	...
	throw (Error)
{
	...
	//if ( m_CanAddEffectFlag.test(pEffect->getEffectClass()) )
	{
		m_pEffectManager->addEffect(pEffect);
	}
...
bool Creature::hasRelicItem() const
{
	return m_Flag.test(Effect::EFFECT_CLASS_HAS_SLAYER_RELIC)
			...
			|| m_Flag.test(Effect::EFFECT_CLASS_SLAYER_HERO_ITEM)
			|| m_Flag.test(Effect::EFFECT_CLASS_VAMPIRE_HERO_ITEM)
			|| m_Flag.test(Effect::EFFECT_CLASS_OUSTERS_HERO_ITEM)
			|| m_Flag.test(Effect::EFFECT_CLASS_SLAYER_LEVELWAR_HERO_ITEM)
			|| m_Flag.test(Effect::EFFECT_CLASS_VAMPIRE_LEVELWAR_HERO_ITEM)
			|| m_Flag.test(Effect::EFFECT_CLASS_OUSTERS_LEVELWAR_HERO_ITEM)
#endif
			;
}
...
void Creature::setCanAddEffectFlag(Effect::EffectClass EClass)
{
		m_CanAddEffectFlag.set(EClass);
}

void Creature::resetCanAddEffectFlag(Effect::EffectClass EClass)
{
		m_CanAddEffectFlag.reset(EClass);
}

void Creature::setCanAddAllEffectFlag()
{
	for(int i = 0 ; i < m_CanAddEffectFlag.count() ; ++i)
	{
		m_CanAddEffectFlag.set(i);
	}
}

void Creature::resetCanAddAllEffectFlag()
{
	for(int i = 0 ; i < m_CanAddEffectFlag.count() ; ++i)
	{
		m_CanAddEffectFlag.reset(i);
	}
}
