	...
	throw (Error)
{
	...
	else if (pDeadPC->isOusters())
	{
		...
		pOusters->setHP(pOusters->getHP(ATTR_MAX)-pOusters->getSilverDamage(), ATTR_CURRENT);
	}
	...
	try 
	{
		pOldZone->NotifyBeforeLeaveZone(pDeadPC);
		...
		pOldZone->NotifyAfterLeaveZone(pDeadPC);
