...
//	bool removeShrineShield( Zone* pZone ) throw(Error);
	...
	bool putBloodBible(PlayerCreature* pPC, Item* pItem, MonsterCorpse* pCorpse) throw (Error);
...
private:
	...
	int	m_BloodBibleWinPoint[3];
public:
	void resetBloodBibleWinPoint();
	int getBloodBibleWinPoint(int race) { return m_BloodBibleWinPoint[race]; }
	void calcBloodBibleWinPoint(Race_t lostRace, Race_t winRace);	
	int getBloodBibleCount(Race_t winRace);
#endif
};
