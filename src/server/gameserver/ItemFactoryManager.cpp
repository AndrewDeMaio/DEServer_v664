	 ...
	 throw (Error)
{
	...
	m_Factories[Item::ITEM_CLASS_COMMON_QUEST_ITEM] = new CommonQuestItemFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_COMMON_QUEST_ITEM]->getItemClassName()] = Item::ITEM_CLASS_COMMON_QUEST_ITEM;

	m_Factories[Item::ITEM_CLASS_ETHEREAL_CHAIN] = new EtherealChainFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_ETHEREAL_CHAIN]->getItemClassName()] = Item::ITEM_CLASS_ETHEREAL_CHAIN;

	// 2006.05.22 결속의 목걸이 추가
	m_Factories[Item::ITEM_CLASS_OUSTERS_HARMONIC_PENDENT] = new OustersHarmonicPendentFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_OUSTERS_HARMONIC_PENDENT]->getItemClassName()] = Item::ITEM_CLASS_OUSTERS_HARMONIC_PENDENT;

	m_Factories[Item::ITEM_CLASS_CHECK_MONEY] = new CheckMoneyFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_CHECK_MONEY]->getItemClassName()] = Item::ITEM_CLASS_CHECK_MONEY;

	m_Factories[Item::ITEM_CLASS_CUE_OF_ADAM] = new CueOfAdamFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_CUE_OF_ADAM]->getItemClassName()] = Item::ITEM_CLASS_CUE_OF_ADAM;

	m_Factories[Item::ITEM_CLASS_CONTRACT_OF_BLOOD] = new ContractOfBloodFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_CONTRACT_OF_BLOOD]->getItemClassName()] = Item::ITEM_CLASS_CONTRACT_OF_BLOOD;

	
	//20070821
	m_Factories[Item::ITEM_CLASS_SKILL_BOOK] = new SkillBookFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_SKILL_BOOK]->getItemClassName()] = Item::ITEM_CLASS_SKILL_BOOK;

	m_Factories[Item::ITEM_CLASS_VAMPIRE_WING_ITEM] = new VampireWingItemFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_VAMPIRE_WING_ITEM]->getItemClassName()] = Item::ITEM_CLASS_VAMPIRE_WING_ITEM;

	m_Factories[Item::ITEM_CLASS_OUSTERS_WING_ITEM] = new OustersWingItemFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_OUSTERS_WING_ITEM]->getItemClassName()] = Item::ITEM_CLASS_OUSTERS_WING_ITEM;
	
	m_Factories[Item::ITEM_CLASS_SLAYER_TUNNING_ITEM] = new SlayerTunningItemFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_SLAYER_TUNNING_ITEM]->getItemClassName()] = Item::ITEM_CLASS_SLAYER_TUNNING_ITEM;

	m_Factories[Item::ITEM_CLASS_VAMPIRE_TUNNING_ITEM] = new VampireTunningItemFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_VAMPIRE_TUNNING_ITEM]->getItemClassName()] = Item::ITEM_CLASS_VAMPIRE_TUNNING_ITEM;

	m_Factories[Item::ITEM_CLASS_OUSTERS_TUNNING_ITEM] = new OustersTunningItemFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_OUSTERS_TUNNING_ITEM]->getItemClassName()] = Item::ITEM_CLASS_OUSTERS_TUNNING_ITEM;

	m_Factories[Item::ITEM_CLASS_CALLNPC_CARD] = new CallNPCCardFactory();
	m_ItemClassMap[m_Factories[Item::ITEM_CLASS_CALLNPC_CARD]->getItemClassName()] = Item::ITEM_CLASS_CALLNPC_CARD;
	   ...
	   throw (NoSuchElementException, Error)
{
	...
	// getItemInfo() returns NULL when the *Info table has no row for this type
	// (e.g. WarItemInfo holds only type 0, yet hero-item creation asks for 4).
	// Dereferencing it segfaulted during zone init.
	ItemInfo* pItemInfo = g_pItemInfoManager->getItemInfo( IClass, ItemType );
	if (pItemInfo != NULL)
		pItem->setUnique( pItemInfo->isUnique() );
	else
		filelog("itembug.log", "ItemFactoryManager::createItem() : no ItemInfo for class %d type %d",
			(int)IClass, (int)ItemType);
