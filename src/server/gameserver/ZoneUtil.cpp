	...
	throw (ProtocolException, Error)
{
	...
	if (pCreature->isDead() 
		...
		|| pCreature->isFlag(Effect::EFFECT_CLASS_IMMUNE_TO_KNOCKBACK)
		)
	{
		return UP;
	}
	...
	if ( pCreature->isMonster() )
	{
		...
		// 헬가든의 몬스터는 넉백 되지 않는다
		if ( pMonster->getMonsterType() >= 821 && pMonster->getMonsterType() <= 921 ) return UP;
...
//		if ( pMonster->hasRelic() || pMonster->getBrain() == NULL ) return UP;
	...
	// 넉백될 위치가  안전지대라면 넉백 시키지 않는다.
	if (pZone->getZoneLevel(nx, ny) & SAFE_ZONE) 
	{
		return UP;
	}
	...
	throw (EmptyTileNotExistException, Error)
{
	...
	{
		filelog( "ViewBug.log", "%s : 이미 hide상태입니다.", pCreature->getName().c_str() );
		return;
	}
	...
	if (pt.x != -1)
	{
		...
		pCreature->getViewable()->addViewEffect( Effect::EFFECT_CLASS_HIDE, 0 );
	...
	throw (EmptyTileNotExistException, Error)
{
	...
	if (pt.x != -1) 
	{
		...
		pCreature->getViewable()->removeViewEffect( Effect::EFFECT_CLASS_HIDE, 0 );
...
void addUntransformCreature(Zone* pZone, Creature* pCreature, bool bForce) throw()
{
	...
	if (pt.x != -1) 
	{
		...
		else if (pCreature->isFlag(Effect::EFFECT_CLASS_TRANSFORM_TO_BAT))
		{
			...
			{
				Vampire* pVampire = dynamic_cast<Vampire*>(pCreature);
				pVampire->SetBatType(Vampire::BAT_TYPE_NONE);
		...
		for (ZoneCoord_t iy = max(0, cy - maxViewportUpperHeight - 1), endy = min(pZone->getHeight() - 1, cy + maxViewportLowerHeight + 1) ; iy <= endy ; iy ++) 
		{
			...
			for (; itr != objectList.end() && (*itr)->getObjectPriority() <= OBJECT_PRIORITY_BURROWING_CREATURE; itr ++) 
			{
					...
					(pViewer->getVisionState(cx, cy) >= IN_SIGHT)) 
				{
					...
					//cout << "checking" << endl;
					// pViewer가 pCreature를 원래 볼 수 있었다면
					if ( !pCreature->isFlag(Effect::EFFECT_CLASS_HIDE)
	...
	throw()
{
	...
	makeGCAddInstalledMineToZone(&gcAddMine, pMine, cx, cy);
...
//			if( tileX != X || tileY != Y ) checkMine( pZone, tileX, tileY );
			...
			for (slist<Object*>::const_iterator itr = oList.begin(); itr != oList.end(); itr++)
			{
				...
				if (pObject->getObjectClass() == Object::OBJECT_CLASS_CREATURE)
				{
					...
					static bool bNonPK = g_pGameServerInfoManager->getGameServerInfo( 1, g_pConfig->getPropertyInt( "ServerID" ), g_pConfig->getPropertyInt( "WorldID" ) )->isNonPKServer();
					bool cannotPK = bNonPK || GDRLairManager::Instance().isGDRLairZone( pZone->getZoneID() );

					if ( cannotPK && pTargetCreature->isPC() ) continue;
...
EventTransport* createEventTransport(GamePlayer *pGamePlayer)
{
	if ( pGamePlayer == NULL )
	{
		return NULL;
	...
	pEventTransport = dynamic_cast<EventTransport*>(pGamePlayer->getEvent(Event::EVENT_CLASS_TRANSPORT));
	if ( pEventTransport == NULL )
	{
		pEventTransport = new EventTransport(pGamePlayer);
	}
	
	return pEventTransport;
}
	...
	throw()
{
	...
	if ( !pNewZone->NotifyBeforeEnterZone(pCreature) )
	{
		// 입장 조건 해당 안됨
		return;
	}
...
//			}
	...
	try 
	{
		...
		if ( pCreature->isFlag( Effect::EFFECT_CLASS_HAS_BLOOD_BIBLE ) 
			...
			|| pCreature->isFlag( Effect::EFFECT_CLASS_DRAGON_EYE )
			 || pCreature->isFlag( Effect::EFFECT_CLASS_VAMPIRE_HERO_ITEM ) || pCreature->isFlag( Effect::EFFECT_CLASS_SLAYER_HERO_ITEM )
		...
		{
			if( pZone->isLevelWarZone() )
			{
				if( !pZoneInfo->isLevelWarZone() )
					dropRelicToZone( pCreature );
			}
		}
#endif
		...
		if ( pCreature->isFlag( Effect::EFFECT_CLASS_MENEGROTH_DOUNGEON) && 
				!g_MenegrothDoungeonManager.isIncludedZone(TargetZoneID))
		{
			pCreature->removeFlag(Effect::EFFECT_CLASS_MENEGROTH_DOUNGEON);
			pCreature->deleteEffect(Effect::EFFECT_CLASS_MENEGROTH_DOUNGEON);

			char message[512];
			sprintf(message, g_pStringPool->c_str(STRID_MENEGROTH_DOUNGEON_LIMITED_TIMER), 0);
		...
		if (pCreature->isOusters() && pCreature->isFlag(Effect::EFFECT_CLASS_SQUALLY_BARRIER1))
		{
			Ousters* pOusters = dynamic_cast<Ousters*>(pCreature);
		...
		if (pCreature->isOusters() && pCreature->isFlag(Effect::EFFECT_CLASS_SQUALLY_BARRIER2))
		{
			Ousters* pOusters = dynamic_cast<Ousters*>(pCreature);
			Assert( pOusters != NULL );
	...
	pZone->NotifyBeforeLeaveZone(pCreature);
	...
	*/
	...
	pZone->NotifyAfterLeaveZone(pCreature);
