...
#ifdef __CONTRIBUTION_SYSTEM__
	m_LevelWarItemManager.init(m_Level, this);
#endif
	...
	throw(Error)
{
	...
	if ( m_bHasWar )
	{
		...
		int lastSec = dt.secsTo(m_GiveContributePointTime);
		if( lastSec < 0 )
		{
			setGiveContributeTime(dt.addSecs(10 * 60));
			m_pZone->giveLevelWarContributePoint();
		}
#endif
...
{ 
	...
	killAllMonsters();
#ifdef __CONTRIBUTION_SYSTEM__
	m_SweeperSetManager.resetSweeperWinPoint();
	m_LevelWarItemManager.addAllHeroItemsToZone();
	VSDateTime dt(VSDateTime::currentDateTime());
	dt = dt.addSecs((10 * 60) - 10); // 마지막 시점이 애매할 수 있으므로 10초정도 미리 주자 
	setGiveContributeTime(dt); // 10분에 한번씩 기여도 점수 부여하기 위해서
#endif
...
#ifdef __CONTRIBUTION_SYSTEM__
	m_pZone->giveLevelWarReward();
	m_LevelWarItemManager.removeAllHeroItems();
#endif
	resumeTimeband();
...
#ifdef __CONTRIBUTION_SYSTEM__
	if(now.time().hour() > 20)
		return VSDateTime( now.date().addDays(1), VSTime( LevelWarTime[0][1], LevelWarTime[0][2] ) );
	else
		return VSDateTime( now.date(), VSTime( LevelWarTime[0][1], LevelWarTime[0][2] ) );
#endif
...
#ifdef __CONTRIBUTION_SYSTEM__
void LevelWarManager::setGiveContributeTime(const VSDateTime &dt) 
throw(Error)
{
	m_GiveContributePointTime = dt; 
}

int LevelWarManager::expBonusTimeCheck(Race_t race)
throw(Error)
{
	__BEGIN_TRY
	...
	if ( hour >=  21 && hour <= 23 )
	{
		return getSweeperWinPoint(race);
	}
...
int LevelWarManager::getSweeperWinPoint(Race_t race)
{
	...
	for (; itr!=endItr; itr++)
	{
		if ( itr->second->getRace() == race && itr->second->getLevel() == (m_Level - 1) )
		{
			nSweeperWinPoint++;
		}
	}
