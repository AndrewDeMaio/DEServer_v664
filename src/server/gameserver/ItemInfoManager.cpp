	...
	throw(Error)
{
	...
	// 2006.05.22 결속의 펜던트 추가
	g_pOustersHarmonicPendentInfoManager = new OustersHarmonicPendentInfoManager ();
	g_pOustersHarmonicPendentInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_OUSTERS_HARMONIC_PENDENT] = g_pOustersHarmonicPendentInfoManager;
	OustersHarmonicPendent::initItemIDRegistry();
	...
	g_pCommonQuestItemInfoManager = new CommonQuestItemInfoManager();
	g_pCommonQuestItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_COMMON_QUEST_ITEM] = g_pCommonQuestItemInfoManager;
	CommonQuestItem::initItemIDRegistry();

	g_pEtherealChainInfoManager = new EtherealChainInfoManager();
	g_pEtherealChainInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_ETHEREAL_CHAIN] = g_pEtherealChainInfoManager;
	EtherealChain::initItemIDRegistry();

	g_pCheckMoneyInfoManager = new CheckMoneyInfoManager();
	g_pCheckMoneyInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_CHECK_MONEY] = g_pCheckMoneyInfoManager;
	CheckMoney::initItemIDRegistry();

	g_pCueOfAdamInfoManager = new CueOfAdamInfoManager();
	g_pCueOfAdamInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_CUE_OF_ADAM] = g_pCueOfAdamInfoManager;
	CueOfAdam::initItemIDRegistry();

	g_pContractOfBloodInfoManager = new ContractOfBloodInfoManager();
	g_pContractOfBloodInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_CONTRACT_OF_BLOOD] = g_pContractOfBloodInfoManager;
	ContractOfBlood::initItemIDRegistry();

	//20070821
	g_pSkillBookInfoManager = new SkillBookInfoManager();
	g_pSkillBookInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_SKILL_BOOK] = g_pSkillBookInfoManager;
	...
	m_InfoClassManagers[Item::ITEM_CLASS_VAMPIRE_WING_ITEM] = g_pVampireWingItemInfoManager;
	...
	m_InfoClassManagers[Item::ITEM_CLASS_OUSTERS_WING_ITEM] = g_pOustersWingItemInfoManager;
	OustersWingItem::initItemIDRegistry();

	g_pSlayerTunningItemInfoManager = new SlayerTunningItemInfoManager();
	g_pSlayerTunningItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_SLAYER_TUNNING_ITEM] = g_pSlayerTunningItemInfoManager;
	SlayerTunningItem::initItemIDRegistry();

	g_pVampireTunningItemInfoManager = new VampireTunningItemInfoManager();
	g_pVampireTunningItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_VAMPIRE_TUNNING_ITEM] = g_pVampireTunningItemInfoManager;
	VampireTunningItem::initItemIDRegistry();

	g_pOustersTunningItemInfoManager = new OustersTunningItemInfoManager();
	g_pOustersTunningItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_OUSTERS_TUNNING_ITEM] = g_pOustersTunningItemInfoManager;
	OustersTunningItem::initItemIDRegistry();
	
	g_pCallNPCCardInfoManager = new CallNPCCardInfoManager();
	g_pCallNPCCardInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_CALLNPC_CARD] = g_pCallNPCCardInfoManager;
	CallNPCCard::initItemIDRegistry();
	...
	throw()
{
	...
	if ( IClass >= Item::ITEM_CLASS_MAX
		|| m_InfoClassManagers[IClass] == NULL )
	{
		return false;
	}
