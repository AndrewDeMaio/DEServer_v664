...
Price_t PriceManager::getPrice(Item* pItem, MarketCond_t nDiscount, ShopRackType_t shopType, Creature* pCreature) const
{
	...
	if ( pItem->getItemClass() == Item::ITEM_CLASS_MOON_CARD && pItem->getItemType() == 16 )
	{
		return (Price_t)g_pVariableManager->getVariable(GOLD_CLOVER_PRICE);
	}
...
//		originalPrice = getPercentValue( originalPrice, gradePercent );
	...
	if (pItem->getItemClass() == Item::ITEM_CLASS_POTION || pItem->getItemClass() == Item::ITEM_CLASS_SERUM 
		|| pItem->getItemClass() == Item::ITEM_CLASS_COMPOS_MEI || pItem->getItemClass() == Item::ITEM_CLASS_PUPA 
		|| pItem->getItemClass() == Item::ITEM_CLASS_LARVA )
	{
		if ( pCreature->isPC() )
		{
			PlayerCreature* pPC = dynamic_cast<PlayerCreature*>(pCreature);
...
//		20080306 가격 버그 수정
//		static hash_map<Level_t, Price_t> sPriceMap;
...
//		if ( price == 0 )
//		{
			// 2006.05.12 포탈 아이템 UVT-7000과 UVT-슈프림 수리 가격
			if( pItem->getItemType() == 3 || pItem->getItemType() == 4 )
				price = (Price_t)(pow((double)level, 1.5) * 100 );
			else
				price = (Price_t)(pow((double)level, 1.25) * 100 );
//			sPriceMap[level] = price;
//		}

		return (MaxCharge - CurCharge) * price;
	}
...
Price_t PriceManager::getSilverCoatingPrice(Item* pItem, Creature* pCreature) const
{
	...
	return max(0, (int)finalPrice*10);
}
...
*/
	...
	switch ( itemClass )
	{
		...
		case Item::ITEM_CLASS_OUSTERS_ARMSBAND:
			finalPrice = 680000 + ( level * 3000 );
			break;

		case Item::ITEM_CLASS_VAMPIRE_WEAPON:
			finalPrice = 680000 + ( level * 6000 );
			break;
