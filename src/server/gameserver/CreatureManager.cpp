...
public:
	...
	bool operator () (pair<const long unsigned int , Creature *> itr) throw ()
	{
		...
		if ( pCreature != NULL )
		{
			return pCreature->getName() == m_Name;
		}
	...
	throw (Error)
{
	...
	Creature* pCreatureToBeSend = NULL; 
	
	for (; itr != m_Creatures.end() ; itr ++) 
	{
		pCreatureToBeSend = itr->second;
		if ( pCreatureToBeSend == NULL || !pCreatureToBeSend->isPC() )
		{
			continue;
		}

		if (pCreatureToBeSend != pCreature)
		{
			Player* pPlayer = pCreatureToBeSend->getPlayer();
