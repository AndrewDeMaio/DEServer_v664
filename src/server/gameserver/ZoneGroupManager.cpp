	...
	throw (Error)
{
	...
	BEGIN_DB
	{
		...
		while (pResult->next())
		{
			...
			filelog("ZoneGroupInfo.txt", "ZoneGroupID : %d", ID);
		}
	...
	for (; itr != ZoneGroupIDList.end(); itr++)
	{
		...
		filelog("ZoneGroup.txt", "ZoneGroupID : %d", ID);
	...
	throw (Error)
{
	...
	filelog("ZoneGroup.txt", "addZoneGroup ZoneGroupID : %d", pZoneGroup->getZoneGroupID());
	...
	throw (Error)
{
	...
	for (; itr != m_ZoneGroups.end(); itr++)
	{
		...
		pZoneGroup->pushBroadcastPacket( pPacket, pFilter );
	}	
