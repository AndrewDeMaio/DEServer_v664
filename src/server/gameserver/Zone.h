...
typedef hash_map<string, Creature*> TPCLogoutQueue;

class Zone 
{
public:
#ifdef __USE_ENCRYPTER__
	typedef pair<BroadcastFilter*, SocketEncryptOutputStream*> PairFilterStream;
#else
...
public:
	...
	void pushLogoutPC(Creature* pCreature) throw(Error);
...
public:
	...
	// ���� ��ε�ĳ��Ʈ �Լ�
	void pushBroadcastPacket(Packet* pPacket, BroadcastFilter* pFilter) throw(Error);
...
//	void updateDetectScan(Creature* pCreature) throw(ProtocolException, Error);
	...
	void updateViewAbility(Creature* pCreature, const ViewAbility& prevView);
	void updateViewable(Creature* pCreature, const Viewable& prevViewable);
...
public:
	...
	MikllizzLairManager* getMikllizzLairManager(void) const { return m_pMikllizzLairManager; }
	// 2007 06 23
	HellGardenTowerManager* getHellGardenTowerManager(void) const { return m_pHellGardenTowerManager; }
	...
	EventSurvivalZoneManager* getEventSurvivalZoneManager(void) const { return m_pEventSurvivalZoneManager; }
#ifdef __MENEGROTH_DOUNGEON_SYSTEM__
	MenegrothDoungeonZoneManager *getMenegrothDoungeonZoneManager(void) const { return m_pMenegrothDoungeonZoneManager; }
#endif

	PVPSystemZoneManager* getPVPSystemZoneManager() const { return m_pPVPSystemZoneManager; }
	...
	//������ �ο�üũ. kim sung yong 20071205
	vector<uint> getPCNumByRace() const	throw(){ return m_pPCManager->getPCNumByRace();}
...
//	void setPremiumZone(bool bPremiumZone=true) throw() { m_bPremiumZone = bPremiumZone; }
	void setPremiumZoneType(BYTE PremiumZoneType=1) throw() { m_PremiumZoneType = PremiumZoneType; }
	BYTE getPremiumZoneType() { return m_PremiumZoneType; }
    bool isPremiumZone() const throw() { return m_PremiumZoneType? true:false; }
	...
	bool isMikllizzLair() const throw() { return m_bMikllizzLair; }
	void setMikllizzLair(bool bMikllizzLair=true) throw() { m_bMikllizzLair = bMikllizzLair; }

	// 2007 06 23
	bool isHellGardenTower() const throw() { return m_bHellGardenTower; }
	void setHellGardenTower(bool bHellGardenTower=true) throw() { m_bHellGardenTower= bHellGardenTower; }

	bool isSurvivalZone() const throw() { return m_bSurvivalZone; }
	void setSurvivalZone(bool bSurvivalZone=true) throw() { m_bSurvivalZone = bSurvivalZone; }
...
public:
	...
	void    giveLevelWarContributePoint()	throw(Error);
	void    giveHolyWarContributePoint()	throw(Error);
	void    giveLevelWarReward() throw(Error);
	bool	hasLevelWar();
	void	warpToLevelWarItemDefaultPosition(Creature* pCreature);
	int expLevelWarBonusTimeCheck(Race_t race);
#endif
	...
	bool IsGiveExperience();
	
	void IncreaseEntranceCount();
	void DecreaseEntranceCount();
	int GetEntranceCount();

protected :
	void OnAttackCreature(Creature *pAttackerCreature, Creature *pDefenderCreature) {}
	void OnKillCreature(Creature *pAttackerCreature, Creature *pDeadCreature) {}
	bool OnBeforeEnterZone(Creature *pCreature) { return true; }
	void OnAfterEnterZone(Creature *pCreature);// {}

	void OnBeforeLeaveZone(Creature *pCreature) {}
	void OnAfterLeaveZone(Creature *pCreature) {}
	
	void OnAddMonster(Monster *pMonster) {}
	bool OnResurrectCreature(Creature *pCreature, ZONE_COORD& zoneCoord) { return false;}
	bool OnSay(Creature *pCreature, string& strMesg)  { return true;}

	bool OnUseItemInGear(Creature *pCreature, ItemClass_t ItemClass, ItemType_t ItemType) { return true;}
	bool OnUseItemInInventory(Creature *pCreature, ItemClass_t ItemClass, ItemType_t ItemType) { return true;}
	bool OnUseItemInQuick(Creature *pCreature, ItemClass_t ItemClass, ItemType_t ItemType) { return true;}
	
	bool OnUseInventorySkill(Creature *pCreature, SkillType_t SkillType) { return true;}
	bool OnUseObjectSkill(Creature *pCreature, SkillType_t SkillType) { return true;}
	bool OnUseNamedSkill(Creature *pCreature, SkillType_t SkillType) { return true;}
	bool OnUseSelfSkill(Creature *pCreature, SkillType_t SkillType) { return true;}
	bool OnUseTileSkill(Creature *pCreature, SkillType_t SkillType) { return true;}
	
	bool OnBeforeUseMonsterSkill(Creature *pCreature, SkillType_t SkillType) { return true;}
	void OnAfterUseMonsterSkill(Creature *pCreature, SkillType_t SkillType) { }
	
	void ProcessLogoutPC();
...
//	EventMonsterManager*   m_pEventMonsterManager;
	...
	TPCLogoutQueue m_PCLogoutQueue;
...
//1222 wlzzi - �����̾� �� ���� //�����̾� �� ������ BYTe��  
//	bool m_bPremiumZone;
	BYTE m_PremiumZoneType;

	// ��Ÿ ����
	bool m_bPKZone;
	bool m_bNoPortalZone;
	bool m_bMasterLair;
	bool m_bMikllizzLair;
	bool m_bSurvivalZone;
	bool m_bCastle;
	bool m_bHolyLand;
	bool m_bCastleZone;
	// 2007 06 23
	bool m_bHellGardenTower;
	...
	// ��Ŭ���� ����
	MikllizzLairManager*	m_pMikllizzLairManager;

	// �̺�Ʈ �����̹� ��
	EventSurvivalZoneManager*	m_pEventSurvivalZoneManager;

	// 2007 06 23
	HellGardenTowerManager*	m_pHellGardenTowerManager;

#ifdef __MENEGROTH_DOUNGEON_SYSTEM__
	MenegrothDoungeonZoneManager *m_pMenegrothDoungeonZoneManager;
#endif

	PVPSystemZoneManager* m_pPVPSystemZoneManager;
	...
	list< PairFilterStream > m_BroadcaseQueue;
	
	int m_iEntranceCount;				// ���� ���� ĳ���� ��
};	
