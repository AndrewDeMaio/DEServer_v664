...
// constructor
//----------------------------------------------------------------------
// 2007 01 05 버그 때문에 고침 (서버 뻣었다 ㅠ.,ㅠ)
//EffectRideMotorcycle::EffectRideMotorcycle (Slayer* pOwner, Item* pMotorcycle, ZoneCoord_t motorX, ZoneCoord_t motorY)
EffectRideMotorcycle::EffectRideMotorcycle (ObjectID_t ownerOID, Zone* pZone, Item* pMotorcycle, ZoneCoord_t motorX, ZoneCoord_t motorY)
	throw (Error)
...
: Effect(pZone,motorX,motorY,pZone->getCreature(ownerOID),0), m_MotorX(motorX), m_MotorY(motorY), m_OwnerObjectID(ownerOID), m_pMotorcycle(pMotorcycle)
{
	__BEGIN_TRY
...
//	Slayer* pOwner = dynamic_cast<Slayer*>(m_pTarget);
	Slayer* pOwner = dynamic_cast<Slayer*>(getZone()->getCreature( m_OwnerObjectID ));
	...
	throw ()
{
	...
	if (m_OwnerObjectID) msg << ",Target ObjectID :" << m_OwnerObjectID;//m_pTarget->toString();
