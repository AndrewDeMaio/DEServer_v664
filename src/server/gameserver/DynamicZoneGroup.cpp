...
void DynamicZoneGroup::deleteDynamicZone(ZoneID_t ZoneID )
{
	HashMapDynamicZoneItor itr = m_DynamicZones.find( ZoneID );

	if ( itr != m_DynamicZones.end() )
	{
		SAFE_DELETE(itr->second);

		m_DynamicZones.erase(itr);
	}
}
...
DynamicZone* DynamicZoneGroup::getAvailableDynamicZone()
{
	...
	pDynamicZone->setDynamicZoneGroup(this);
