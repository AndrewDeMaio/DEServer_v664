...
public:
	void loadFromFile(ifstream& file) throw();
	void parseString(const string& text) throw();
	bool getRandomItem(ITEM_TEMPLATE* pTemplate, bool bPremium) throw();
	bool getRandomItem(ITEM_TEMPLATE* pTemplate, int nPercent, bool bPremium) throw();
	string toString(void) const throw();
//	XMLTree*	makeXMLTree() const;
...
public:
	...
	int m_PremiumItemRatio; // 프리미엄 사용자의 아이템 자체가 나올 확률
	int m_PremiumOptionRatio; // 프리미엄 사용자의 아이템에 옵션이 붙을 확률
