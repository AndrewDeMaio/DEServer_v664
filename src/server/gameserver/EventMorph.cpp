	...
	throw (Error)
{
	...
	// 20070822 변신 될 당시에 EffectBonusExp를 삭제하여 준다. 문제가 된다고 해서 급조 ㅠ.,ㅠ 생각해서 만든건대... 어쨋든....
/*	BEGIN_DB
	{
		pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
		pStmt->executeQuery( "DELETE FROM EffectBonusExp where Name='%s'", pSlayer->getName().c_str() );
		SAFE_DELETE( pStmt );
	}
	END_DB(pStmt) */
