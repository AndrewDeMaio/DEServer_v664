		...
		if (tile.hasItem()) 
		{
			...
			if (pTempItem != NULL)
			{
				...
				if (pTempItem->getObjectID() == m_ObjectID)
				{
					...
					{
						TPOINT pt = m_pTargetZone->addItem( pTempItem, x, y );
						...
						if ( pOwner != NULL )
						{
							...
							EffectRideMotorcycle* pEffect = new EffectRideMotorcycle( m_OwnerOID, m_pTargetZone, pTempItem, pt.x, pt.y );
					...
					else
					{
						...
						{
							//EffectRideMotorcycle* pEffect = new EffectRideMotorcycle( pOwner, pTempItem, x, y );
							EffectRideMotorcycle* pEffect = new EffectRideMotorcycle( m_OwnerOID, m_pTargetZone, pTempItem, x, y );
							pEffect->setDeadline(0);
							m_pTargetZone->registerObject( pEffect );
