...
public:
	...
	// 2007 06 21 Add T_T
	int getEnhanceDecreaseDamage(void) const { return m_EnhanceDecreaseDamage; }
	int getEnhanceExp(void) const { return m_EnhanceExp; }
	int getEnhanceDelay(void) const { return m_EnhanceDelay; }
	int getEnhanceADelay(void) const { return m_EnhanceADelay; }
	int getEnhanceSight(void) const { return m_EnhanceSight; }
	int getEnhanceNextRatio(void) const { return m_EnhanceNextRatio; }
...
private:
	...
	// 2007 06 21 Add T_T
	int				   m_EnhanceDecreaseDamage;		// 데미지 감소
	int                m_EnhanceExp; 	            // 경험치배율 강화 배수
	int	m_EnhanceDelay;		// AI 속도 강화수치
	int m_EnhanceADelay;	// 공격 속도 강화수치
	int m_EnhanceSight;		// 시야  강화수치
	...
	int               	m_EnhanceNextRatio; // 다음 옵션 적용 확률
};
...
private:
	typedef vector<MonsterType_t> TMonsterList;
	typedef hash_map<SpriteType_t, TMonsterList> TMonsterSpriteSet;

	uint                  m_MaxMonsterType;      // size of MonsterInfo* array
	MonsterInfo**         m_MonsterInfos;          // array of monster info
	TMonsterSpriteSet m_MonsterSpriteSet;
