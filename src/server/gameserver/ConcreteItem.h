...
public:
	...
	virtual ~ConcreteItem() throw() {}	//by kim
	// 가상 함수 구체화
...
public:
	...
	// 옵션 관련
	bool						hasThirdOptionType() const throw() { return m_Option.hasThirdOptionType(); }
	int							getThirdOptionTypeSize() const throw() { return m_Option.getThirdOptionTypeSize(); }
	const list<OptionType_t>&	getThirdOptionTypeList() const throw() { return m_Option.getThirdOptionTypeList(); }
	void						setThirdOptionType(const list<OptionType_t>& OptionType) throw() { m_Option.setThirdOptionType(OptionType); }
	BYTE						getThirdEnchantType() const throw() { return m_Option.getThirdEnchantType(); }
	void						setThirdEnchantType(const BYTE thirdEnchantType) throw() { m_Option.setThirdEnchantType(thirdEnchantType); }
	void						clearThirdOption() throw() { m_Option.clearThirdOption(); }
#endif
public:
	// 아이템 등급 관련
...
public:
	...
	int			getMagicProtectionBonus() const throw(Error) { return max(0,((int)getItemInfo()->getMagicProtectionBonus())) ; }
