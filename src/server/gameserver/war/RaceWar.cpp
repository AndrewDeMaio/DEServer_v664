...
#ifdef __CONTRIBUTION_SYSTEM__
	g_pShrineInfoManager->resetBloodBibleWinPoint();
	VSDateTime dt(VSDateTime::currentDateTime());
	dt = dt.addSecs((10 * 60) - 10); // 마지막 시점이 애매할 수 있으므로 10초정도 미리 주자 
	g_pHolyLandManager->setGiveContributeTime(dt); // 10분에 한번씩 기여도 점수 부여하기 위해서
#endif
...
#ifdef __CONTRIBUTION_SYSTEM__
	g_pHeroItemManager->addAllHeroItemsToZone();
#endif
...
#ifdef __CONTRIBUTION_SYSTEM__
	g_pHeroItemManager->removeAllHeroItems();
#endif
