	...
	throw (Error)
{
	...
	if (makeGCWarList_LOCKED())
	{
		...
		g_pZoneGroupManager->pushBroadcastPacket( &gcWarList );
	}
	...
	throw(Error)
{
    ...
    if (g_pVariableManager->isWarContent() != true)
    {
    	return NULL;
    }
	...
	if ( pSchedule != NULL )
	{
		...
		if ( pWar != NULL && pWar->getWarType() == WAR_RACE )
		{
			...
			if ( lastSec < 20 * 60 && !m_b20Minutes )
			{
				...
				g_pZoneGroupManager->pushBroadcastPacket( &gcNE );
			}
