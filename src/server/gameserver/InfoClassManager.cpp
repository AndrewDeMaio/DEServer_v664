	...
	throw (Error)
{
	...
	for (uint i=0; i<=m_InfoCount; i++)
	{
		// *Info tables can be sparse (ItemType gaps) or entirely empty when the
		// feature has no content configured, which leaves NULL slots here. The
		// original code dereferenced unconditionally and segfaulted.
		if (m_pItemInfos[i] == NULL) continue;
	...
	// An empty *Info table leaves every slot NULL. That is legitimate, so only
	// assert slot 0 exists when rows were actually loaded.
	if (m_InfoCount > 0)
...
//	Assert(pItemInfo->getItemType() < Item::ITEM_CLASS_MAX);	by kim 2021-08-17 주석
	...
	throw (NoSuchElementException , Error)
{
	...
	// Bounds check FIRST: the old code indexed m_pItemInfos[itemType] inside an
	// Assert before checking the range, so an out-of-range type read out of
	// bounds. A missing entry also asserted (aborting the server) even though
	// this function already returns NULL for unknown types and callers handle it.
	if ( m_InfoCount < itemType )
	{
		return NULL;
	}

	if ( m_pItemInfos[itemType] == NULL )
	{
		// no *Info row for this type (e.g. hero items whose table has one row)
		return NULL;
	}
